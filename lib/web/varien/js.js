    ...
    if (_options['tr td']) {
      ...
      if (allRows.length) {
        for (var i = 0; i < allRows.length; i++) {
          decorateGeneric(
            allRows[i].getElementsByTagName('TD'),
            _options['tr td'],
          );
        }
      }
...
function formatCurrency(price, format, showPlus) {
  var precision = isNaN((format.precision = Math.abs(format.precision)))
    ? 2
    : format.precision;
  var requiredPrecision = isNaN(
  ...
  validate: function() {
    ...
    if (
      ...
      if (day < 1 || day > countDaysInMonth) {
        ...
        var testFull =
          this.month.value + '/' + this.day.value + '/' + this.year.value;
