define(['jquery'], function($) {
  ...
  $.fn.multiselect = function(opts) {
    var o = $.extend(
      {
        ...
        layout:
          '<section class="block %mselectListClass%">' +
          ...
          '</section>',
        item:
          '<div  class="%mselectListItemClass% %mselectDisabledClass% %iseditable% %isremovable%"><label><input type="checkbox" class="%mselectCheckedClass%" value="%value%" %checked% %disabled% /><span>%label%</span></label>' +
    ...
    return this.filter('select[multiple]:not(.' + o.mselectHiddenClass + ')')
      .each(function() {
        var select = $(this)
            .addClass(o.mselectHiddenClass)
            .hide(),
          ...
          items = (function() {
            ...
            select.children('option').each(function(i, option) {
              ...
              str += o.item
                ...
                .replace(
                  /%mselectCheckedClass%/gi,
                  option.attr('selected')
                    ? '' + o.mselectCheckedClass + ''
                    : '',
                )
                .replace(
                  /%disabled%/gi,
                ...
                .replace(
                  /%mselectDisabledClass%/gi,
                  option.attr('disabled')
                    ? '' + o.mselectDisabledClass + ''
                    : '',
                )
                .replace(/%mselectListItemClass%/gi, o.mselectListItemClass)
                .replace(
                  /%iseditable%/gi,
                  option.attr('data-is-editable')
                    ? '' + o.mselectItemNotEditableClass + ''
                    : '',
                )
                .replace(
                  /%isremovable%/i,
                  option.attr('data-is-removable')
                    ? '' + o.mselectItemNotRemovableClass + ''
                    : '',
                )
          ...
          html = o.layout
            ...
            .replace(
              /%mselectInputContainerClass%/gi,
              o.mselectInputContainerClass,
            )
          ...
          widget = $(html)
            ...
            .on('change.mselectCheck', '[type=checkbox]', function() {
              var checkbox = $(this),
                index = checkbox
                  .closest('.' + o.mselectListItemClass + '')
                  .index();

              select
                .find('option')
                .eq(index)
                .attr('selected', !!checkbox.attr('checked'));
            }),
          list = widget.find('.' + o.mselectItemsWrapperClass + ''),
          buttonAdd = widget
            .find('.' + o.mselectButtonAddClass + '')
          ...
          input = container
            .find('[type=text].' + o.mselectInputClass + '')
          ...
          buttonSave = container
            .find('.' + o.mselectButtonSaveClass + '')
            .on('mousedown.mselectSave', function(e) {
              append(input.val());
            }),
          buttonCancel = container
            .find('.' + o.mselectButtonCancelClass + '')
          ...
          append = function(v) {
            ...
            $.each(
              typeof o.parse == 'function' ? o.parse(v) : [$.trim(v)],
              function(i, v) {
                var item;
                ...
                if (v && !select.children('[value="' + v + '"]').length) {
                  item = $(
                    o.item
                      .replace(/%value%|%label%/gi, v)
                      .replace(
                        /%mselectDisabledClass%|%iseditable%|%isremovable%/gi,
                        '',
                      )
                      .replace(
                        /%mselectListItemClass%/gi,
                        o.mselectListItemClass,
                      ),
                  )
                    .find('[type=checkbox]')
                    .attr('checked', true)
                  ...
                  list.children('.' + o.mselectListItemClass + '').length
                    ? list
                        .children('.' + o.mselectListItemClass + ':last')
                        .after(item)
                    : list.prepend(item);
          ...
          reset = function() {
            ...
            if (
              ch.length >= size &&
              !list.hasClass(o.mselectItemsWrapperOverflowClass)
            ) {
              list
                .height(
                  list
                    .children('.' + o.mselectListItemClass + ':first')
