...
(function(root, factory) {
  ...
  $.widget('mage.zoom', {
    ...
    _render: function() {
      ...
      this.element.append(
        this._renderControl('track').append(this._renderControl('lens')),
      );
      this.element
        .append(this._renderControl('display'))
        .find(this.options.selectors.zoomInner)
        .append(this._renderLargeImage());
      noticeContainer = this.element.find(
        this.options.controls.notice.container,
      );
      noticeContainer = noticeContainer.length ? noticeContainer : this.element;
      ...
      switch (control) {
        case 'display':
          position = {
            my:
              'left+' +
              this.options.controls.display.left +
              ' top+' +
    ...
    _bind: function() {
      ...
      events[this.options.startZoomEvent + ' ' + this.options.selectors.image] =
        'show';

      /** Handler */
      events[
        this.options.stopZoomEvent + ' ' + this.options.selectors.track
      ] = function() {
    ...
    _setZoomData: function() {
      ...
      this.largeImageSrc =
        this.options.largeImage || this.element.find(this.image).data('large');
    },
    ...
    _largeImageLoaded: function() {
      ...
      this.largeImageSize = {
        ...
        height:
          this.largeImage.height() || this.largeImage.get(0).naturalHeight,
      };
      this.ratio = null;
      this._toggleNotice();
      $(this.options.selectors.image).trigger('processStop');
  ...
  $.widget('mage.zoom', $.mage.zoom, {
    ...
    _getWhiteBordersOffset: function() {
      var ratio = this.getZoomRatio(),
        ...
        offsetLeft =
          width - largeWidth > 0 ? Math.ceil((width - largeWidth) / 2) : 0,
        offsetTop =
          height - largeHeight > 0 ? Math.ceil((height - largeHeight) / 2) : 0;
    ...
    _largeImageLoaded: function() {
      ...
      if (
        this._getAspectRatio(this.image) !==
        this._getAspectRatio(this.largeImage)
      ) {
