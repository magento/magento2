define([
    ...
    /**
     * Polyfill for the native 'Element.matches' method.
     */
    matchesSelector = (function () {
        ...
        methodName = _.find([
                ...
                return typeof elemProto[name] == 'function';
            });

        if (methodName) {
            return function (elem, selector) {
                return elem[methodName](selector);
            };
        }

        return function (elem, selector) {
            var parent = elem.parentNode || document,
                nodes = parent.querySelectorAll(':scope > ' + selector);

            return !!~_.toArray(nodes).indexOf(elem);
        };
    })();
    ...
    function processAdded(node) {
        _.each(queries.selectors, function (listeners, selector) {
            if (!matchesSelector(node, selector)) {
                return;
            }

            listeners.forEach(function (data) {
                if (data.ctx && !data.ctx.contains(node)) {
                    return;
                }
    ...
    function getNodes(selector, ctx) {
        ...
        if (typeof selector === 'object') {
            ...
            nodes = _.toArray(ctx.querySelectorAll(selector));
        }
    ...
    return {
        ...
        off: function (selector, fn) {
            var selectors = queries.selectors,
