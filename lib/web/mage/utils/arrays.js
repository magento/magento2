...
define([
    ...
    function getIndex(elem, container) {
        var index = container.indexOf(elem);

        if (~index) {
            return index;
        }

        return _.findIndex(container, function (item) {
            return item && (item.name === elem || item === elem);
        });
    }
    ...
    return {
       /**
        ...
        insert: function (item, position, container) {
            ...
            return !~currentIndex ? item : false;
        },
