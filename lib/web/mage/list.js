...
define(['jquery', 'mage/template', 'jquery/ui'], function($, mageTemplate) {
  ...
  $.widget('mage.list', {
    ...
    _create: function() {
      ...
      this.element.addClass('list-widget');
      ...
      destination.on(
        'click',
        this.options.removeButton,
        $.proxy(this.removeItem, this),
      );
    },
    ...
    _destroy: function() {
      ...
      this.element.removeClass('list-widget');
      destination.find('[data-role="addedItem"]').remove();
    },
  });
