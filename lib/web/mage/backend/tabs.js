  ...
  $.widget('mage.tabs', $.mage.tabs, {
    ...
    _getPanelForTab: function(tab) {
      ...
      if (!panel.length) {
        ...
        panel = $(this.options.destination).find(
          this._sanitizeSelector('#' + id),
        );
      }
    ...
    _movePanelsInDestination: function(panels) {
      if (
        ...
        !panels.parents(this.options.destination).length
      ) {
        ...
        panels.appendTo(this.options.destination).each(
  ...
  $.widget('mage.tabs', $.mage.tabs, {
    ...
    _refresh: function() {
      ...
      $.each(
        ...
        $.proxy(function(i, tab) {
          $(this._getPanelForTab(tab))
            ...
            .on(
              'highlight.validate' + this.eventNamespace,
              {
                index: i,
              },
      ...
      ($(this.options.destination).is('form')
        ...
        .on(
          'beforeSubmit' + this.eventNamespace,
          $.proxy(this._onBeforeSubmit, this),
        );
    },
    ...
    _onBeforeSubmit: function(e, data) {
      ...
      if (this.options.tabsBlockPrefix) {
        if (activeAnchor.is('[id*="' + this.options.tabsBlockPrefix + '"]')) {
          activeTabId = activeAnchor
            .prop('id')
            .substr(this.options.tabsBlockPrefix.length);
        }
      }
