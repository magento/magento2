...
define([
  ...
  window.EditableMultiselect = function(settings) {
    ...
    this.addButtonCaption =
      this.settings['add_button_caption'] || 'Add new value';
     ...
     */
    EditableMultiselect.prototype.attachEventsToControls = function(
      mselectList,
    ) {
      mselectList.on(
    ...
    EditableMultiselect.prototype.makeMultiselectEditable = function() {
      ...
      selectList
        .find(
          '.mselect-list-item:not(.mselect-list-item-not-editable) label span',
        )
        .editable(this.saveUrl, {
          type: 'text',
          ...
          isChecked: function() {
            ...
            if (
              !that.closest('.mselect-list-item').hasClass('mselect-disabled')
            ) {
              checked = that
                .parent()
                .find('[type=checkbox]')
          ...
          onsubmit: function(sett, original) {
            var select = $(original)
                .closest('.mselect-list')
                .prev(),
              current = $(original)
                .closest('.mselect-list-item')
                .index(),
              entityId = select
                .find('option')
                .eq(current)
            ...
            if (result.success) {
              if (typeof result[entityValueName] === 'string') {
                select
                  .find('option')
                  .eq(current)
                  .val(result[entityIdName])
                  .text(result[entityValueName]);
      ...
      ajaxOptions = {
        ...
        success: function(result) {
          ...
          if (result.success) {
            ...
            mselectItemHtml = $(
              options.item
                .replace(/%value%|%label%/gi, resultEntityValueName)
                .replace(
            ...
            itemsWrapper = sectionBlock.find(
              '.' + options.mselectItemsWrapperClass + '',
            );

            if (
              itemsWrapper.children('.' + options.mselectListItemClass + '')
                .length
            ) {
              itemsWrapper
                .children('.' + options.mselectListItemClass + ':last')
                .after(mselectItemHtml);
            } else {
            ...
            inputSelector =
              '.' +
              options.mselectInputContainerClass +
              ' [type=text].' +
