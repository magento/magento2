...
(function(factory) {
  ...
  $.widget('mage.validation', $.mage.validation, {
    options: {
      ...
      forceIgnore: '',
      ignore:
        ':disabled, .ignore-validate, .no-display.template, ' +
      ...
      highlight: function(element) {
        if (
          $.validator.defaults.highlight &&
          $.isFunction($.validator.defaults.highlight)
        ) {
      ...
      unhighlight: function(element) {
        if (
          $.validator.defaults.unhighlight &&
          $.isFunction($.validator.defaults.unhighlight)
        ) {
    ...
    _create: function() {
      if (
        ...
        $.type(this.options.submitHandler) !== 'function'
      ) {
    ...
    _getByCode: function(code) {
      ...
      element =
        ...
        parent.querySelector('input[name=' + code + ']');
  ...
  _.each(
    {
      'validate-greater-zero-based-on-option': [
        function(v, el) {
          ...
          if (
            optionType &&
            (optionTypeVal == 'checkbox' || optionTypeVal == 'multi') &&
            v <= 0
          ) {
            //eslint-disable-line
            return false;
        ...
        function(v, element) {
          ...
          if (linkType.is(':checked') && (v === '' || v === '[]')) {
            ...
            if (
              ...
              (newFileContainer.empty() || newFileContainer.is(':visible'))
            ) {
