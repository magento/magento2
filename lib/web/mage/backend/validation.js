            ...
            if (this.settings.focusInvalid) {
                try {
                    $(this.errorList.length && this.errorList[0].element || [])
                        ...
                        // Auto focus in the invalid field if the fieldset collapse is closed
                        $('[data-toggle=dropdown].active').trigger('open.dropdown');
                        $('.section-config').each(function () {
                            if ($(this).find('input.mage-error').length && !$(this).hasClass('active')) {
                            $(this).find('a[id$="-head"]').click();
                                $(this).find('input.mage-error:text:visible:first').focus();
                            }
                        });
                } catch (e) {
