...
(function(root, factory) {
  ...
  $.widget('mage.suggest', {
    ...
    options: {
      ...
      className: null,
      inputWrapper:
        '<div class="mage-suggest"><div class="mage-suggest-inner"></div></div>',
     ...
     */
    _destroy: function() {
      this.element.unwrap().removeAttr('autocomplete');
            ...
            keydown: function(event) {
              ...
              switch (event.keyCode) {
                ...
                case keyCode.UP:
                  ...
                  suggestList = event.currentTarget.parentNode.getElementsByTagName(
                    'ul',
                  )[0];
                  hasSuggestedItems =
                    event.currentTarget.parentNode.getElementsByTagName('ul')[0]
                      .children.length >= 0;

                  if (hasSuggestedItems) {
                    selectedItem = $(
                      suggestList.getElementsByClassName('_active')[0],
                    )
                      .removeClass('_active')
                      .prev()
                      .addClass('_active');
                ...
                case keyCode.DOWN:
                  ...
                  if (hasSuggestedItems) {
                    hasSelectedItems =
                      suggestList.getElementsByClassName('_active').length ===
                      0;

                    if (hasSelectedItems) {
                      //eslint-disable-line max-depth
                      selectedItem = $(suggestList.children[0]).addClass(
                        '_active',
                      );
                      event.currentTarget.value = selectedItem.find('a').text();
                    } else {
    ...
    _toggleEnter: function(event) {
      var suggestList, activeItems, selectedItem;

      if (!this.options.submitInputOnEnter) {
        event.preventDefault();
      }

      suggestList = $(event.currentTarget.parentNode)
        .find('ul')
        .first();
      ...
      if (activeItems.length >= 0) {
        ...
        if (
          selectedItem.find('a') &&
          selectedItem.find('a').attr('href') !== undefined
        ) {
    ...
    _bindDropdown: function() {
      ...
      $.each(
        this._control.eventsMap,
        $.proxy(function(suggestEvent, controlEvents) {
          $.each(
            controlEvents,
            $.proxy(function(i, handlerName) {
              switch (suggestEvent) {
                case 'select':
    ...
    _onSelectItem: function(e, item) {
      ...
      if (
        this._trigger('beforeselect', e || null, {
          item: this._focused,
        }) === false
      ) {
        return;
    ...
    _isItemSelected: function(item) {
      return (
        item.id ==
        (this._selectedItem && this._selectedItem.id //eslint-disable-line eqeqeq
    ...
    filter: function(items, term) {
      ...
      return $.grep(itemsArray, function(value) {
  ...
  $.widget('mage.suggest', $.mage.suggest, {
    ...
    _addRecent: function(item) {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(this._recentItems),
      );
    },
  });
  ...
  $.widget('mage.suggest', $.mage.suggest, {
    options: {
      ...
      choiceTemplate:
        '<li class="mage-suggest-choice button"><div><%- text %></div>' +
    ...
    _renderMultiselect: function() {
      ...
      this.elementWrapper = this.element.closest(
        '[data-role="parent-choice-element"]',
      );
      $(function() {
        that._getOptions().each(function(i, option) {
          option = $(option);
          that._createOption({
            id: option.val(),
            label: option.text(),
    ...
    _addOption: function(e, item) {
      this.valueField.append(
        this._createOption(item).data('selectTarget', $(e.target)),
      );
    },
