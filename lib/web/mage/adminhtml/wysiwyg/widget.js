...
define([
            ...
            this.dialogWindow = jQuery('<div/>').modal({
                ...
                opened: function () {
                    var dialog = jQuery(this).addClass('loading magento-message');
                        ...
                        widgetUrl += 'mode/' + mode;new Ajax.Updater($(this), widgetUrl, {
                            evalScripts: true,
    ...
    WysiwygWidget.Widget.prototype = {
            ...
            if (validationResult) {
                ...
                new Ajax.Request($(this.formEl).action, {
                    ...
                    onComplete: function (transport) {
                        try {
                            var bookmarkContent;
                            var editor = wysiwyg.activeEditor();
                            widgetTools.onAjaxSuccess(transport);
                            widgetTools.dialogWindow.modal('closeModal');

                            if (typeof wysiwyg != 'undefined' && editor) {
                                editor.focus();
                                var activeNode = widgetTools.getActiveSelectedNode();
        ...
        updateContent: function (content) {
            ...
            if (this.wysiwygExists()) {
                content = window.wysiwygSetup.prototype.updateContent(content);
