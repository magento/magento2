...
define([
  ...
  WysiwygWidget.Widget.prototype = {
    ...
    getOptionsContainerId: function() {
      return (
        this.widgetOptionsEl.id + '_' + this.widgetEl.value.gsub(/\//, '_')
      );
    },
    ...
    loadOptions: function() {
      ...
      if (!this.widgetEl.value) {
        if (typeName) {
          msgTmpl = jQuery.mage.__(
            'The widget %1 is no longer available. Select a different widget.',
          );
    ...
    getWysiwygNode: function() {
      return (
        widgetTools.getActiveSelectedNode() ||
        wysiwyg.activeEditor().selection.getNode()
      );
    },
  };
