...
define([
...
], function (jQuery, _, tinyMCE4, configDirectiveGenerator, variables) {
    'use strict';
    ...
    tinyMce4Wysiwyg.prototype = {
        ...
        setup: function () {
            ...
            variablePlugin = this.getVariablePluginData(),
            data = this.requestVariablesData(variablePlugin.options.variablesUrl);

            data.done(function (variables) {
                tinyMCE4.init(this.getSettings());
                this.magentoVariables = variables;
            }.bind(this));
        ...
        getSettings: function () {
            ...
            settings = {
                ...
                'content_style': this.config['content_style'],
                adapter: this,
        ...
        encodeVariables: function (content) {
            content = content.gsub(/\{\{config path=\"([^\"]+)\"\}\}/i, function (match) {
                var path = match[1];
                var imageHtml;
                if (this.magentoVariables[match[1]]) {
                    ...
                    imageHtml = '<span id="%id" class="magento-variable magento-variable-lost mceNonEditable">Not found</span>';
                }
         ...
         * @param url
         */
        requestVariablesData: function (url) {
            if (!this.magentoVariablesPlugin) {
                this.magentoVariablesPlugin = jQuery.Deferred();
                ...
                new jQuery.ajax({
                    ...
                    success: function (resp) {
                        this.magentoVariablesPlugin.resolve(resp);
                    },
                    fail: function () {
                        alert('Failed to load Magento Variables Data')
                    },
                });
            }

            return this.magentoVariablesPlugin.promise();
        },

        decodeVariables: function (content) {
            return content.gsub(/<span\sid=\"(.*)\"\sclass=\"magento-variable.*\">.*<\/span>/i, function (match) {
                return configDirectiveGenerator.processConfig(Base64.idDecode(match[1]));
            }.bind(this));
        },
