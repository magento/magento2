...
define([
    ...
    tinyMce4Wysiwyg.prototype = {
        ...
        addpluginToToolbar: function (plugin, separator) {
            ...
            var toolbar = this.config.tinymce4.toolbar.split(' ');
        ...
        decodeWidgets: function (content) {
            return content.gsub(/(<span class="[^"]*magento-widget[^"]*"[^>]*>)?<img([^>]+id="[^>]+)>(([^>]*)<\/span>)?/i, function (match) {
        ...
        encodeVariables: function (content) {
            content = content.gsub(/\{\{config path=\"([^\"]+)\"\}\}/i, function (match) {
                var path = match[1], magentoVariables, imageHtml;
