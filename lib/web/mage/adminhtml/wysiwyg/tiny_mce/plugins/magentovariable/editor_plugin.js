  ...
  return function(config) {
    tinymce.create('tinymce.plugins.magentovariable', {
      ...
      init: function(editor, url) {
         ...
         */
        varienGlobalEvents.attachEventHandler('wysiwygEncodeContent', function(
          content,
        ) {
         ...
         */
        varienGlobalEvents.attachEventHandler('wysiwygDecodeContent', function(
          content,
        ) {
      ...
      encodeVariables: function(content) {
        content = content.gsub(/\{\{config path=\"([^\"]+)\"\}\}/i, function(
          match,
        ) {
          var path = match[1],
            magentoVariables,
        ...
        content = content.gsub(/\{\{customVar code=([^\}\"]+)\}\}/i, function(
          match,
        ) {
          var path = match[1],
            magentoVariables,
      ...
      decodeVariables: function(content) {
        ...
        [].forEach.call(doc.querySelectorAll('span.magento-variable'), function(
          el,
        ) {
          var $el = jQuery(el);
    ...
    tinymce.PluginManager.add(
      'magentovariable',
      tinymce.plugins.magentovariable,
    );
  };
});
