...
/* global tinymce, widgetTools, jQuery, varienGlobalEvents, Base64 */
...
tinymce.create('tinymce.plugins.magentowidget', {
    ...
    init: function (editor, url) {
        ...
        editor.addCommand('mceMagentowidget', function (img) {
            ...
            widgetTools.openDialog(
                this.constructor.config['window_url'] + 'widget_target_id/' + editor.getElement().id + '/'
            );
        }.bind(this));
        ...
        varienGlobalEvents.attachEventHandler('wysiwygClosePopups', function () {
            self.constructor.adapter.closeEditorPopup('widget_window' + self.constructor.adapter.id);
        });
    },
    ...
    encodeWidgets: function (content) {
        return content.gsub(/\{\{widget(.*?)\}\}/i, function (match) {
            var attributes = this.constructor.adapter.parseAttributesString(match[1]),
                imageSrc,
                imageHtml = '';

            if (attributes.type) {
                attributes.type = attributes.type.replace(/\\\\/g, '\\');
                imageSrc = this.constructor.config.placeholders[attributes.type];

                if (this.constructor.config.types.indexOf(attributes['type_name']) > -1) {
                    imageHtml += '<span class="magento-placeholder magento-widget mceNonEditable" ' +
                        'contenteditable="false">';
                } else {
                    imageSrc = this.constructor.config['error_image_url'];
                    imageHtml += '<span ' +
