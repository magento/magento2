...
(function () {
    ...
    tinymce.create('tinymce.plugins.MagentoWidget', {
        ...
        init: function (editor, url) {
            ...
            if (editor.on !== undefined) {
                editor.on('dblClick', function (e) {
                    //executeWidgetSlideoutIfElHasWidgetCode.call(this, e.target);
                    doubleClickPlaceholder(this, e.target);
                });

                // Add a node change handler, selects the button in the UI when a image is selected
                editor.on('nodeChange', function (e) {
                    nodeChanged(this, e.element);
                });
            }
            else
            {
                editor.onDblClick.add(function (edi, e) {
                    doubleClickPlaceholder(edi, e.target)
                });

                // Add a node change handler, selects the button in the UI when a image is selected
                editor.onNodeChange.add(function (edi, button, e) {
                    nodeChanged(button, e.element);
                });
            }

            function nodeChanged(ctrl, placeholder) {
                ctrl.active(false);
            ...
            function doubleClickPlaceholder(edi, placeholder)
            {
                if (isWidgetPlaceholderSelected(placeholder)) {
                    edi.execCommand('mceMagentowidget', getImage(placeholder));
                }
            }
