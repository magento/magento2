  ...
  varienGrid.prototype = {
    ...
    initGrid: function() {
      ...
      if (this.sortVar && this.dirVar) {
        columns = $$(
          '#' + this.containerId + this.tableSufix + ' thead [data-sort]',
        );
    ...
    rowMouseOver: function(event) {
      ...
      if (
        !Element.hasClassName('_clickable') &&
    ...
    doSort: function(event) {
      ...
      if (
        element.readAttribute('data-sort') &&
        element.readAttribute('data-direction')
      ) {
    ...
    reload: function(url, onSuccessCallback) {
      ...
      if (this.useAjax) {
        ...
        jQuery('#' + this.containerId).trigger(
          'gridajaxsettings',
          ajaxSettings,
        );
      ...
      if (jQuery('#' + this.containerId).find('.draggable-handle').length) {
        jQuery('#' + this.containerId)
          ...
          .sortable({
            ...
            update: this.sortableUpdateCallback
              ? this.sortableUpdateCallback
              : function() {},
            tolerance: 'pointer',
          });
      }
  ...
  varienGridMassaction.prototype = {
    ...
    updateCount: function() {
      ...
      if (checkboxesChecked == checkboxesTotal && checkboxesTotal != 0) {
        ...
        this.multiselect
          .addClassName('_checked')
          .removeClassName('_indeterminate');
      } else if (checkboxesChecked == 0) {
        //eslint-disable-line eqeqeq
        this.count.addClassName('_empty');
        this.multiselect
          .removeClassName('_checked')
          .removeClassName('_indeterminate');
      } else {
        this.count.removeClassName('_empty');
        this.multiselect
          .addClassName('_checked')
          .addClassName('_indeterminate');
      }
        ...
        if (checkbox && inputs.length > 0) {
          ...
          for (i = 0; i < inputs.length; i++) {
            ...
            if (
              ...
              inputs[i].value = this.gridData.get(checkbox.value)[
                inputs[i].name
              ];
            }
    ...
    getGridDataHash: function(_object) {
      return $H(
        this.multidimensionalMode
          ? _object
          : this.convertArrayToObject(_object),
      );
    },
