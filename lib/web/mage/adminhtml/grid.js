...
define([
    ...
    varienGrid.prototype = {
        ...
        reload: function (url, onSuccessCallback) {
            var ajaxSettings, ajaxRequest, fragmentPosition, fragment;
            ...
            if (this.useAjax) {
                fragmentPosition = url.indexOf('#');
                fragment = fragmentPosition === -1 ? '' : url.slice(fragmentPosition);
                url = fragmentPosition === -1 ? url : url.slice(0, fragmentPosition);
                ajaxSettings = {
                    url: url + (url.match(new RegExp('\\?')) ? '&ajax=true' : '?ajax=true') + fragment,
                    showLoader: true,
                    method: 'post',
        ...
        _addVarToUrl: function (url, varName, varValue) {
            var escapedName = String(varName).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'),
                re = new RegExp('/' + escapedName + '/[^/]*(?=/)', 'g'),
                fragmentPosition = url.indexOf('#'),
                fragment = fragmentPosition === -1 ? '' : url.slice(fragmentPosition),
                queryPosition,
                query,
                encodedName = encodeURIComponent(varName);

            url = fragmentPosition === -1 ? url : url.slice(0, fragmentPosition);
            queryPosition = url.indexOf('?');
            query = queryPosition === -1 ? [] : url.slice(queryPosition + 1).split('&').filter(function (param) {
                var queryName = param.split('=')[0];

                try {
                    return decodeURIComponent(queryName.replace(/\+/g, ' ')) !== String(varName);
                } catch (e) {
                    return queryName !== encodedName;
                }
            });
            url = (queryPosition === -1 ? url : url.slice(0, queryPosition)).replace(re, '');

            if (String(varValue) === '') {
                query.push(encodedName + '=');
            } else {
        ...
        bindFieldsChange: function () {
            ...
            // eslint-disable-next-line magento-coding-standard-eslint-plugin/jquery-no-input-event-shorthand
