...
define(['jquery', 'prototype', 'mage/adminhtml/events'], function(jQuery) {
  ...
  RegionUpdater.prototype = {
    ...
    initialize: function(
      ...
      this.clearRegionValueOnDisable =
        typeof clearRegionValueOnDisable === 'undefined'
          ? false
          : clearRegionValueOnDisable;
    ...
    _checkRegionRequired: function() {
      ...
      elements.each(function(currentElement) {
        ...
        if (!regionRequired || !currentElement.visible()) {
          ...
          if (
            currentElement.tagName.toLowerCase() == 'select' && //eslint-disable-line eqeqeq
  ...
  FormElementDependenceController.prototype = {
        ...
        for (idFrom in elementsMap[idTo]) {
          ...
          if ($(idFrom)) {
            Event.observe(
              $(idFrom),
              'change',
              this.trackChange.bindAsEventListener(
                this,
                idTo,
                elementsMap[idTo],
              ),
    ...
    trackChange: function(e, idTo, valuesFrom) {
      ...
      for (idFrom in valuesFrom) {
        ...
        if (from) {
          ...
          if (fromId) {
            ...
            isInArray =
              radioFrom.length > 0 && values.indexOf(radioFrom[0].value) !== -1;
            isNegative = valuesFrom[idFrom].negative;
      ...
      isInheritCheckboxChecked =
        $(idTo + '_inherit') && $(idTo + '_inherit').checked;
      target = $(idTo);

      // Target won't always exist (for example, if field type is "label")
      if (target) {
        inputs = target
          .up(this._config['levels_up'])
          .select('input', 'select', 'td');
        isAnInputOrSelect =
          ['input', 'select'].indexOf(target.tagName.toLowerCase()) != -1; //eslint-disable-line
      ...
      if (shouldShowUp) {
        ...
        if (inputs) {
          inputs.each(function(item) {
            ...
            if (
              (!item.type || item.type != 'hidden') &&
              !($(item.id + '_inherit') && $(item.id + '_inherit').checked) && //eslint-disable-line
              !(
                currentConfig['can_edit_price'] != undefined &&
                !currentConfig['can_edit_price']
              ) //eslint-disable-line
            ) {
              item.disabled = false;
        ...
        if (inputs) {
          inputs.each(function(item) {
            // don't touch hidden inputs (and Use Default inputs too), bc they may have custom logic
            if (
              (!item.type || item.type != 'hidden') && //eslint-disable-line eqeqeq
