...
(function(root, factory) {
  ...
  $.widget('mage.translateInlineDialogVde', $.mage.translateInline, {
    ...
    openWithWidget: function(e, widget, callback) {
      if (widget && callback) {
        ...
        this.element.html(
          this._prepareContent($(widget.element).data('translate')),
        );
        this.triggerElement = widget.element;
        $(window).on(
          'resize.translateInlineDialogVde',
          $.proxy(this._positionNearTarget, this),
        );
    ...
    toggleStyle: function(mode) {
      ...
      this.options.imageTranslations.translateInlineImageVde(
        'toggleIcon',
        mode,
      );
      this.options.scriptTranslations.translateInlineScriptVde(
        'toggleIcon',
        mode,
      );
    },
    ...
    _checkTranslateEditing: function(event, data) {
      ...
      if (this.isBeingEdited) {
        ...
        parent
          .jQuery('[vde-translate]')
          .trigger('disableInlineTranslation', dataDisable);

        // Inline translation text is not being edited.  Continue on.
        parent
          .jQuery('[data-frame="editor"]')
          .trigger(data['next_action'], data);
      }
    },
    ...
    _formSubmitComplete: function() {
      ...
      this.element.find('[data-translate-input-index]').each(
        $.proxy(function(count, elem) {
    ...
    _toggleOutline: function(mode) {
      ...
      this.element
        .closest('[data-container="body"]')
        .addClass('trnslate-inline-' + mode + '-area');
      $.each(this.options.translateModes, function() {
        if (this != mode) {
          //eslint-disable-line eqeqeq
          that.element
            .closest('[data-container="body"]')
            .removeClass('trnslate-inline-' + this + '-area');
        }
      });
    },
  ...
  $.widget('mage.translateInlineVde', {
    ...
    _initIconTemplate: function() {
      this.iconTemplate = $(
        mageTemplate(this.options.iconTemplateSelector, {
          data: this.options,
        }),
      );

      this.iconTemplate
        .on('click', $.proxy(this._invokeAction, this))
