...
define([
    ...
    return function (config, element) {
        ...
        function resetVars($image) {
            ...
            allowAnimating = allowZoomIn = true;
            allowZoomOut = false;
            $image.css({
                top: 0,
                left: 0,
        ...
        function checkFullscreenImagePosition($image, dimentions, widthStep, heightStep) {
            ...
            if ($(gallerySelector).data('fotorama').fullScreen && allowAnimating) {
                ...
                if (dimentions.height >= $imageContainer.height()) {
                    ...
                    top = top > dimentions.height - $imageContainer.height() ? dimentions.height - $imageContainer.height() : top;
                } else {
                    top = 0;
                }
        ...
        function magnifierFullscreen() {
            ...
            function setImageFullSize (e) {
                var imgOriginalSize = getImageSize($image[0].src),
                    zoomWidthStepTemp = imgOriginalSize.rw - $image.width(),
                    zoomHeightStepTemp = imgOriginalSize.rh - $image.height();

                zoomWidthStep = Math.ceil((imgOriginalSize.rw - $image.width()) / parseFloat(config.magnifierOpts.fullscreenzoom));
                zoomHeightStep = Math.ceil((imgOriginalSize.rh - $image.height()) / parseFloat(config.magnifierOpts.fullscreenzoom));

                checkFullscreenImagePosition($image, getDimension($image), -zoomWidthStepTemp, -zoomHeightStepTemp);
            ...
            $image.off('touchstart mousedown pointerdown MSPointerDown');
            ...
            $image.off('touchmove mousemove pointermove MSPointerMove');
