...
(function(window, document, location, $, undefined) {
  ...
  function bindTransitionEnd($el) {
    ...
    addEvent(el, transitionEndEvent[Modernizr.prefixed('transition')], function(
      e,
    ) {
  ...
  function updateData(data, _dataFrame, i, fotorama) {
    for (var _i = 0, _l = data.length; _i < _l; _i++) {
      ...
      if (dataFrame.i === i && dataFrame.thumbsReady) {
        ...
        clear[STAGE_FRAME_KEY] = clear[NAV_THUMB_FRAME_KEY] = clear[
          NAV_DOT_FRAME_KEY
        ] = false;
    ...
    $el.children().each(function() {
      ...
      if ($this.is('a, img')) {
        ...
        getDimensions(
          $this,
          null,
          $.extend(dataFrame, { html: this, _html: $this.html() }),
        );
      } else return;
      data.push(dataFrame);
    });
  ...
  function validateSlidePos(
    ...
    if ((size + opt.margin) * (guessIndex + 1) >= wrapSize - offsetNav) {
      ...
      if ((size + opt.margin) * guessIndex <= Math.abs(offsetNav)) {
        if (dir === 'horizontal') {
          position =
            -$guessNavFrame.position().left + wrapSize - (size + opt.margin);
        } else {
          position =
            -$guessNavFrame.position().top + wrapSize - (size + opt.margin);
        }
      } else {
        position = offsetNav;
      }
  ...
  function moveOnTouch($el, options) {
    ...
    function startTracking(e, noStop) {
      ...
      startElPos = moveElPos =
        tail.noMove || noStop ? 0 : stop($el, (options.getPos || noop)());
      (options.onStart || noop).call(el, e);
    }
  ...
  jQuery.Fotorama = function($fotorama, opts) {
    ...
    function setMeasures(width, height, ratio, index) {
      if (
        ...
        that.resize(
          { width: width, ratio: opts.ratio || ratio || width / height },
          0,
          index !== startIndex && '*',
        );
      }
    ...
    function loadImg(indexes, type, specialMeasures, again) {
      eachIndex(indexes, type, function(
        i,
        index,
        dataFrame,
        $frame,
    ...
    function stageFramePosition(indexes) {
      eachIndex(indexes, 'stage', function(
        i,
        index,
        dataFrame,
        $frame,
            ...
            .each(function() {
              ...
              var $this = $(this),
                ...
                thumbwidth =
                  ...
                  o_thumbSide,
                thumbheight =
                  Math.round(o_thumbSide / frameData.data.thumbratio) ||
                  o_thumbSide2;
              frameData.t = top;
    ...
    that.showSlide = function(slideDir) {
      var currentPosition = readPosition($navShaft, opts.navdir),
        ...
        time = 500 * 1.1,
        size =
          opts.navdir === 'horizontal' ? opts.thumbwidth : opts.thumbheight,
        onEnd = function() {
          thumbArrUpdate();
        };
      if (slideDir === 'next') {
    ...
    that.showStage = function(silent, options, time) {
      ...
      if (!o_fade) {
        ...
        fade(
          ...
          $prevActiveFrame,
          $stageFrame,
          { time: time, method: opts.transition, onEnd: onEnd },
          fadeStack,
        );
      }
      arrsUpdate();
    ...
    that.requestFullScreen = function() {
      if (o_allowFullScreen && !that.fullScreen) {
        var isVideo = $((that.activeFrame || {}).$stageFrame || {}).hasClass(
            ...
            switch (opts.navdir) {
              case 'vertical':
                ...
                $nav
                  .stop()
                  .animate(
                    { height: measures.h, width: opts.thumbwidth },
                    time,
                  );
                break;
              case 'list':
        ...
        waitFor(
          ...
          function() {
            if (_activeIndex === activeIndex) {
              dataFrame.$video =
                dataFrame.$video ||
                $(div(videoClass)).append(createVideoFrame(video));
