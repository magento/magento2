...
abstract class AbstractDb extends \Magento\Framework\Data\Collection
{
     ...
     *
     * @return  $this
     * @throws \Magento\Framework\Exception\LocalizedException If SQL injection is detected
     */
        ...
        foreach ($this->_filters as $filter) {
            switch ($filter['type']) {
                case 'or':
                    ...
                    break;
                case 'string':
                    // SECURITY: Validate string filters to prevent SQL injection
                    // The 'string' type is meant for pre-escaped SQL fragments only
                    $this->validateSqlFilter($filter['value']);
    ...
    /**
     * Validate SQL filter for potential injection attacks
     *
     * This method provides defense-in-depth validation for the 'string' filter type,
     * which is designed to accept pre-escaped SQL fragments generated by database
     * adapter methods like quoteInto().
     *
     * @param string $sqlFragment The SQL fragment to validate
     * @return void
     * @throws \Magento\Framework\Exception\LocalizedException If dangerous SQL is detected
     * @since 2.4.8
     */
    protected function validateSqlFilter(string $sqlFragment)
    {
        // Use the SQL injection validator
        $validator = new SqlInjectionValidator();
        $validator->validate($sqlFragment, true);
    }
