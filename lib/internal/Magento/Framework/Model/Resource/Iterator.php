    ...
    protected function _getStatement($query, $conn = null)
    {
        ...
        if (is_string($query)) {
            if (!$conn instanceof \Zend_Db_Adapter_Abstract) {
                throw new \Magento\Framework\Exception\LocalizedException(__('Invalid connection'));
            }
            return $conn->query($query);
        }

        throw new \Magento\Framework\Exception\LocalizedException(__('Invalid query'));
    }
}
