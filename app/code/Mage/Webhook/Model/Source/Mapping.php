 ...
 */
class Mage_Webhook_Model_Source_Mapping
{
    /**
    ...
    public function toOptionArray()
    {
        ...
        $config = Mage::getConfig()->getNode(self::XML_PATH_MAPPINGS);
        if (!$config) {
            return $this->_options;
        }
    ...
    public function getMappingsForForm()
    {
        $elements = array();
        // process groups
        foreach ($this->toOptionArray() as $mappingName => $mapping) {
            if (!empty($mapping['label'])) {
                $elements[] = array(
                    'label' => Mage::helper('Mage_Webhook_Helper_Data')->__($mapping['label']),
                    'value' => $mappingName,
                );
            }
        }
