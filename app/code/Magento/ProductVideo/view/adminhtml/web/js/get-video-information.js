...
define([
  ...
  $.widget('mage.videoData', {
    ...
    _init: function() {
      this.element.on(
        this._START_UPDATE_INFORMATION_TRIGGER,
        $.proxy(this._onRequestHandler, this),
      );
      this.element.on(
        this._ERROR_UPDATE_INFORMATION_TRIGGER,
        $.proxy(this._onVideoInvalid, this),
      );
      this.element.on(
      ...
      this.element.on(
        this._VIDEO_URL_VALIDATE_TRIGGER,
        $.proxy(this._onUrlValidateHandler, this),
      );
    },
    ...
    _onRequestHandler: function() {
      ...
      function _onYouTubeLoaded(data) {
        var tmp, uploadedFormatted, respData, createErrorMessage;
        ...
        createErrorMessage = function() {
          ...
          return (
            $.mage.__('Video cant be shown due to the following reason: ') +
        ...
        tmp = data.items[0];
        uploadedFormatted = tmp.snippet.publishedAt
          .replace('T', ' ')
          .replace(/\..+/g, '');
        respData = {
      ...
      function _onVimeoLoaded(data) {
        ...
        respData = {
          ...
          title: tmp.title,
          description: tmp.description.replace(/(&nbsp;|<([^>]+)>)/gi, ''),
