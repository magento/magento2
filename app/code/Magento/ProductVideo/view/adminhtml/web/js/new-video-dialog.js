...
define([
  ...
  $.widget('mage.createVideoPlayer', {
     ...
     */
    _doUpdate: function() {
      var uploaderLinkUrl, uploaderLink;
      ...
      this.element
        .find(this.options.metaData.DOM.title)
        .text(this.options.metaData.data.title);
      this.element
        .find(this.options.metaData.DOM.uploaded)
        .text(this.options.metaData.data.uploaded);
      this.element
        .find(this.options.metaData.DOM.duration)
        .text(this.options.metaData.data.duration);
      ...
      this.element
        .find(this.options.metaData.DOM.uploader)[0]
        .appendChild(uploaderLink);
  ...
  $.widget('mage.updateInputFields', {
    options: {
      ...
      DOM: {
        ...
        thumbnailLocation:
          '.field-new_video_screenshot_preview .admin__field-control',
      },
      data: {
        url: '',
        title: '',
  ...
  $.widget('mage.newVideoDialog', {
    ...
    _bind: function() {
      ...
      this._videoUrlWidget = this.element
        .find(this._videoUrlSelector)
        .videoData({
          youtubeKey: this.options.youTubeApiKey,
          eventSource: 'focusout',
        });

      this._videoInformationGetBtn = this.element.find(
        this._videoInformationBtnSelector,
      );
      this._videoInformationGetUrlField = this.element.find(
        this._videoUrlSelector,
      );
      this._videoInformationGetEditBtn = this._gallery.find(
        this._editVideoBtnSelector,
      );

      this._videoInformationGetBtn.on(
        'click',
        $.proxy(this._onGetVideoInformationClick, this),
      );
      this._videoInformationGetUrlField.on(
        'focusout',
        $.proxy(this._onGetVideoInformationFocusOut, this),
      );

      this._videoUrlWidget.on(
        'updated_video_information',
        $.proxy(this._onGetVideoInformationSuccess, this),
      );
      this._videoUrlWidget.on(
        'error_updated_information',
        $.proxy(this._onGetVideoInformationError, this),
      );
      this._videoUrlWidget.on(
    ...
    _onGetVideoInformationSuccess: function(e, data) {
      ...
      self.element
        .on(
          ...
          $.proxy(function(element, playerData) {
            if (
              !self._onlyVideoPlayer ||
              (!self._isEditPage &&
                playerData.oldVideoId !== playerData.newVideoId) ||
              (playerData.oldVideoId &&
                playerData.oldVideoId !== playerData.newVideoId)
            ) {
    ...
    _onImageLoaded: function(result, file, oldFile, callback) {
      ...
      if (
        this.element
          .find('#video_url')
          .parent()
          .find('.image-upload-error').length > 0
      ) {
        ...
        this.element
          ...
          .append(
            '<div class="image-upload-error">' +
              '<div class="image-upload-error-cross"></div><span>' +
              data.error +
              '</span></div>',
          );
      ...
      data.disabled = this.element
        .find(this._videoDisableinputSelector)
        .attr('checked')
        ? 1
        : 0;
    ...
    _create: function() {
      var imgs =
          _.values(
            this.element.closest(this.options.videoSelector).data('images'),
          ) || [],
        widget,
        uploader,
        tmp,
      ...
      this.element.modal({
        ...
        opened: function() {
          ...
          if (!file) {
            ...
            roles.prop(
              'checked',
              widget._gallery.find('.image.item:not(.removed)').length < 1,
            );
          ...
          if (!imageData) {
            imageData = {
              url: _.find(widget._gallery.find('.product-image'), function(
                image,
              ) {
    ...
    _blockActionButtons: function(status) {
      this.element
        ...
        .find(
          '.page-actions-buttons button.video-create-button, .page-actions-buttons button.video-edit',
        )
    ...
    createVideoItemIcons: function() {
      var $imageWidget = this._gallery.find('.product-image.video-item'),
        $productGalleryWrapper = $(
          this._imageProductGalleryWrapperSelector,
        ).find('.product-image.video-item');
    ...
    _onCreate: function() {
      ...
      this.isValid(
        $.proxy(function(videoValidStatus) {
          if (!videoValidStatus) {
            return;
          }
        ...
        $.proxy(function(videoValidStatus) {
          ...
          $.each(
            ...
            function(i, el) {
              ...
              if (_field.length > 0) {
                _tmp =
                  _inputSelector.slice(0, _inputSelector.length - 2) +
                  '[' +
                  fieldName +
                  ']"]';
          ...
          flagChecked = this.element
            .find(this._videoDisableinputSelector)
            .attr('checked')
            ? 1
            : 0;
          this._gallery
            .find('input[name*="' + itemId + '][disabled]"]')
            .val(flagChecked);
          this._gallery
            .find(_inputSelector)
            .siblings('.image-fade')
            .css('visibility', flagChecked ? 'visible' : 'hidden');
    ...
    _getPreviewImage: function() {
      if (!this._previewImage) {
        ...
        $(this._previewImage).insertAfter(
          this.element.find(this._videoPreviewImagePointer),
        );
    ...
    _onClose: function() {
      ...
      $(newVideoForm)
        .find('input[type="hidden"][name!="form_key"]')
        .val('');
      this._gallery
        .find(
          'input[name*="' +
            this.element.find(this._itemIdSelector).val() +
            '"]',
        )
        .parent()
        .removeClass('active');
    ...
    findElementId: function(file) {
      var elem = this._gallery
        .find('.image.item')
        .find('input[value="' + file + '"]');
    ...
    _changeRole: function(imageType, isEnabled, imageData) {
      ...
      if (!isEnabled) {
        needCheked =
          this._gallery
            .find('input[name="product[' + imageType + ']"]')
            .val() === imageData.file;
      }
        ...
        $.each(
          ...
          function() {
            ...
            if ($(this).val() === file.val()) {
              ...
              modal
                .find('#new_video_form input[value="' + imageRole + '"]')
                .prop('checked', true);
            }
          },
        );
    ...
    toggleButtons: function() {
      ...
      modal
        .find('.video-placeholder, .add-video-button-container > button')
        .click(function() {
          modal.find('.video-create-button').show();
          modal.find('.video-delete-button').hide();
          modal.find('.video-edit').hide();
      ...
      this._gallery.on('click', '.item.video-item:not(.removed)', function() {
        ...
        $.each(formFields, function(i, field) {
          $(field).val(
            container.find('input[name*="' + field.name + '"]').val(),
          );
        });

        flagChecked = container.find('input[name*="disabled"]').val() > 0;
        self._gallery
          .find(self._videoDisableinputSelector)
          .attr('checked', flagChecked);

        file = self._gallery
          .find('#file_name')
          .val(container.find('input[name*="file"]').val());
  ...
  $('#group-fields-image-management > legend > span').text(
    $.mage.__('Images and Videos'),
  );
