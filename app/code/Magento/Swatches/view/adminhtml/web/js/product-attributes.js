  ...
  return function(optionConfig) {
    var activePanelClass = 'selected-type-options',
      swatchProductAttributes = {
         ...
         */
        get tabsFront() {
          return this.attrTabsFront.length
            ? this.attrTabsFront.closest('li')
        ...
        bindAttributeInputType: function() {
          ...
          if (
            this.frontendInput.val() === 'multiselect' ||
        ...
        switchDefaultValueField: function() {
          ...
          if (currentValue === 'media_image') {
            ...
            $.each(optionConfig.hiddenFields[currentValue], function(
              key,
              option,
            ) {
              switch (option) {
          ...
          /* swatch attributes */
          this.setRowVisibility(
            this.useProductImageForSwatch,
            useProductImageForSwatch,
          );
          this.setRowVisibility(
            this.updateProductPreviewImage,
            defaultValueUpdateImage,
          );
    ...
    $(function() {
      ...
      editForm.on('beforeSubmit', function() {
        ...
        activePanel = swatchTextPanel.hasClass(activePanelClass)
          ? swatchTextPanel
          : swatchVisualPanel;
        optionContainer = activePanel.find('table tbody');

        if (activePanel.hasClass(activePanelClass)) {
          optionsValues = $.map(optionContainer.find('tr'), function(row) {
            return $(row)
    ...
    window.saveAttributeInNewSet =
      swatchProductAttributes.saveAttributeInNewSet;
    window.toggleApplyVisibility =
      swatchProductAttributes.toggleApplyVisibility;
  };
});
