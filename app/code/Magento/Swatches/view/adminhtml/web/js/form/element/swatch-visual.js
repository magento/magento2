  ...
  function oldCode(value, container, uploadUrl, elementName) {
    var swatchVisualOption = {
      ...
      updateItemsCountField: function() {
        $('swatch-visual-option-count-check').value =
          this.totalItems > 0 ? '1' : '';
      },
    };
    ...
    jQuery(function($) {
      ...
      swatchComponents.inputFile.change(function() {
        var localContainer = $('.' + $(this).attr('data-called-by'))
            .parents()
            .eq(2)
            .children('.swatch_window'),
          /**
           * @this {iframe}
           */
          iframeHandler = function() {
            var imageParams = $.parseJSON(
                $(this)
                  .contents()
                  .find('body')
                  .html(),
              ...
              fullMediaUrl =
                imageParams['swatch_path'] + imageParams['file_path'];

            localContainer
              .prev('input')
              .val(imageParams['file_path'])
              .trigger('change');
            localContainer.css({
      ...
      $(container).on('click', '.btn_choose_file_upload', function() {
        swatchComponents.inputFile.attr(
          'data-called-by',
          $(this).data('class'),
        );
