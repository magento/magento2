        ...
        $.each(item.options, function() {
          if (this.products.length > 0) {
            $widget.optionsMap[item.id][this.id] = {
              price: parseInt(
                $widget.options.jsonConfig.optionPrices[this.products[0]]
                  .finalPrice.amount,
                10,
              ),
    ...
    _RenderSwatchOptions: function(config, controlId) {
      ...
      $.each(config.options, function() {
        ...
        if (type === 0) {
          ...
          // Default
          html +=
            '<div class="' + optionClass + '" ' + attr + '>' + label + '</div>';
        }
      });
    ...
    _Rebuild: function() {
      ...
      controls.each(function() {
        ...
        $this.find('[option-id]').each(function() {
          ...
          if (
            _.intersection(products, $widget.optionsMap[id][option].products)
              .length <= 0
          ) {
      ...
      _.each(
        ...
        function(attribute) {
          if (
            ...
            if (
              $(attribute).find('.' + this.options.classes.selectClass).length >
              0
            ) {
              _.each(
        ...
        selectedAttributes = _.invert(
          _.mapObject(
            ...
            function(attributeId) {
              var attribute = this.options.jsonConfig.mappedAttributes[
                attributeId
              ];
