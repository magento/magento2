...
define([
  ...
  window.Variables = {
    ...
    openDialogWindow: function(variablesContent, selectedElement) {
      ...
      if (typeof selectedElement !== 'undefined') {
        registry.get(
          ...
          function(radioSelect) {
            radioSelect.selectedVariableCode(
              MagentovariablePlugin.getElementVariablePath(selectedElement),
            );
          },
        );
      }
    ...
    addNotAvailableMessage: function(selectedElement) {
      ...
      if (
        ...
        variablePath = MagentovariablePlugin.getElementVariablePath(
          selectedElement,
        );
        name = variablePath.split(':');
        msg = $t(
          'The variable %1 is no longer available. Select a different variable.',
        ).replace('%1', name[1]);
    ...
    getVariableCode: function() {
      var code = registry
          .get('variables_modal.variables_modal.variables.variable_selector')
      ...
      if (typeof code !== 'undefined') {
        if (code.match('^default:')) {
          directive = configGenerator.processConfig(
            code.replace('default:', ''),
          );
        } else if (code.match('^custom:')) {
          directive = customGenerator.processConfig(
            code.replace('custom:', ''),
          );
        }
  ...
  window.MagentovariablePlugin = {
    ...
    loadChooser: function(url, textareaId, selectedElement) {
      ...
      new Ajax.Request(url, {
        ...
        onComplete: function(transport) {
          Variables.init(
            this.textareaId,
            'MagentovariablePlugin.insertVariable',
            this.editor,
          );
    ...
    getElementVariablePath: function(element) {
      ...
      type = jQuery(element).hasClass('magento-custom-var')
        ? 'custom'
        : 'default';
