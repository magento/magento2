...
class ConditionsTest extends TestCase
{
    ...
    public function testExecute()
    {
        ...
        $this->request->expects($this->at(0))
            ->method('getParam')
            ->with('id')->willReturn('1--1');
        $this->request->expects($this->at(1))
            ->method('getParam')
            ->with('type')->willReturn($type);
        ...
        $condition->expects($this->once())
            ->method('setRule')
            ->with($this->rule)->willReturnSelf();
        $condition->expects($this->once())
            ->method('setPrefix')
            ->with('conditions')->willReturnSelf();
        $condition->expects($this->once())
            ->method('setJsFormObject')
            ->with('request_form_param_value')->willReturnSelf();
        $condition->expects($this->once())
            ->method('setAttribute')
