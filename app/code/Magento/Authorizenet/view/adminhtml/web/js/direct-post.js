...
(function(factory) {
  if (typeof define === 'function' && define.amd) {
    define(['jquery', 'mage/backend/validation', 'prototype'], factory);
  } else {
    factory(jQuery);
  }
})(function(jQuery) {
  ...
  directPost.prototype = {
    initialize: function(
      ...
      var prepare = function(event, method) {
        if (method === 'authorizenet_directpost') {
          ...
          jQuery('#edit_form').off('submitOrder.authorizenet');
        }
      };
      ...
      jQuery('#edit_form').trigger('changePaymentMethod', [
        jQuery('#edit_form')
          .find(':radio[name="payment[method]"]:checked')
          .val(),
      ]);
    },
    ...
    validate: function() {
      ...
      this.inputs.each(function(elemIndex) {
        if ($(this.code + '_' + elemIndex)) {
          if (
            !jQuery.validator.validateElement($(this.code + '_' + elemIndex))
          ) {
            this.isValid = false;
          }
        }
    ...
    loadOrderIframe: function() {
      if (this.orderRequestSent) {
        ...
        var data = $(
          'order-' + this.iframeId,
        ).contentWindow.document.body.getElementsByTagName('pre')[0].innerHTML;
    ...
    submitAdminOrder: function() {
      ...
      if (editForm.valid()) {
        // Temporary solution will be removed after refactoring Authorize.Net (sales) functionality
        paymentMethodEl = editForm.find(
          ':radio[name="payment[method]"]:checked',
        );
        this.hasError = false;
        ...
        if (paymentMethodEl.val() == this.code) {
          ...
          editForm.append(
            this.createHiddenElement('controller', this.controller),
          );
    ...
    sendPaymentRequest: function(preparedData) {
      ...
      this.tmpForm.setAttribute(
        'target',
        $(this.iframeId).readAttribute('name'),
      );

      for (var param in preparedData) {
        this.tmpForm.appendChild(
          this.createHiddenElement(param, preparedData[param]),
        );
      }
    ...
    recreateIframe: function() {
      if ($(this.iframeId)) {
        ...
        var iframe =
          '<iframe id="' +
          this.iframeId +
          '" allowtransparency="true" frameborder="0"  name="' +
          name +
          '" style="display:none;width:100%;background-color:transparent" src="' +
          src +
          '" />';
