...
define([
  ...
  AdminOrder.prototype = {
    initialize: function(data) {
      ...
      jQuery.async(
        '#order-items',
        function() {
          this.dataArea = new OrderFormArea(
            'data',
            $(this.getAreaId('data')),
            this,
          );
          this.itemsArea = Object.extend(
            new OrderFormArea('items', $(this.getAreaId('items')), this),
            {
          ...
          if (jQuery('#' + this.getAreaId('items')).is(':visible')) {
            this.dataArea.onLoad = this.dataArea.onLoad.wrap(function(proceed) {
              proceed();
              this._parent.itemsArea.setNode(
                $(this._parent.getAreaId('items')),
              );
              this._parent.itemsArea.onLoad();
            });

            this.itemsArea.onLoad = this.itemsArea.onLoad.wrap(function(
              proceed,
            ) {
              proceed();
              if (
                $(searchAreaId) &&
                !$(searchAreaId).visible() &&
                !$(searchButtonId)
              ) {
    ...
    changeAddressField: function(event) {
      ...
      if (name === 'customer_address_id') {
        data['order[' + type + '_address][customer_address_id]'] = $(
          'order-' + type + '_address_customer_address_id',
        ).value;
      }
    ...
    fillAddressFields: function(container, data) {
      ...
      for (var i = 0; i < fields.length; i++) {
        ...
        if (
          fields[i].tagName.toLowerCase() == 'input' &&
          fields[i].type.toLowerCase() == 'file'
        ) {
          continue;
        }
    ...
    disableShippingAddress: function(flag) {
      ...
      if ($(this.shippingAddressContainer)) {
        var dataFields = $(this.shippingAddressContainer).select(
          'input',
          'select',
          'textarea',
        );
    ...
    setShippingMethod: function(method) {
      ...
      this.loadArea(
        ['shipping_method', 'totals', 'billing_method'],
        true,
        data,
      );
    },
    ...
    setPaymentMethod: function(method) {
      ...
      if ($('payment_form_' + method)) {
        jQuery('#' + this.getAreaId('billing_method')).trigger(
          'contentUpdated',
        );
    ...
    productGridRowInit: function(grid, row) {
      ...
      if (checkbox && inputs.length > 0) {
        ...
        for (var i = 0; i < inputs.length; i++) {
          ...
          input.disabled =
            !checkbox.checked || input.hasClassName('input-inactive');

          Event.observe(
            input,
            'keyup',
            this.productGridRowInputChange.bind(this),
          );
          Event.observe(
            input,
            'change',
    ...
    productGridRowInputChange: function(event) {
      ...
      if (
        ...
        element.checkboxElement.checked
      ) {
    ...
    productGridRowClick: function(grid, event) {
      ...
      var isInputCheckbox =
        eventElement.tagName == 'INPUT' && eventElement.type == 'checkbox';
      var isInputQty =
        eventElement.tagName == 'INPUT' && eventElement.name == 'qty';
      if (trElement && !isInputQty) {
        ...
        if (checkbox) {
          ...
          if (confLink.readAttribute('disabled')) {
            ...
            !isInputCheckbox ||
            (isInputCheckbox && checkbox.checked)
          ) {
            ...
            if (typeof this.productPriceBase[productId] == 'undefined') {
              ...
              if (!priceBase) {
                ...
                this.productPriceBase[productId] = parseFloat(
                  priceBase[1].replace(/,/g, ''),
                );
              }
            }
            productConfigure.setConfirmCallback(
              ...
              function() {
                ...
                productPrice = parseFloat(
                  Math.round(productPrice + 'e+2') + 'e-2',
                );
                priceColl.innerHTML =
                  this.currencySymbol + productPrice.toFixed(2);
    ...
    _calcProductPrice: function() {
      ...
      productPrice += getPriceFields(
        $(productConfigure.confirmedCurrentId).getElementsByTagName('input'),
      );
      productPrice += getPriceFields(
        $(productConfigure.confirmedCurrentId).getElementsByTagName('select'),
      );
      productPrice += getPriceFields(
        $(productConfigure.confirmedCurrentId).getElementsByTagName('textarea'),
      );
      return productPrice;
    },
    ...
    sidebarConfigureProduct: function(listType, productId, itemId) {
      ...
      productConfigure.setBeforeSubmitCallback(
        listType,
        function() {
     ...
     * @param itemsFilter
     */
    productConfigureSubmit: function(
      listType,
      area,
      fieldsPrepare,
      itemsFilter,
    ...
    showQuoteItemConfiguration: function(itemId) {
      ...
      var qtyElement = $('order-items_grid').select(
        'input[name="item[' + itemId + '][qty]"]',
      )[0];
      productConfigure.setConfirmCallback(
        listType,
        function() {
    ...
    accountGroupChange: function() {
      this.loadArea(
        ['data'],
        true,
        this.serializeData('order-form_account').toObject(),
      );
    },
    ...
    giftmessageOnItemChange: function(event) {
      var element = Event.element(event);
      if (
        element.name.indexOf('giftmessage') != -1 &&
    ...
    loadArea: function(area, indicator, params) {
      ...
      if (
        typeof productConfigure != 'undefined' &&
        area instanceof Array &&
        area.indexOf('items') != -1
      ) {
        productConfigure.clean('quote_items');
      }
    ...
    isPaymentValidationAvailable: function() {
      return (
        typeof this.paymentMethod == 'undefined' ||
          ...
          try {
            ...
            if (null === response.group) {
              if (true === response.valid) {
                ...
                message = parameters.vatInvalidMessage.replace(
                  /%s/,
                  params.vat,
                );
              } else {
    ...
    processCustomerGroupChange: function(
      ...
      try {
        ...
        var customerGroupOption = $$(
          '#' + groupIdHtmlId + ' > option[value=' + groupId + ']',
        )[0];
        groupMessage = customerGroupMessage.replace(
          /%s/,
  ...
  OrderFormArea.prototype = {
    ...
    initialize: function(name, node, parent) {
      ...
      parent[this._callbackName] = parent[this._callbackName].wrap(
        function(proceed) {
          proceed();
          this.onLoad();
        }.bind(this),
