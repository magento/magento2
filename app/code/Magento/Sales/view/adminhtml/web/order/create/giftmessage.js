...
define([
  ...
  window.giftMessagesController = {
    ...
    toogleGiftMessage: function(container) {
      if (!$(container).toogleGiftMessage) {
        ...
        $(this.getFieldId(container, 'message')).formObj = $(
          this.getFieldId(container, 'form'),
        );
  ...
  GiftOptionsPopup.prototype = {
    ...
    showItemGiftOptions: function(event) {
      ...
      Event.observe(
        $('gift_options_cancel_button'),
        'click',
        this.onCloseButton.bind(this),
      );
      Event.observe(
        $('gift_options_ok_button'),
        'click',
    ...
    setData: function(event) {
      ...
      if ($('gift-message-form-data-' + this.id)) {
        this.fields.each(function(el) {
          if (
            $(this.sourcePrefix + this.id + '_' + el) &&
            $(this.destPrefix + el)
          ) {
            $(this.destPrefix + el).value = $(
              this.sourcePrefix + this.id + '_' + el,
            ).value;
          }
        }, this);
    ...
    saveData: function(event) {
      this.fields.each(function(el) {
        if (
          ...
          $(this.sourcePrefix + this.id + '_' + el).value = $(
            this.destPrefix + el,
          ).value;
        }
      }, this);
      ...
      if ($(this.sourcePrefix + this.id + '_form')) {
        ...
        jQuery
          .when(order.loadArea(['items'], true, data.toObject()))
          .done(function() {
