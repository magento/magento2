...
class DbStorage extends AbstractStorage
{
    ...
    protected function prepareSelect(array $data)
    {
        ...
        foreach ($data as $column => $value) {
            $select->where(
                $this->connection->quoteIdentifier($column) . ' IN (?)', $value
            );
        }
        ...
        if (array_key_exists(UrlRewrite::REQUEST_PATH, $data)
            ...
            if (count($resultsFromDb) === 1) {
                ...
                foreach ($resultsFromDb as $resultFromDb) {
                    if ($resultFromDb[UrlRewrite::REQUEST_PATH]
                        === $requestPath
                    ) {
                        $result = $resultFromDb;
                        break;
    ...
    private function deleteOldUrls(array $urls): void
    {
        ...
        /** @var UrlRewrite $url */
        foreach ($urls as $url) {
            $result[$url->getEntityType()][$url->getStoreId()][]
                = $url->getEntityId();
        }
        ...
        try {
            ...
            foreach ($urls as $url) {
                ...
                if (isset($urlFound[UrlRewrite::URL_REWRITE_ID])) {
                    $urlConflicted[$urlFound[UrlRewrite::URL_REWRITE_ID]]
                        = $url->toArray();
                }
            }
            if ($urlConflicted) {
    ...
    protected function insertMultiple($data)
    {
        try {
            $this->connection->insertMultiple(
                $this->resource->getTableName(self::TABLE_NAME), $data
            );
        } catch (\Exception $e) {
    ...
    protected function createFilterDataBasedOnUrls($urls)
    {
        ...
        foreach ($urls as $url) {
            ...
            foreach ([UrlRewrite::ENTITY_ID, UrlRewrite::STORE_ID] as $key) {
                ...
                if (!isset($data[$entityType][$key])
                    || !in_array(
                        $fieldValue, $data[$entityType][$key]
                    )
                ) {
    ...
    {
        $this->connection->query(
            $this->prepareSelect($data)->deleteFromSelect(
                $this->resource->getTableName(self::TABLE_NAME)
            )
        );
    }
