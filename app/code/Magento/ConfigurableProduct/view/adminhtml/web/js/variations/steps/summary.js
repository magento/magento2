...
define([
  ...
  return Component.extend({
    ...
    initObservable: function() {
      ...
      this._super().observe(
        'gridExisting gridNew gridDeleted attributes sections',
      );
    ...
    calculate: function(variations, getSectionValue) {
      ...
      _.each(
        ...
        function(options) {
          var product,
            ...
            price,
            variation,
            productId = this.variationsComponent().getProductIdByOptions(
              options,
            );
          ...
          variationsKeys.push(
            this.variationsComponent().getVariationKey(options),
          );
        },
        this,
      );
      ...
      _.each(
        _.omit(this.variationsComponent().productAttributesMap, variationsKeys),
        function(productId) {
          gridDeleted.push(
            this.prepareRowForGrid(
    ...
    generateGrid: function() {
      var pageExisting =
          this.pagingExisting.pageSize * this.pagingExisting.current,
      ...
      this.gridExisting(
        this.variationsExisting.slice(
          pageExisting - this.pagingExisting.pageSize,
          pageExisting,
        ),
      );
      ...
      this.gridNew(
        this.variationsNew.slice(pageNew - this.pagingNew.pageSize, pageNew),
      );

      this.pagingDeleted.totalRecords = this.variationsDeleted.length;
      this.gridDeleted(
        this.variationsDeleted.slice(
          pageDeleted - this.pagingDeleted.pageSize,
          pageDeleted,
        ),
      );
    },
    ...
    prepareRowForGrid: function(variation) {
      ...
      row.push(
        this.variationsComponent().getCurrencySymbol() + ' ' + variation.price,
      );
    ...
    render: function(wizard) {
      ...
      this.gridExisting([]);
      this.gridDeleted([]);
      this.gridExisting.columns(
        this.getColumnsName(this.wizard.data.attributes),
      );
      this.gridNew.columns(this.getColumnsName(this.wizard.data.attributes));
      this.gridDeleted.columns(
        this.getColumnsName(this.variationsComponent().productAttributes),
      );
