...
define(['jquery', 'mage/template', 'mage/translate', 'prototype'], function(
  jQuery,
  mageTemplate,
) {
  'use strict';
  ...
  Product.Config.prototype = {
    ...
    initialize: function(config) {
      ...
      if (config.containerId) {
        this.settings = $$(
          '#' + config.containerId + ' ' + '.super-attribute-select',
        );
      } else {
      ...
      if (this.values) {
        this.settings.each(
          function(element) {
            ...
            element.value =
              typeof this.values[attributeId] === 'undefined'
                ? ''
                : this.values[attributeId];
    ...
    reloadOptionLabels: function(element) {
      ...
      if (
        ...
        selectedPrice = parseFloat(
          this.config.optionPrices[option.allowedProducts[0]].finalPrice.amount,
        );
      }
