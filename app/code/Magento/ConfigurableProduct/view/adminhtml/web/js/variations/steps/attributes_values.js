...
define([
      ...
      $(element).sortable({
        ...
        update: function(event, ui) {
          var item = ko.contextFor(ui.item[0]).$data,
            position = ko.utils.arrayIndexOf(
  ...
  return Collapsible.extend({
    ...
    initialize: function() {
      ...
      this.createAttribute = _.memoize(
        this.createAttribute.bind(this),
        _.property('id'),
      );
    },
    ...
    createAttribute: function(attribute, index) {
      ...
      attribute.options = ko.observableArray(
        _.map(attribute.options, function(option) {
    ...
    saveAttribute: function() {
      ...
      this.attributes.each(function(attribute) {
        ...
        attribute.chosenOptions.each(function(id) {
          attribute.chosen.push(
            attribute.options.findWhere({
              id: id,
            }),
          );
        });
    ...
    doInitSavedOptions: function(attribute) {
      var selectedOptions,
        selectedOptionsIds,
        selectedAttribute = _.findWhere(this.initData.attributes, {
          id: attribute.id,
        });

      if (selectedAttribute) {
        selectedOptions = _.pluck(selectedAttribute.chosen, 'value');
        selectedOptionsIds = _.pluck(
          _.filter(attribute.options(), function(option) {
            return _.contains(selectedOptions, option.value);
          }),
          'id',
        );
        attribute.chosenOptions(selectedOptionsIds);
        this.initData.attributes = _.without(
