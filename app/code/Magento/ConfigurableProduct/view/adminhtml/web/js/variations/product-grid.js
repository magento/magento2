...
define([
  ...
  return Component.extend({
    defaults: {
      ...
      listens: {
        '${ $.productsProvider }:data':
          '_showMessageAssociatedGrid _handleManualGridOpening',
        '${ $.productsMassAction }:selected': '_handleManualGridSelect',
        '${ $.configurableVariations }:productMatrix':
          '_showButtonAddManual _switchProductType',
      },
    },
    ...
    initialize: function(options) {
      ...
      this._switchProductType = _.wrap(
        this._switchProductType.bind(this),
    ...
    open: function(filterData, callbackName, showMassActionColumn) {
      ...
      this.productsMassAction(
        function(massActionComponent) {
    ...
    close: function(rowIndex) {
      try {
        if (this.productsMassAction().selected.getLength()) {
          this.variationsComponent()[this.callbackName](
            this.productsMassAction().selected.map(
          ...
          this.variationsComponent()[this.callbackName]([
            this.getProductByIndex(rowIndex),
          ]);
        }
    ...
    showManuallyGrid: function() {
      var filterModifier = _.mapObject(
          _.object(this._getAttributesCodes(), []),
          function() {
            return {
    ...
    _disableRows: function(items, selectedVariationKeys, selected) {
      selectedVariationKeys =
        selectedVariationKeys === undefined ? [] : selectedVariationKeys;
      ...
      this.productsMassAction(
        function(massaction) {
          var configurableVariationKeys = _.union(
              selectedVariationKeys,
              _.pluck(
                this.variationsComponent().productMatrix(),
                'variationKey',
              ),
            ),
            variationKeyMap = this._getVariationKeyMap(items),
            rowsForDisable = _.keys(
              _.pick(variationKeyMap, function(variationKey) {
    ...
    _handleManualGridSelect: function(selected) {
      ...
      if (this.callbackName == 'appendProducts') {
        //eslint-disable-line eqeqeq
        selectedRows = _.filter(this.productsProvider().data.items, function(
          row,
        ) {
          return selected.indexOf(row['entity_id']) !== -1;
        });
        selectedVariationKeys = _.values(
          this._getVariationKeyMap(selectedRows),
        );
        this._disableRows(
          this.productsProvider().data.items,
          selectedVariationKeys,
          selected,
        );
      }
