  ...
  return Component.extend({
    defaults: {
      ...
      imports: {
        ...
        attributeSetSelection:
          '${ $.provider }:configurableAffectedAttributeSet',
      ...
      links: {
        ...
        skeletonAttributeSet:
          '${ $.provider }:data.new-variations-attribute-set-id',
      },
    },
    ...
    _makeProduct: function(product) {
      ...
      return {
        ...
        options: options,
        price: parseFloat(
          Math.round(product.price.replace(/[^\d.]+/g, '') + 'e+4') + 'e-4',
        ).toFixed(4),
        productId: productId,
    ...
    getProductValue: function(name) {
      ...
      return $(
        '[name="product[' + name + ']"]:enabled:not(.ignore-validate)',
        this.productForm,
      ).val();
    },
        ...
        function(variation) {
          ...
          tmpArray.push(
            _.extend(variation, types, {
              ...
              status:
                variation.status === undefined
                  ? 1
                  : parseInt(variation.status, 10),
    ...
    initProductAttributesMap: function() {
      if (this.productAttributesMap === null) {
        ...
        _.each(
          this.variations,
          function(product) {
            this.productAttributesMap[this.getVariationKey(product.options)] =
              product.productId;
          }.bind(this),
        );
      }
    ...
    disableConfigurableAttributes: function(attributes) {
      ...
      _.each(this.disabledAttributes, function(attribute) {
        registry
          .get('inputName = ' + 'product[' + attribute + ']')
          .disabled(false);
      });
      ...
      _.each(
        ...
        function(attribute) {
          element = registry.get(
            'inputName = ' + 'product[' + attribute.code + ']',
          );
    ...
    serializeData: function() {
      if (this.source.data['configurable-matrix']) {
        this.source.data['configurable-matrix-serialized'] = JSON.stringify(
      ...
      if (this.source.data['associated_product_ids']) {
        this.source.data['associated_product_ids_serialized'] = JSON.stringify(
    ...
    unserializeData: function() {
      if (this.source.data['configurable-matrix-serialized']) {
        this.source.data['configurable-matrix'] = JSON.parse(
      ...
      if (this.source.data['associated_product_ids_serialized']) {
        this.source.data['associated_product_ids'] = JSON.parse(
    ...
    checkForNewAttributes: function() {
      ...
      _.each(
        this.source.get('data.attribute_codes'),
        function(attribute) {
    ...
    addNewAttributeSetHandler: function() {
      ...
      if (this.formElement().source.get('params.invalid') === false) {
        ...
        if (chosenAttributeSetOption === 'existing') {
          this.set('skeletonAttributeSet', this.attributeSetId);
        }
    ...
    createNewAttributeSet: function() {
      ...
      $.ajax({
        ...
        .done(function(data) {
          if (!data.error) {
            this.set('skeletonAttributeSet', data.id);
