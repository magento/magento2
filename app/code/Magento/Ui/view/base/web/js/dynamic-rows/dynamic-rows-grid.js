 ...
 * @api
 */
define(['underscore', './dynamic-rows'], function(_, dynamicRows) {
  'use strict';
  ...
  return dynamicRows.extend({
    ...
    initObservable: function() {
      this._super().observe(['insertData']);
    ...
    setToInsertData: function() {
      ...
      if (this.recordData().length && !this.update) {
        _.each(
          ...
          function(recordData) {
            obj = {};
            obj[this.map[this.identificationProperty]] =
              recordData[this.identificationProperty];
            insertData.push(obj);
          },
          this,
        );
    ...
    initChildren: function() {
      this.getChildItems().forEach(function(data, index) {
        this.processingAddChild(
          ...
          data[this.identificationDRProperty],
        );
      }, this);
      ...
      if (newData.length) {
        if (this.insertData().length) {
          this.processingAddChild(
            newData[0],
            data.length - 1,
            newData[0][this.identificationProperty],
          );
        }
      }
    ...
    updateInsertData: function(recordId) {
      ...
      this.insertData(
        _.reject(
          this.source.get(this.dataProvider),
          function(recordData) {
    ...
    getElementData: function(array, index, property) {
      ...
      property
        ? (obj[property] = index)
        : (obj[this.map[this.identificationDRProperty]] = index);
    ...
    getNewData: function(data) {
      ...
      if (data.length !== this.relatedData.length) {
        _.each(
          data,
          function(obj) {
            tmpObj[this.identificationDRProperty] =
              obj[this.identificationDRProperty];
    ...
    processingInsertData: function(data) {
      ...
      if (changes.length) {
        obj[this.identificationDRProperty] =
          changes[0][this.map[this.identificationProperty]];
    ...
    mappingValue: function(data) {
      ...
      this.source.set(
        this.dataScope + '.' + this.index + '.' + this.recordData().length,
        obj,
      );
    },
    ...
    _checkGridData: function(data) {
      ...
      max.forEach(function(record, index) {
        obj[this.map[this.identificationDRProperty]] =
          record[this.map[this.identificationDRProperty]];
