  ...
  return uiCollection.extend({
    defaults: {
      ...
      listens: {
        ...
        recordData:
          'setDifferedFromDefault parsePagesData setRecordDataToCache',
    ...
    deleteHandler: function(index, id) {
      ...
      this.pagesChanged[this.currentPage()] = !compareArrays(
        defaultState,
        this.arrayFilter(this.getChildItems()),
      );
    ...
    setInitialProperty: function() {
      if (_.isArray(this.recordData())) {
        this.recordData.each(function(data, index) {
          this.source.set(
            this.dataScope + '.' + this.index + '.' + index + '.initialize',
            true,
          );
        }, this);
      }
    ...
    onChildrenUpdate: function(state) {
      var changed, dataScope, changedElemDataScope;
      ...
      if (state && !this.hasInitialPagesState[this.currentPage()]) {
        ...
        changed.forEach(function(elem) {
          ...
          changedElemDataScope[0] = (
            parseInt(changedElemDataScope[0], 10) -
    ...
    setDefaultState: function(data) {
      ...
      if (!this.hasInitialPagesState[this.currentPage()]) {
        ...
        this.defaultPagesState[this.currentPage()] = data
          ? data
          : this.arrayFilter(componentData);
      }
    },
    ...
    checkDefaultState: function() {
      var isRecordDataArray = _.isArray(this.recordData()),
        initialize,
        hasNotDefaultRecords = isRecordDataArray
      ...
      if (
        !this.hasInitialPagesState[this.currentPage()] &&
        isRecordDataArray &&
        hasNotDefaultRecords
      ) {
        this.hasInitialPagesState[this.currentPage()] = true;
        this.defaultPagesState[this.currentPage()] = utils.copy(
          this.getChildItems().filter(function(data) {
    ...
    createHeaderTemplate: function(prop) {
      var visible = prop.visible !== false,
        disabled = _.isUndefined(prop.disabled)
          ? this.disabled()
          : prop.disabled;
    ...
    checkSpinner: function(elems) {
      this.showSpinner(
        !(
          !this.recordData().length ||
          (elems && elems.length === this.getChildItems().length)
        ),
      );
    },
    ...
    processingAddChild: function(ctx, index, prop) {
      ...
      if (
        this.relatedData.length &&
        this.relatedData.length % this.pageSize === 0
      ) {
    ...
    updatePosition: function(collection, position, elemName) {
      ...
      if (result[1]) {
        ...
        result[0].name === elemName
          ? (result[1].position = curPos)
          : (result[0].position = curPos);
    ...
    deleteRecord: function(index, recordId) {
      var recordInstance, lastRecord, recordsData, lastRecordIndex;
      ...
      if (this.deleteProperty) {
        ...
        recordsData[recordInstance.index][
          this.deleteProperty
        ] = this.deleteValue;
    ...
    _updateData: function(data) {
      ...
      dataArr = this.recordData.splice(
        this.startIndex,
        this.recordData().length - this.startIndex,
      );
      ...
      data.concat(dataArr).forEach(function(rec, idx) {
        ...
        path =
          this.dataScope + '.' + this.index + '.' + (this.startIndex + idx);
