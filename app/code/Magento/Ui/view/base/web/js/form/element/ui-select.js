...
define([
  ...
  function parseOptions(nodes) {
    var caption, value, cacheNodes, copyNodes;
  ...
  return Abstract.extend({
    defaults: {
      ...
      visibleOptionSelector:
        'li.admin__action-multiselect-menu-inner-item:visible',
    ...
    initialize: function() {
      ...
      $.async(this.rootListSelector, this, this.onRootListRender.bind(this));
    ...
    initConfig: function(config) {
      var result = parseOptions(config.options),
        ...
        multiple = _.isBoolean(config.multiple)
          ? config.multiple
          : defaults.multiple,
      ...
      config.showOpenLevelsActionIcon =
        showOpenLevelsActionIcon && openLevelsAction;
      _.extend(
        config,
        result,
        defaults.presets[multiple],
        defaults.presets[type],
      );
      this._super();
    ...
    addLastElement: function(data) {
      if (!data.hasOwnProperty(this.separator)) {
        ...
        if (
          !_.findWhere(this.cacheOptions.lastOptions, {
            value: data.value,
          })
        ) {
    ...
    showLevels: function(data) {
      ...
      if (data.visible) {
        ...
        isVisible =
          (!!data.hasOwnProperty(this.separator) &&
            _.isBoolean(this.levelsVisibility) &&
            this.levelsVisibility) ||
          (data.hasOwnProperty(this.separator) &&
            parseInt(this.levelsVisibility, 10) >= curLevel);
      ...
      _.each(
        ...
        function(opt) {
          if (opt.value == option.parent) {
            ...
            opt[this.separator]
              ? opt[this.separator].push(option)
              : (opt[this.separator] = [option]);
            copyOptionsTree = JSON.parse(
              JSON.stringify(this.cacheOptions.tree),
            );
            this.cacheOptions.plain = flattenCollection(
              copyOptionsTree,
              this.separator,
            );
    ...
    filterOptionsList: function() {
      var value = this.filterInputValue()
          .trim()
          .toLowerCase(),
        array = [];
    ...
    openChildLevel: function(data) {
      ...
      if (
        ...
        (this.openLevelsAction &&
          data.hasOwnProperty(this.separator) &&
          parseInt(this.levelsVisibility, 10) <= data.level)
      ) {
    ...
    onDelegatedMouseMouve: function(event) {
      var target = $(event.currentTarget).closest(
        this.visibleOptionSelector,
      )[0];

      if (
        this.isCursorPositionChange(event) ||
    ...
    _scrollTo: function(element) {
      ...
      if (curElPos.start < wrapperPos.start) {
        wrapper.scrollTop(
          wrapper.scrollTop() - (wrapperPos.start - curElPos.start),
        );
    ...
    setCaption: function() {
      ...
      if (
        this.isDisplayMissingValuePlaceholder &&
        length &&
      ...
      if (length > 1) {
        this.placeholder(
          length + ' ' + this.selectedPlaceholders.lotPlaceholders,
        );
    ...
    _getLastIn: function(scope) {
      return $(scope)
        .find(this.visibleOptionSelector)
        .last()[0];
    },
    ...
    loadOptions: function(searchKey) {
      var currentPage =
          searchKey === this.lastSearchKey ? this.lastSearchPage + 1 : 1,
        cachedSearchResult;
      ...
      if (this.isSearchKeyCached(searchKey)) {
        ...
        this.afterLoadOptions(
          searchKey,
          cachedSearchResult.lastPage,
          cachedSearchResult.total,
        );
    ...
    onScrollDown: function(data, event) {
      ...
      if (
        clientHight > scrollHeight - this.deviation &&
        !this.isSearchKeyCached(data.filterInputValue())
      ) {
    ...
    isSearchKeyCached: function(searchKey) {
      ...
      return (
        totalCached > 0 &&
        totalCached >= this.cachedSearchResults[searchKey].total
      );
    },
