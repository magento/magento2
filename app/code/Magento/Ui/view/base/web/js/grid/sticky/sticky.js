...
define([
  ...
  return Component.extend({
    defaults: {
      listingSelector:
        '${ $.listingProvider }::not([data-role = "sticky-el-root"])',
      toolbarSelector:
        '${ $.toolbarProvider }::not([data-role = "sticky-el-root"])',
    ...
    initialize: function() {
      ...
      $.async(this.listingSelector, this.initListingNode);
      $.async(this.toolbarSelector, this.initToolbarNode);

      $.async(this.stickyContainerSelector, this, this.initContainerNode);
    ...
    initContainerNode: function(node) {
      ...
      $.async(this.leftDataGridCapSelector, node, this.initLeftDataGridCap);
      $.async(this.rightDataGridCapSelector, node, this.initRightDataGridCap);
    ...
    adjustStickyElems: function() {
      if (this.flags.resized || this.flags.scrolled) {
        this.checkPos();
      }
    ...
    onWindowScroll: function() {
      ...
      if (horizontal) {
        this.adjustOffset().adjustDataGridCapPositions();
    ...
    onResize: function() {
      this.checkPos();
      this.adjustContainerElemsWidth().adjustDataGridCapPositions();
    },
    ...
    getColsChecksum: function() {
      return _.reduce(this.columns, function(pv, cv) {
    ...
    getBulkRowHeight: function() {
      return this.listingNode
        .find(this.bulkRowSelector)
        .filter(':visible')
        .height();
    },
    ...
    getListingTopYCoord: function() {
      ...
      return (
        this.listingNode.find('tbody').offset().top -
    ...
    getMustBeSticky: function() {
      var stickyTopCondition =
          this.getListingTopYCoord() - this.getOtherStickyElementsSize(),
        stickyBottomCondition =
          this.listingNode.offset().top +
    ...
    resizeContainer: function() {
      ...
      this.stickyListingNode
        .find(this.tableSelector)
        .innerWidth(this.getTableWidth());
    ...
    resetToTop: function() {
      var posOfTopEl =
        this.getTopElement().offset().top - this.getOtherStickyElementsSize() ||
        0;
    ...
    adjustDataGridCapPositions: function() {
      this.adjustLeftDataGridCapPos().adjustRightDataGridCapPos();
    ...
    collapseOriginalElements: function() {
      ...
      $(this.listingNode.find(this.bulkRowSelector)[0]).css(
        'visibility',
        'hidden',
      );
    },
