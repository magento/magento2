...
define([
  ...
  return Element.extend({
    ...
    initObservable: function() {
      this._super().observe(['recordsCache']);
    ...
    mousedownHandler: function(data, elem, event) {
      ...
      drEl.eventMousedownY = isTouchDevice
        ? event.originalEvent.touches[0].pageY
        : event.pageY;
      drEl.minYpos =
        $table.offset().top -
        originRecord.offset().top +
        $table.children('thead').outerHeight();
      drEl.maxYpos =
        drEl.minYpos +
        $table.children('tbody').outerHeight() -
        originRecord.outerHeight();
    ...
    mousemoveHandler: function(event) {
      ...
      if (positionY > depEl.minYpos && positionY < depEl.maxYpos) {
        $(depEl.instance)[0].style[transformProp] =
    ...
    mouseupHandler: function(event) {
      ...
      drEl.depElement = this.getDepElement(
        drEl.instance,
        positionY,
        this.draggableElement.originRow,
      );
    ...
    getDepElement: function(curInstance, position, row) {
      var tableSelector = this.tableClass + ' tr',
        ...
        curInstancePositionBottom =
          curInstancePositionTop + $curInstance.height();

      if (position < 0) {
        return this._getDepElement(
          recordsCollection,
          'before',
          curInstancePositionTop,
        );
      } else if (position > 0) {
        return this._getDepElement(
          recordsCollection,
      ...
      for (i; i < length; i++) {
        ...
        if (dragPosition > rangeStart && dragPosition < rangeEnd) {
          result = {
            elem: rec,
            insert:
              rec[0] === this.draggableElement.originRow[0] ? 'none' : position,
            className: className,
          };
        }
      }
