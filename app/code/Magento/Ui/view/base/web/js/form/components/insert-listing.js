 ...
 * @api
 */
define(['jquery', './insert', 'mageUtils', 'underscore'], function(
  $,
  Insert,
  utils,
  _,
  ...
  return Insert.extend({
    defaults: {
      ...
      exports: {
        externalFiltersModifier:
          '${ $.externalProvider }:params.filters_modifier',
      },
      listens: {
    ...
    initConfig: function(config) {
      ...
      if (config.behaviourType === 'edit') {
        ...
        _.map(
          defaults.settings.edit.imports,
          function(value, key) {
      ...
      if (config.externalFilterMode === true) {
        _.map(
          defaults.settings.filter.exports,
          function(value, key) {
    ...
    initObservable: function() {
      return this._super().observe(['externalValue']);
    },
    ...
    updateExternalValueByEditableData: function() {
      ...
      if (
        !this.behaviourType === 'edit' ||
        _.isEmpty(this.editableData) ||
        _.isEmpty(this.externalValue())
      ) {
        return;
      }
    ...
    updateExternalValue: function() {
      ...
      itemsType =
        selections && selections.excludeMode ? 'excluded' : 'selected';
      rows = provider && provider.rows();

      if (
        this.canUpdateFromClientData(totalSelected, selections.selected, rows)
      ) {
        this.updateFromClientData(selections.selected, rows);
        this.updateExternalValueByEditableData();
        result.resolve();
      } else {
        this.updateFromServerData(selections, itemsType).done(
          function() {
            this.updateExternalValueByEditableData();
            result.resolve();
          }.bind(this),
    ...
    canUpdateFromClientData: function(totalSelected, selected, rows) {
      var alreadySavedSelectionsIds = _.pluck(
          this.externalValue(),
          this.indexField,
        ),
      ...
      return (
        ...
        _.intersection(
          _.union(alreadySavedSelectionsIds, rowsOnCurrentPageIds),
          selected,
    ...
    updateSelections: function(items) {
      ...
      if (_.isString(items)) {
        ...
        ids = _.pluck(items || [], this.indexField).map(function(item) {
