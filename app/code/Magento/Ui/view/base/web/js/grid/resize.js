  ...
  return Element.extend({
    ...
    checkSumColumnsWidth: function() {
      var table = $(this.table),
        ...
        elemsWidthMin = table.find(
          'th[style*="width: ' + (this.minColumnWidth - 1) + 'px"]:visible',
        ),
      ...
      _.each(
        ...
        function(elem) {
          ...
          model.width && model.width !== 'auto'
            ? (this.sumColumnsWidth += model.width)
            : false;
        },
        this,
      );
      ...
      if (
        this.sumColumnsWidth +
          elemsWidthAuto.length * this.minColumnWidth +
    ...
    setWidthToColumnsWidthAuto: function() {
      var elemsWidthAuto = $(this.table).find(
        'th[style*="width: auto"]:visible',
      );
    ...
    initColumn: function(column) {
      ...
      if (!this.hasColumn(model, ctxIndex, false)) {
        this.columnsElements[model.index] =
          this.columnsElements[model.index] || {};
    ...
    initResizableElement: function(column) {
      var model = ko.dataFor(column),
        templateDragElement =
          '<div class="' + this.resizableElementClass + '"></div>';
    ...
    _canResize: function(column) {
      if (
        ...
        !$(this.resizeConfig.depResizeElem.elems[0]).find(
    ...
    mousedownHandler: function(event) {
      ...
      cfg.curResizeElem.elems = this.hasColumn(
        cfg.curResizeElem.model,
        false,
        true,
      );
      cfg.curResizeElem.position = event.pageX;
      cfg.depResizeElem.elems = this.getNextElements(
        cfg.curResizeElem.elems[0],
      );
    ...
    mousemoveHandler: function(event) {
      ...
      if (
        ...
        width <= -(cfg.curResizeElem.model.width - this.minColumnWidth)
      ) {
        cfg.curResizeElem.elems.forEach(function(el) {
    ...
    mouseupHandler: function(event) {
      ...
      this.storageColumnsData[cfg.curResizeElem.model.index] =
        cfg.curResizeElem.model.width;
      this.storageColumnsData[cfg.depResizeElem.model.index] =
        cfg.depResizeElem.model.width;
    ...
    hasColumn: function(model, ctxIndex, returned) {
      ...
      if (colElem && (getFromAllCtx || colElem.hasOwnProperty(ctxIndex))) {
        if (returned) {
          return getFromAllCtx ? _.values(colElem) : colElem[ctxIndex];
        }
    ...
    getCtxIndex: function(ctx) {
      return ctx
        ? ctx.$parents.reduce(function(pv, cv) {
