...
class UpdateCustomerAccount
{
    ...
    public function execute(CustomerInterface $customer, array $data, StoreInterface $store): void
    {
        ...
        if (isset($email) && $customer->getEmail() !== $email) {
            if (empty($password)) {
                throw new GraphQlInputException(__('Provide the current "password" to change "email".'));
            }
            $this->checkCustomerPassword->execute($password, $customerId);
        ...
        if (isset($isSubscribed)) {
            $this->changeSubscriptionStatus->execute((int)$customer->getId(), (bool)$isSubscribed);
        }
    }
}
