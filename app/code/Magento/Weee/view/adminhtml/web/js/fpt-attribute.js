...
define([
  ...
  $.widget('mage.fptAttribute', {
    ...
    _create: function() {
      ...
      this.rowTmpl = mageTemplate(
        this.element.find('[data-role="row-template"]').html(),
      );
    ...
    _initOptionItem: function() {
      ...
      this._on({
        ...
        'change [data-role="select-country"]': function(event, data) {
          var currentElement =
            ...
            parentElement = $(currentElement).closest(
          ...
          updater = new RegionUpdater(
            parentElement.find('[data-role="select-country"]').attr('id'),
            null,
            parentElement.find('[data-role="select-state"]').attr('id'),
            widget.options.region,
            'disable',
            true,
          );
          updater.update();
      ...
      $(this.options.bundlePriceType).on('change', function(event) {
        var attributeItems = widget.element.find(
        ...
        if ($(event.target).val() === '0') {
          ...
          attributeItems.each(function() {
            if (
              $(this)
                .closest('[data-role="fpt-item-row"]')
                .is(':visible')
            ) {
              $(this).val(0);
            }
    ...
    addItem: function(event) {
      ...
      if (data['website_id']) {
        this.element
          .find(
            '[data-role="select-website"][id$="_' + data.index + '_website"]',
          )
          .val(data['website_id']);
      }
      ...
      if (data.country) {
        this.element
          .find(
            '[data-role="select-country"][id$="_' + data.index + '_country"]',
          )
          .val(data.country)
          .trigger('change', data);
      }
