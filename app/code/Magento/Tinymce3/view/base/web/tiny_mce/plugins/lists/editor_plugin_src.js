...
(function() {
  ...
  function attemptMergeWithPrevious(
    ...
    if (prev) {
      return attemptMerge(
        prev,
        e,
        allowDifferentListStyles ? prev : false,
        mergeParagraphs,
      );
    } else {
      return e;
  ...
  function attemptMergeWithNext(e, allowDifferentListStyles, mergeParagraphs) {
    ...
    if (next) {
      return attemptMerge(
        e,
        next,
        allowDifferentListStyles ? next : false,
        mergeParagraphs,
      );
    } else {
      return e;
  ...
  function isListForIndent(e) {
    ...
    return (
      ...
      firstLI === lastLI &&
      (isList(firstLI) ||
        firstLI.style.listStyleType === 'none' ||
  ...
  tinymce.create('tinymce.plugins.Lists', {
    init: function(ed) {
      ...
      function isNestedList(grandParent) {
        if (isList(grandParent)) {
          return (
            grandParent.parentNode && grandParent.parentNode.tagName === 'LI'
          );
        } else {
      ...
      function getLi() {
        ...
        return (n.tagName == 'BR' || n.tagName == '') &&
          n.parentNode.tagName == 'LI'
          ? n.parentNode
          : n;
      }
      ...
      function imageJoiningListItem(ed, e) {
        ...
        var n = ed.selection.getStart();
        if (e.keyCode != tinymce.VK.BACKSPACE || n.tagName !== 'IMG') return;
        ...
        if (prevSibling.tagName === 'UL' || prevSibling.tagName === 'OL')
          ...
          (prevSibling.previousSibling.tagName === 'UL' ||
            prevSibling.previousSibling.tagName === 'OL')
        )
      ...
      ed.onKeyUp.add(function(ed, e) {
        if (state == LIST_TABBING) {
          ...
          var li = getLi();
          var shouldOutdent =
            ed.settings.list_outdent_on_enter === true || e.shiftKey;
    ...
    applyList: function(targetListType, oppositeListType) {
        ...
        if (element.tagName === 'LI') {
          ...
          processBrs(element, function(startSection, br) {
            doWrapList(
              startSection,
              br,
              element.tagName === 'BODY' ? null : startSection.parentNode,
            );
      ...
      function doWrapList(start, end, template) {
        var li,
          ...
          tmp;
        while (
          !dom.isBlock(start.parentNode) &&
          start.parentNode !== dom.getRoot()
        ) {
      ...
      function processBrs(element, callback) {
        ...
        function isAnyPartSelected(start, end) {
          ...
          return !(
            ...
            r.compareBoundaryPoints(START_TO_END, sel) <= 0
          );
        }

        function nextLeaf(br) {
          if (br.nextSibling) return br.nextSibling;
        ...
        trailingContentSelected =
          startSection && isAnyPartSelected(startSection, undefined);
        startSection = element.firstChild;
        each(dom.select(breakElements, element), function(br) {
      ...
      each(selectedBlocks, function(e) {
        ...
        if (
          ...
          (e.tagName === 'LI' && e.parentNode.tagName === oppositeListType)
        ) {
          hasOppositeType = true;
        } else if (
          e.tagName === targetListType ||
          (e.tagName === 'LI' && e.parentNode.tagName === targetListType)
        ) {
          hasSameType = true;
        } else {
          hasNonList = true;
    ...
    process: function(actions) {
      ...
      function isEmptyElement(element) {
        var excludeBrsAndBookmarks = tinymce.grep(element.childNodes, function(
          n,
        ) {
          return !(
            n.nodeName === 'BR' ||
            (n.nodeName === 'SPAN' &&
              dom.getAttrib(n, 'data-mce-type') == 'bookmark') ||
            (n.nodeType == 3 &&
      ...
      function processElement(element) {
        ...
        if (
          !element ||
          element.nodeType !== 1 ||
          (selectedBlocks.length > 1 && isEmptyElement(element))
        ) {
          return;
        }
