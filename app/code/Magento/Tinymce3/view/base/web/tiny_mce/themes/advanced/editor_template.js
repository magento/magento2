(function(e) {
  ...
  e.create('tinymce.themes.AdvancedTheme', {
    ...
    init: function(j, k) {
      ...
      j.forcedHighContrastMode =
        j.settings.detect_highcontrast && l._isHighContrast();
      j.settings.skin = j.forcedHighContrastMode
        ? 'highcontrast'
        : j.settings.skin;
      l.settings = m = h(
        {
    ...
    _isHighContrast: function() {
      ...
      i = (d.getStyle(j, 'background-color', true) + '')
        .toLowerCase()
        .replace(/ /g, '');
      d.remove(j);
    ...
    _createFontSizeSelect: function() {
      ...
      n = k.controlManager.createListBox('fontsizeselect', {
        ...
        onselect: function(i) {
          ...
          if (
            o &&
            (o.value.fontSize == i.fontSize || o.value['class'] == i['class'])
          ) {
            n.select(null);
          }
          return false;
    ...
    renderUI: function(k) {
      var m,
        ...
        i;
      if (r.settings) {
        r.settings.aria_label =
          w.aria_label + r.getLang('advanced.help_shortcut');
      }
      m = j = d.create('span', {
        role: 'application',
      ...
      d.add(
        m,
        'span',
        { class: 'mceVoiceLabel', style: 'display:none;', id: r.id + '_voice' },
        w.aria_label,
      );
      if (!d.boxModel) {
    ...
    _addToolbars: function(x, k) {
      ...
      y = 'mce' + A._ufirst(y);
      l = d.add(d.add(x, 'tr', { role: 'presentation' }), 'td', {
        class: 'mceToolbar ' + y,
        role: 'presentation',
      });
    ...
    _nodeChanged: function(m, r, D, q, E) {
      ...
      if (z.theme_advanced_path && z.theme_advanced_statusbar_location) {
        C =
          d.get(m.id + '_path') ||
          d.add(m.id + '_path_row', 'span', { id: m.id + '_path' });
        ...
        o(function(H) {
          ...
          if (
            H.nodeType != 1 ||
            p === 'br' ||
            H.getAttribute('data-mce-bogus') ||
            d.hasClass(H, 'mceItemHidden') ||
            d.hasClass(H, 'mceItemRemoved')
          ) {
            return;
          }
          ...
          if (C.hasChildNodes()) {
            C.insertBefore(
              d.create('span', { 'aria-hidden': 'true' }, '\u00a0\u00bb '),
              C.firstChild,
            );
      ...
      i.windowManager.open(
        {
          ...
          height: 260 + parseInt(i.getLang('advanced.charmap_delta_height', 0)),
          inline: true,
        },
        { theme_url: this.url },
      );
