...
(function() {
  var rootAttributes = tinymce.explode(
  ...
  tinymce.create('tinymce.plugins.MediaPlugin', {
    init: function(ed, url) {
      ...
      for (i = 0; i < mediaTypes.length; i++) {
        ...
        for (y = 0; y < item.mimes.length; y++) lookup[item.mimes[y]] = item;
      ...
      ed.onPreInit.add(function() {
        ...
        ed.parser.addNodeFilter(
          ...
          function(nodes) {
            ...
            while (i--) self.objectToImg(nodes[i]);
          },
        );
      ...
      ed.onInit.add(function() {
        ...
        if (ed && ed.plugins.contextmenu) {
          ed.plugins.contextmenu.onContextMenu.add(function(
            plugin,
            menu,
            element,
          ) {
      ...
      function addPlayer(video_src, poster_src) {
        ...
        if (flashPlayer) {
          ...
          tinymce.each(flashVars, function(value, name) {
            ...
            if (value.length > 0)
              flashVarsOutput +=
                (flashVarsOutput ? '&' : '') + name + '=' + escape(value);
          });
    ...
    objectToImg: function(node) {
      ...
      if (video) {
        if (node.name === 'video') type = lookup.video.name;
        else if (node.name === 'audio') type = lookup.audio.name;
      }
