...
function insertDateTime(id) {
  document.getElementById(id).value = getDateTime(
    new Date(),
    '%Y-%m-%dT%H:%M:%S',
  );
}
...
SXE.initElementDialog = function(element_name) {
  ...
  var elm = SXE.inst.dom.getParent(
    SXE.focusElement,
    element_name.toUpperCase(),
  );
  ...
  document.forms[0].insert.value = tinyMCEPopup.getLang(
    SXE.currentAction,
    'Insert',
    true,
  );
