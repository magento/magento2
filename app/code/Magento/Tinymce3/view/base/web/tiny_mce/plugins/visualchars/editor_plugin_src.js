    ...
    _toggleVisualChars: function(bookmark) {
      ...
      if (t.state) {
        ...
        for (i = 0; i < nl.length; i++) {
          ...
          nv = nv.replace(
            /(\u00a0)/g,
            '<span data-mce-bogus="1" class="mceItemHidden mceItemNbsp">$1</span>',
          );

          div = ed.dom.create('div', null, nv);
          while ((node = div.lastChild)) ed.dom.insertAfter(node, nl[i]);
        ...
        for (i = nl.length - 1; i >= 0; i--) ed.dom.remove(nl[i], 1);
      }
