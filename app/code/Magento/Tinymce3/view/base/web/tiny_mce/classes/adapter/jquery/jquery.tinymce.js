...
(function($) {
  ...
  $.fn.tinymce = function(settings) {
    ...
    // Get editor instance
    if (!settings) return tinyMCE.get(self[0].id);
    ...
    function init() {
      ...
      self.each(function(i, node) {
        ...
        if (!id) node.id = id = tinymce.DOM.uniqueId();
    ...
    if (!win['tinymce'] && !lazyLoading && (url = settings.script_url)) {
      ...
      $.ajax({
        ...
        success: function() {
          ...
          if (settings.script_loaded) settings.script_loaded();
      ...
      if (lazyLoading === 1) delayedInits.push(init);
      else init();
    }
  ...
  function applyPatch() {
    ...
    function loadOrSave(value) {
      ...
      if (value !== undefined) {
        ...
        self.each(function(i, node) {
          ...
          if ((ed = tinyMCE.get(node.id))) ed.setContent(value);
        });
      } else if (self.length > 0) {
        // Handle get value
        if ((ed = tinyMCE.get(self[0].id))) return ed.getContent();
      }
    }
    ...
    $.each(['text', 'html', 'val'], function(i, name) {
      ...
      $.fn[name] = function(value) {
        ...
        if (!containsTinyMCE(self)) return origFn.apply(self, arguments);
    ...
    $.fn.attr = function(name, value, type) {
      ...
      if (value !== undefined) {
        ...
        return ed
          ? ed.getContent()
          : jQueryFn.attr.call($(node), name, value, type);
      }
    };
  }
})(jQuery);
