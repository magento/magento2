...
(function(tinymce) {
  ...
  function unpack(lookup, data) {
    ...
    // Unpack lookup
    for (key in lookup) {
      if (lookup.hasOwnProperty(key)) lookup[key] = replace(lookup[key]);
    }
  ...
  blockElementsMap = makeMap(
    blockElementsMap,
    ',',
    makeMap(blockElementsMap.toUpperCase()),
  );
  ...
  shortEndedElementsMap = makeMap(
    'area,base,basefont,br,col,frame,hr,img,input,isindex,link,meta,param,embed,source',
  );
  nonEmptyElementsMap = tinymce.extend(
    makeMap('td,th,iframe,video,audio,object'),
    shortEndedElementsMap,
  );
  defaultWhiteSpaceElementsMap = makeMap('pre,script,style,textarea');
  selfClosingElementsMap = makeMap(
  ...
  tinymce.html.Schema = function(settings) {
    ...
    whiteSpaceElementsMap = settings.whitespace_elements
      ? makeMap(settings.whitespace_elements)
      : defaultWhiteSpaceElementsMap;
                ...
                if (matches) {
                  ...
                  if (attrType === '-') {
                    ...
                    attributesOrder.splice(
                      tinymce.inArray(attributesOrder, attrName),
                      1,
                    );
                    continue;
                  }
      ...
      if (valid_children) {
        each(split(valid_children), function(rule) {
          ...
          if (matches) {
            ...
            if (prefix) parent = children[matches[2]];
            else parent = children[matches[2]] = { '#comment': {} };

            parent = children[matches[2]];

            each(split(matches[3], '|'), function(child) {
              if (prefix === '-') delete parent[child];
              else parent[child] = {};
            });
          }
        });
    ...
    if (!settings.valid_elements) {
      ...
      each(
        split('ol,ul,sub,sup,blockquote,span,font,a,table,tbody,tr'),
        function(name) {
