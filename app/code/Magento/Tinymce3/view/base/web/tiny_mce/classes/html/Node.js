...
(function(tinymce) {
  ...
  tinymce.extend(Node.prototype, {
    ...
    insert: function(node, ref_node, before) {
      ...
      if (before) {
        if (ref_node === parent.firstChild) parent.firstChild = node;
        else ref_node.prev.next = node;

        node.prev = ref_node.prev;
        node.next = ref_node;
        ref_node.prev = node;
      } else {
        if (ref_node === parent.lastChild) parent.lastChild = node;
        else ref_node.next.prev = node;
    ...
    getAll: function(name) {
      ...
      for (node = self.firstChild; node; node = walk(node, self)) {
        if (node.name === name) collection.push(node);
      }
        ...
        do {
          if (node.type === 1) {
            ...
            while (i--) {
              ...
              if (name === 'name' || name.indexOf('data-') === 0) return false;
            }
          }
