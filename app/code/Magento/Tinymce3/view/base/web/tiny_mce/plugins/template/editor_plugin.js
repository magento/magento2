  ...
  tinymce.create('tinymce.plugins.TemplatePlugin', {
    ...
    _insertTemplate: function(i, j) {
      ...
      a(d.select('*', c), function(h) {
        if (
          e(
            h,
            g.getParam('template_cdate_classes', 'cdate').replace(/\s+/g, '|'),
          )
        ) {
          h.innerHTML = k._getDateTime(
            new Date(),
            g.getParam(
              'template_cdate_format',
              g.getLang('template.cdate_format'),
            ),
          );
        }
        if (
          e(
            h,
            g.getParam('template_mdate_classes', 'mdate').replace(/\s+/g, '|'),
          )
        ) {
          h.innerHTML = k._getDateTime(
            new Date(),
            g.getParam(
              'template_mdate_format',
              g.getLang('template.mdate_format'),
            ),
          );
        }
        if (
    ...
    _getDateTime: function(e, b) {
      ...
      b = b.replace(
        '%B',
        '' +
          this.editor.getLang('template_months_long').split(',')[e.getMonth()],
      );
      b = b.replace(
        '%b',
        '' +
          this.editor.getLang('template_months_short').split(',')[e.getMonth()],
      );
      b = b.replace(
        '%A',
        '' + this.editor.getLang('template_day_long').split(',')[e.getDay()],
      );
      b = b.replace(
        '%a',
        '' + this.editor.getLang('template_day_short').split(',')[e.getDay()],
      );
      b = b.replace('%%', '%');
      return b;
    },
