var firebug = {
  ...
  initConsole: function() {
    ...
    try {
      if (
        ...
        (firebug.env.override && !/Firefox\/3/i.test(navigator.userAgent))
      ) {
        window.console = { provider: 'Firebug Lite' };

        for (command in firebug.d.console.cmd) {
          window.console[command] = firebug.lib.util.Curry(
            firebug.d.console.run,
            window,
            command,
          );
        }
  ...
  init: function(_css) {
    var iconTitle =
      'Click here or press F12, (CTRL|CMD)+SHIFT+L or SHIFT+ENTER to show Firebug Lite. CTRL|CMD click this icon to hide it.';

    with (firebug) {
      if (
        document
          .getElementsByTagName('html')[0]
          .attributes.getNamedItem('debug')
      ) {
        env.debug =
          document
            .getElementsByTagName('html')[0]
            .attributes.getNamedItem('debug').nodeValue !== 'false';
      }
      ...
      if (env.isPopup) {
        ...
        env.popupWidth =
          window.opener.firebug.env.popupWidth ||
          window.opener.firebug.lib.util.GetViewport().width;
        env.popupHeight =
          window.opener.firebug.env.popupHeight ||
          window.opener.firebug.lib.util.GetViewport().height;
      } else {
      ...
      if (
        env.init ||
        (env.detectFirebug && window.console && window.console.firebug)
      ) {
        return;
      }

      document.getElementsByTagName('head')[0].appendChild(
        new lib.element('link').attribute
          .set('rel', 'stylesheet')
          .attribute.set('type', 'text/css')
          .attribute.set('href', env.css).element,
      );
       ...
       * Firebug Icon
       */
      el.firebugIcon = new lib.element('div').attribute
        .set('id', 'firebugIconDiv')
        .attribute.set('title', iconTitle)
        .attribute.set('alt', iconTitle)
        .event.addListener('mousedown', win.iconClicked)
       ...
       */
      el.content = {};
      el.mainiframe = new lib.element('IFRAME').attribute
        .set('id', 'FirebugIFrame')
        .environment.addStyle({
          display: 'none',
          width: lib.util.GetViewport().width + 'px',
        })
        .insert(document.body);
      el.main = new lib.element('DIV').attribute
        .set('id', 'Firebug')
        .environment.addStyle({
          display: 'none',
          width: lib.util.GetViewport().width + 'px',
        ...
        .insert(document.body);
      if (!env.isPopup) {
        el.resizer = new lib.element('DIV').attribute
          .addClass('Resizer')
          .event.addListener('mousedown', win.resizer.start)
          .insert(el.main);
      }
      el.header = new lib.element('DIV').attribute
        .addClass('Header')
        .insert(el.main);
      el.left = {};
      el.left.container = new lib.element('DIV').attribute
        .addClass('Left')
        .insert(el.main);
      el.right = {};
      el.right.container = new lib.element('DIV').attribute
        .addClass('Right')
        .insert(el.main);
       ...
       */
      el.button = {};
      el.button.container = new lib.element('DIV').attribute
        .addClass('ButtonContainer')
        .insert(el.header);
      el.button.logo = new lib.element('A').attribute
        .set('title', 'Firebug Lite')
        .attribute.set('target', '_blank')
        .attribute.set('href', 'http://getfirebug.com/lite.html')
        .update('&nbsp;')
        .attribute.addClass('Button Logo')
        .insert(el.button.container);
      el.button.inspect = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', env.targetWindow.firebug.d.inspector.toggle)
        .update('Inspect')
        .insert(el.button.container);
      el.button.dock = new lib.element('A').attribute
        .addClass('Button Dock')
        .event.addListener('click', win.dock)
        .insert(el.button.container);
      el.button.newWindow = new lib.element('A').attribute
        .addClass('Button NewWindow')
        .event.addListener('click', win.newWindow)
        .insert(el.button.container);

      if (!env.isPopup) {
        el.button.maximize = new lib.element('A').attribute
          .addClass('Button Maximize')
          .event.addListener('click', win.maximize)
          .insert(el.button.container);
        el.button.minimize = new lib.element('A').attribute
          .addClass('Button Minimize')
          .event.addListener('click', win.minimize)
          .insert(el.button.container);
        el.button.close = new lib.element('A').attribute
          .addClass('Button Close')
          .event.addListener('click', win.hide)
          .insert(el.button.container);
      }
       ...
       */
      el.nav = {};
      el.nav.container = new lib.element('DIV').attribute
        .addClass('Nav')
        .insert(el.left.container);
      el.nav.console = new lib.element('A').attribute
        .addClass('Tab Selected')
        .event.addListener(
          'click',
          lib.util.Curry(d.navigate, window, 'console'),
        )
        .update('Console')
        .insert(el.nav.container);
      el.nav.html = new lib.element('A').attribute
        .addClass('Tab')
        .update('HTML')
        .event.addListener('click', lib.util.Curry(d.navigate, window, 'html'))
        .insert(el.nav.container);
      el.nav.css = new lib.element('A').attribute
        .addClass('Tab')
        .update('CSS')
        .event.addListener('click', lib.util.Curry(d.navigate, window, 'css'))
        .insert(el.nav.container);
      if (!env.isPopup) {
        el.nav.scripts = new lib.element('A').attribute
          .addClass('Tab')
          .update('Script')
          .event.addListener(
            'click',
            lib.util.Curry(d.navigate, window, 'scripts'),
          )
          .insert(el.nav.container);
      }
      el.nav.dom = new lib.element('A').attribute
        .addClass('Tab')
        .update('DOM')
        .event.addListener(
          'click',
          lib.util.Curry(d.navigate, env.targetWindow, 'dom'),
        )
        .insert(el.nav.container);
      el.nav.xhr = new lib.element('A').attribute
        .addClass('Tab')
        .update('XHR')
        .event.addListener('click', lib.util.Curry(d.navigate, window, 'xhr'))
        .insert(el.nav.container);
      el.nav.optionsdiv = new lib.element('DIV').attribute
        .addClass('Settings')
        .insert(el.nav.container);
      el.nav.options = new lib.element('A').attribute
        .addClass('Tab')
        .update('Options&nbsp;&or;')
        .event.addListener('click', settings.toggle)
        .insert(el.nav.optionsdiv);

      /*
       * inspector
       */
      el.borderInspector = new lib.element('DIV').attribute
        .set('id', 'FirebugBorderInspector')
        .event.addListener('click', listen.inspector)
        .insert(document.body);
      el.bgInspector = new lib.element('DIV').attribute
        .set('id', 'FirebugBGInspector')
        .insert(document.body);
       ...
       */
      el.left.console = {};
      el.left.console.container = new lib.element('DIV').attribute
        .addClass('Console')
        .insert(el.left.container);
      el.left.console.mlButton = new lib.element('A').attribute
        .addClass('MLButton')
        .event.addListener('click', d.console.toggleML)
        .insert(el.left.console.container);
      el.left.console.monitor = new lib.element('DIV').insert(
        new lib.element('DIV').attribute
          .addClass('Monitor')
          .insert(el.left.console.container),
      );
      ...
      el.left.console.input = new lib.element('INPUT').attribute
        .set('type', 'text')
        .attribute.addClass('Input')
        .event.addListener('keydown', listen.consoleTextbox)
        .insert(
          new lib.element('DIV').attribute
      ...
      el.right.console = {};
      el.right.console.container = new lib.element('DIV').attribute
        .addClass('Console Container')
        .insert(el.right.container);
      el.right.console.mlButton = new lib.element('A').attribute
        .addClass('MLButton CloseML')
        .event.addListener('click', d.console.toggleML)
        .insert(el.right.console.container);
      el.right.console.input = new lib.element('TEXTAREA').attribute
        .addClass('Input')
        .insert(el.right.console.container);
      el.right.console.input.event.addListener(
        'keydown',
        lib.util.Curry(tab, window, el.right.console.input.element),
      );
      el.right.console.run = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', listen.runMultiline)
        .update('Run')
        .insert(el.right.console.container);
      el.right.console.clear = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener(
          'click',
          lib.util.Curry(d.clean, window, el.right.console.input),
        )
        .update('Clear')
        .insert(el.right.console.container);

      el.button.console = {};
      el.button.console.container = new lib.element('DIV').attribute
        .addClass('ButtonSet')
        .insert(el.button.container);
      el.button.console.clear = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', d.console.clear)
        .update('Clear')
        .insert(el.button.console.container);
      ...
      el.left.html = {};
      el.left.html.container = new lib.element('DIV').attribute
        .addClass('HTML')
        .insert(el.left.container);

      el.right.html = {};
      el.right.html.container = new lib.element('DIV').attribute
        .addClass('HTML Container')
        .insert(el.right.container);

      el.right.html.nav = {};
      el.right.html.nav.container = new lib.element('DIV').attribute
        .addClass('Nav')
        .insert(el.right.html.container);
      el.right.html.nav.computedStyle = new lib.element('A').attribute
        .addClass('Tab Selected')
        .event.addListener(
          'click',
          lib.util.Curry(d.html.navigate, firebug, 'computedStyle'),
        )
        .update('Computed Style')
        .insert(el.right.html.nav.container);
      el.right.html.nav.dom = new lib.element('A').attribute
        .addClass('Tab')
        .event.addListener(
          'click',
          lib.util.Curry(d.html.navigate, firebug, 'dom'),
        )
        .update('DOM')
        .insert(el.right.html.nav.container);

      el.right.html.content = new lib.element('DIV').attribute
        .addClass('Content')
        .insert(el.right.html.container);

      el.button.html = {};
      el.button.html.container = new lib.element('DIV').attribute
        .addClass('ButtonSet HTML')
        .insert(el.button.container);
      ...
      el.left.css = {};
      el.left.css.container = new lib.element('DIV').attribute
        .addClass('CSS')
        .insert(el.left.container);

      el.right.css = {};
      el.right.css.container = new lib.element('DIV').attribute
        .addClass('CSS Container')
        .insert(el.right.container);

      el.right.css.nav = {};
      el.right.css.nav.container = new lib.element('DIV').attribute
        .addClass('Nav')
        .insert(el.right.css.container);
      el.right.css.nav.runCSS = new lib.element('A').attribute
        .addClass('Tab Selected')
        .update('Run CSS')
        .insert(el.right.css.nav.container);

      el.right.css.mlButton = new lib.element('A').attribute
        .addClass('MLButton CloseML')
        .event.addListener('click', d.console.toggleML)
        .insert(el.right.css.container);
      el.right.css.input = new lib.element('TEXTAREA').attribute
        .addClass('Input')
        .insert(el.right.css.container);
      el.right.css.input.event.addListener(
        'keydown',
        lib.util.Curry(firebug.tab, window, el.right.css.input.element),
      );
      el.right.css.run = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', listen.runCSS)
        .update('Run')
        .insert(el.right.css.container);
      el.right.css.clear = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener(
          'click',
          lib.util.Curry(d.clean, window, el.right.css.input),
        )
        .update('Clear')
        .insert(el.right.css.container);

      el.button.css = {};
      el.button.css.container = new lib.element('DIV').attribute
        .addClass('ButtonSet CSS')
        .insert(el.button.container);
      el.button.css.selectbox = new lib.element('SELECT').event
        .addListener('change', listen.cssSelectbox)
        .insert(el.button.css.container);
      ...
      el.left.scripts = {};
      el.left.scripts.container = new lib.element('DIV').attribute
        .addClass('Scripts')
        .insert(el.left.container);

      el.right.scripts = {};
      el.right.scripts.container = new lib.element('DIV').attribute
        .addClass('Scripts Container')
        .insert(el.right.container);

      el.button.scripts = {};
      el.button.scripts.container = new lib.element('DIV').attribute
        .addClass('ButtonSet Scripts')
        .insert(el.button.container);
      el.button.scripts.selectbox = new lib.element('SELECT').event
        .addListener('change', listen.scriptsSelectbox)
        .insert(el.button.scripts.container);
      el.button.scripts.lineNumbers = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', d.scripts.toggleLineNumbers)
      ...
      el.left.dom = {};
      el.left.dom.container = new lib.element('DIV').attribute
        .addClass('DOM')
        .insert(el.left.container);

      el.right.dom = {};
      el.right.dom.container = new lib.element('DIV').attribute
        .addClass('DOM Container')
        .insert(el.right.container);

      el.button.dom = {};
      el.button.dom.container = new lib.element('DIV').attribute
        .addClass('ButtonSet DOM')
        .insert(el.button.container);
      el.button.dom.label = new lib.element('LABEL')
        .update('Object Path:')
        .insert(el.button.dom.container);
      el.button.dom.textbox = new lib.element('INPUT').event
        .addListener('keydown', listen.domTextbox)
        .update(env.isPopup ? 'window.opener' : 'window')
        .insert(el.button.dom.container);
       ...
       */
      el.left.str = {};
      el.left.str.container = new lib.element('DIV').attribute
        .addClass('STR')
        .insert(el.left.container);

      el.right.str = {};
      el.right.str.container = new lib.element('DIV').attribute
        .addClass('STR')
        .insert(el.left.container);

      el.button.str = {};
      el.button.str.container = new lib.element('DIV').attribute
        .addClass('ButtonSet XHR')
        .insert(el.button.container);
      el.button.str.watch = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', lib.util.Curry(d.navigate, window, 'xhr'))
        .update('Back')
        .insert(el.button.str.container);

      /*
       * xhr
       */
      el.left.xhr = {};
      el.left.xhr.container = new lib.element('DIV').attribute
        .addClass('XHR')
        .insert(el.left.container);

      el.right.xhr = {};
      el.right.xhr.container = new lib.element('DIV').attribute
        .addClass('XHR')
        .insert(el.left.container);

      el.button.xhr = {};
      el.button.xhr.container = new lib.element('DIV').attribute
        .addClass('ButtonSet XHR')
        .insert(el.button.container);
      el.button.xhr.label = new lib.element('LABEL')
        .update('XHR Path:')
        .insert(el.button.xhr.container);
      el.button.xhr.textbox = new lib.element('INPUT').event
        .addListener('keydown', listen.xhrTextbox)
        .insert(el.button.xhr.container);
      el.button.xhr.watch = new lib.element('A').attribute
        .addClass('Button')
        .event.addListener('click', listen.addXhrObject)
        .update('Watch')
        .insert(el.button.xhr.container);
       ...
       */
      el.settings = {};
      el.settings.container = new lib.element('DIV').child
        .add(
          new lib.element('DIV').attribute
            .addClass('Header')
            .child.add(
              new lib.element().attribute
                .addClass('Title')
                .update('Firebug Lite Settings'),
            ),
        )
        .attribute.addClass('SettingsDiv')
        .insert(el.main);
      el.settings.content = new lib.element('DIV').attribute
        .addClass('Content')
        .insert(el.settings.container);
      el.settings.progressDiv = new lib.element('DIV').attribute
        .addClass('ProgressDiv')
        .insert(el.settings.content);
      el.settings.progress = new lib.element('DIV').attribute
        .addClass('Progress')
        .insert(el.settings.progressDiv);
      el.settings.cbxDebug = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(document.createTextNode('Start visible'));
      new lib.element('BR').insert(el.settings.content);
      el.settings.cbxDetectFirebug = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(
        document.createTextNode('Hide when Firebug active'),
      );
      new lib.element('BR').insert(el.settings.content);
      el.settings.cbxHideDOMFunctions = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(
        document.createTextNode('Hide DOM functions'),
      );
      new lib.element('BR').insert(el.settings.content);
      el.settings.cbxOverride = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(
        document.createTextNode('Override window.console'),
      );
      new lib.element('BR').insert(el.settings.content);
      el.settings.cbxShowIcon = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(
        document.createTextNode('Show icon when hidden'),
      );
      new lib.element('BR').insert(el.settings.content);
      el.settings.cbxOpenInPopup = new lib.element('INPUT').attribute
        .set('type', 'checkbox')
        .attribute.addClass('SettingsCBX')
        .insert(el.settings.content);
      el.settings.content.child.add(document.createTextNode('Open in popup'));
      el.settings.buttonDiv = new lib.element('DIV').insert(
        el.settings.content,
      );
      el.settings.buttonLeftDiv = new lib.element('DIV').attribute
        .addClass('ButtonsLeft')
        .insert(el.settings.buttonDiv);
      el.settings.resetButton = new lib.element('INPUT').attribute
        .set('type', 'button')
        .update('Reset')
        .event.addListener('click', settings.reset)
        .insert(el.settings.buttonLeftDiv);
      el.settings.buttonRightDiv = new lib.element('DIV').attribute
        .addClass('ButtonsRight')
        .insert(el.settings.buttonDiv);
      el.settings.cancelButton = new lib.element('INPUT').attribute
        .set('type', 'button')
        .update('Cancel')
        .event.addListener('click', settings.hide)
        .insert(el.settings.buttonRightDiv);
      el.settings.buttonRightDiv.child.add(document.createTextNode(' '));
      el.settings.saveButton = new lib.element('INPUT').attribute
        .set('type', 'button')
        .update('Save')
        .event.addListener('click', settings.saveClicked)
        .insert(el.settings.buttonRightDiv);

      lib.util.AddEvent(document, 'mousemove', listen.mouse)(
        'mousemove',
        win.resizer.resize,
      )('mouseup', win.resizer.stop)('keydown', listen.keyboard);
  ...
  settings: {
    ...
    show: function() {
      with (firebug) {
        var posXY = lib.util.Element.getPosition(
          firebug.el.nav.options.element,
        );
    ...
    writeCookie: function() {
      ...
      with (firebug.env) {
        ...
        values +=
          'height:' +
          (parseInt(
            targetWindow.firebug.el.main.element.style.height.replace(/px/, ''),
            10,
          ) -
            38);
        ...
        targetWindow.document.cookie =
          'FBLiteSettings=' + values + ';expires=' + exdate.toUTCString();
      }
    },
  ...
  win: {
      ...
      if (!fe.popupWin) {
        ...
        fe.popupWin = window.open(
          ...
          'status=0,menubar=0,resizable=1,top=' +
            ...
            ',height=' +
            fe.popupHeight +
            ',scrollbars=0,addressbar=0,outerWidth=' +
            fe.popupWidth +
            ',outerHeight=' +
            fe.popupHeight +
        ...
        if (!fe.popupWin) {
          alert(
            'Firebug Lite could not open a pop-up window, most likely because of a popup blocker.\nPlease enable popups for this domain',
          );
        } else {
          ...
          if (scriptPath) {
            ...
            script[
              firebug.lib.env.ie ? 'onreadystatechange' : 'onload'
            ] = function() {
              if (
                !done &&
                (!firebug.lib.env.ie ||
                ...
                if (fe.popupWin.firebug) {
                  with (fe.popupWin.firebug) {
                    ...
                    el.button.newWindow.environment.addStyle({
                      display: 'none',
                    });
                  }
                }
            ...
            alert(
              'Unable to detect the following script "' +
                fe.liteFilename +
    ...
    resizer: {
      ...
      resize: function(_event) {
        with (firebug) {
          ...
          win.resizer.y[2] =
            win.resizer.y[0] + (win.resizer.y[1] - _event.clientY);
          el.main.environment.addStyle({ height: win.resizer.y[2] + 'px' });
          el.mainiframe.environment.addStyle({
            height: win.resizer.y[2] + 'px',
          });
          if (lib.env.ie6) {
            el.main.environment.addStyle({
              top:
                win.resizer.y[3] - (win.resizer.y[1] - _event.clientY) + 'px',
            });
            el.mainiframe.environment.addStyle({
              top:
                win.resizer.y[3] - (win.resizer.y[1] - _event.clientY) + 'px',
    ...
    setHeight: function(_height) {
      with (firebug) {
        ...
        // console
        el.left.console.monitor.element.parentNode.style.height =
          _height - 47 + 'px';
        el.left.console.mlButton.environment.addStyle({
          top: _height + 19 + 'px',
        });
        el.right.console.mlButton.environment.addStyle({
          top: _height + 19 + 'px',
        });
        el.right.console.input.environment.addStyle({
          height: _height - 29 + 'px',
        });

        // html
        el.left.html.container.environment.addStyle({
          height: _height - 23 + 'px',
        });
        el.right.html.content.environment.addStyle({
          height: _height - 23 + 'px',
        });

        // css
        el.left.css.container.environment.addStyle({
          height: _height - 33 + 'px',
        });
        el.right.css.input.environment.addStyle({
          height: _height - 55 + 'px',
        });

        // script
        el.left.scripts.container.environment.addStyle({
          height: _height - 23 + 'px',
        });

        // dom
        el.left.dom.container.environment.addStyle({
          height: _height - 31 + 'px',
        });

        // xhr
        el.left.xhr.container.environment.addStyle({
          height: _height - 32 + 'px',
        });

        // string
        el.left.str.container.environment.addStyle({
          height: _height - 32 + 'px',
        });
      }
    },
    refreshDOM: function() {
      with (firebug) {
        d.dom.open(
          eval(el.button.dom.textbox.environment.getElement().value),
          el.left.dom.container,
        );
    ...
    refreshSize: function() {
      with (firebug) {
        ...
        if (lib.env.ie6) win.setVerticalPosition(dim);
      }
    },
    setVerticalPosition: function(_dim, _event) {
      with (firebug) {
        ...
        el.main.environment.addStyle({
          top:
            dim.height -
            el.main.environment.getSize().offsetHeight +
            Math.max(
  ...
  d: {
    ...
    console: {
      addLine: function() {
        with (firebug) {
          return new lib.element('DIV').attribute
            .addClass('Row')
            .insert(el.left.console.monitor);
        }
      },
      cache: [],
      clear: function() {
      ...
      printException: function(_exception) {
        with (firebug) {
          var message =
            _exception.description || _exception.message || _exception;
          if (_exception.fileName) {
            message +=
              ' (' +
              ((_exception.name && _exception.name + ', ') || '') +
              getFileName(_exception.fileName) +
              ', ' +
              _exception.lineNumber +
              ')';
          }
          d.console
            .addLine()
            .attribute.addClass('Error')
            .update('<strong>Error: </strong>' + message, true);
        }
      },
      eval: function(_cmd) {
        var result;
      ...
      scroll: function() {
        with (firebug) {
          el.left.console.monitor.environment.getElement().parentNode.scrollTop = Math.abs(
            el.left.console.monitor.environment.getSize().offsetHeight -
              (el.left.console.monitor.element.parentNode.offsetHeight - 11),
          );
        }
      },
      run: function(_command) {
        with (firebug) {
          if (!env.init) {
            d.console.cache.push({
              command: _command,
              arg: Array.prototype.slice.call(arguments, 1),
            });
          } else {
            d.console.cmd[_command].apply(
              window,
      ...
      toggleML: function() {
        with (firebug) {
          ...
          el[open ? 'left' : 'right'].console.mlButton.environment.addStyle({
            display: 'none',
          });
          el[!open ? 'left' : 'right'].console.mlButton.environment.addStyle({
            display: 'block',
          });
          el.left.console.mlButton.attribute[
            (open ? 'add' : 'remove') + 'Class'
          ]('CloseML');
        }
      },
      countMap: {},
      ...
      cmd: {
        ...
        trace: function(_value) {
          with (firebug) {
            var stackAmt = 3,
              f = arguments.caller,
              isArray = lib.util.IsArray(f); //function that called trace

            if ((!isArray && f) || (isArray && f.length > 0)) {
              d.console
                .addLine()
                .attribute.addClass('Arrow')
                .update('>>> console.trace(stack)');
          ...
          with (firebug) {
            if (_name in d.console.timeMap) {
              var delta = new Date().getTime() - d.console.timeMap[_name],
                args = d.console.formatArgs.apply(window, [
                  _name + ':',
                  delta + 'ms',
                ]);
              d.console
        ...
        count: function(_name) {
          with (firebug) {
            if (!d.console.countMap[_name]) d.console.countMap[_name] = 0;
        ...
        groupEnd: function() {
          with (firebug) {
            d.console.cmd.log.apply(this, [
              'console.groupEnd is not supported',
            ]);
          }
        },
        profile: function() {
        ...
        profileEnd: function() {
          with (firebug) {
            d.console.cmd.log.apply(this, [
              'console.profileEnd is not supported',
            ]);
          }
        },
      },
      ...
      open: function(_index) {
        with (firebug) {
          var item = env.targetWindow.document.styleSheets[_index],
            uri = item.href;
          try {
            var rules = item[lib.env.ie ? 'rules' : 'cssRules'],
              str = '';
            for (var i = 0; i < rules.length; i++) {
              var item = rules[i];
              var selector = item.selectorText;
              var cssText = lib.env.ie
                ? item.style.cssText
                : item.cssText.match(/\{(.*)\}/)[1];
              str += d.css.printRule(
                selector,
                cssText.split(';'),
                el.left.css.container,
              );
            }
          } catch (e) {
        ...
        with (firebug) {
          ...
          for (var i = 0, len = _css.length; i < len; i++) {
            ...
            str +=
              ...
              item.replace(
                /(.+\:)(.+)/,
                "<span class='CSSProperty'>$1</span><span class='CSSValue'>$2;</span>",
              ) +
              '</div>';
          }
    ...
    dom: {
      open: function(_object, _layer) {
        with (firebug) {
          _layer.clean();
          var container = new lib.element('DIV').attribute
            .addClass('DOMContent')
            .insert(_layer);
        ...
        with (firebug) {
          ...
          for (var key in obj) {
            try {
              ...
              var value = obj[key],
                property = key,
                container = new lib.element('DIV').attribute
                  .addClass('DOMRow')
                  .insert(parentElement),
                left = new lib.element('DIV').attribute
                  .addClass('DOMRowLeft')
                  .insert(container),
                right = new lib.element('DIV').attribute
                  .addClass('DOMRowRight')
                  .insert(container);
              ...
              var link = new lib.element('A').attribute
                .addClass(
              ...
              var subContainer = new lib.element('DIV').attribute
                .addClass('DOMRowSubContainer')
                .insert(container);
              ...
              link.event.addListener(
                'click',
                lib.util.Curry(d.dom.print, window, value, subContainer, true),
              );
            } catch (e) {}
          }
      ...
      with (firebug) {
        ...
        if (vtype == 'object') {
          if (Object.prototype.toString.call(_value) === '[object Date]') {
            ...
            Object.prototype.toString.call(_value) === '[object Boolean]'
          ) {
            vtype = 'boolean';
          } else if (
            Object.prototype.toString.call(_value) === '[object RegExp]'
          ) {
            vtype = 'regexp';
          }
        }
        ...
        try {
          ...
          isElement =
            _value != undefined &&
            Boolean(_value.nodeName) &&
            Boolean(_value.nodeType);
            ...
            if (_value.nodeType == 3)
              ...
              result.push(
                "<span class='Blue" +
                  (!_link
                    ? "'"
                    : ' ObjectLink\' onmouseover=\'this.className=this.className.replace("ObjectLink","ObjectLinkHover")\' onmouseout=\'this.className=this.className.replace("ObjectLinkHover","ObjectLink")\' onclick=\'firebug.d.html.inspect(firebug.d.console.cache[' +
                      (d.console.cache.push(_value) - 1) +
                      "])'") +
                  "'>",
              );
              ...
              if (_inArray) {
                ...
                if (_value.getAttribute) {
                  if (_value.getAttribute && _value.getAttribute('id'))
                    result.push(
                      "<span class='DarkBlue'>#" +
                        _value.getAttribute('id') +
                        '</span>',
                    );
                  var elClass =
                    _value.getAttribute(
                      lib.env.ie && !lib.env.ie8 ? 'className' : 'class',
                    ) || '';
                  result.push(
                    !elClass
                      ? ''
                ...
                result.push(
                  "<span class='DarkBlue'>&#60;<span class='Blue TagName'>" +
                    _value.nodeName.toLowerCase(),
                );

                if (_value.attributes) {
                  for (
                    var i = 0, len = _value.attributes.length;
                    i < len;
                    i++
                  ) {
          ...
          // array, hash
            ...
            if (isArray) {
              if (_inObject) {
                result.push(
                  "<span class='Gray Italic'>[" + _value.length + ']</span>',
                );
              } else {
                ...
                for (var i = 0, len = _value.length; i < len; i++) {
                  ...
                  result.push(
                    (i > 0 ? ', ' : '') +
                      d.highlight(_value[i], false, true, true),
                  );
                }
              ...
              result.push(
                "<span class='Strong Green" +
                  (!_link
                    ? "'"
                    : ' ObjectLink\' onmouseover=\'this.className=this.className.replace("ObjectLink","ObjectLinkHover")\' onmouseout=\'this.className=this.className.replace("ObjectLinkHover","ObjectLink")\' onclick=\'firebug.d.console.openObject(' +
                      (d.console.cache.push(_value) - 1) +
                      ")'") +
                  '>Object',
              );
    ...
    html: {
      ...
      highlight: function(_element, _clear, _event) {
        with (firebug) {
          ...
          if (_clear) {
            env.targetWindow.firebug.el.bgInspector.environment.addStyle({
              display: 'none',
            });
            return;
          }
      ...
      inspect: function(_element) {
        ...
        with (firebug) {
          ...
          for (t = 0; t < el.left.html.container.child.get().length; t++) {
            if (
              el.left.html.container.child.get()[t].childNodes[0].childNodes[1]
                .childNodes[0].childNodes[0]
            ) {
              if (
                el.left.html.container.child.get()[t].childNodes[0]
                  .childNodes[1].childNodes[0].childNodes[0].innerText
              ) {
                tagName = el.left.html.container.child.get()[t].childNodes[0]
                  .childNodes[1].childNodes[0].childNodes[0].innerText;
              } else {
                tagName = el.left.html.container.child.get()[t].childNodes[0]
                  .childNodes[1].childNodes[0].childNodes[0].textContent;
              }

              if (/<body/i.test(tagName)) {
                parentLayer = el.left.html.container.child.get()[t]
                  .childNodes[1].lib;
                break;
              }
            }
          ...
          if (!parentLayer) {
            parentLayer = el.left.html.container.child.get()[3].childNodes[1]
              .lib;
          }

          for (t = 0, len = map.length; map[t]; t++) {
            if (t == len - 1) {
              var link = parentLayer.environment.getElement().previousSibling
                .lib;
      ...
      openHtmlTree: function(
        _element,
        _parent,
        _returnParentElementByElement,
        _event,
      ) {
        with (firebug) {
          ...
          if (
            element.childNodes &&
            (len == 0 || (len == 1 && element.childNodes[0].nodeType == 3))
          )
            return;
          parent.clean();
          ...
          if (
            parent.opened &&
            Boolean(_returnParentElementByElement) == false
          ) {
            parent.opened = false;
          ...
          if (element == document.documentElement) {
            new lib.element('A').attribute
              .addClass('Block')
              .update(
                "<span class='DarkBlue'>&#60;<span class='Blue'>html</span>&#62;",
              )
              .insert(parent);
          }
          ...
          for (var i = 0; i <= len; i++) {
            if (i == len) {
              new lib.element('A').attribute
                .addClass('Block')
                .update(
                  "<span class='DarkBlue'>&#60;/<span class='Blue'>" +
            ...
            if (item.nodeType != 3) {
              var container = new lib.element().attribute
                  .addClass('Block')
                  .insert(parent),
                link = new lib.element('A').attribute
                  .addClass('Link')
                  .insert(container),
                spacer = new lib.element('SPAN').attribute
                  .addClass('Spacer')
                  .update('&nbsp;')
                  .insert(link),
                html = new lib.element('SPAN').attribute
                  .addClass('Content')
                  .update(d.highlight(item))
                  .insert(link),
                subContainer = new lib.element('DIV').attribute
              ...
              link.event.addListener(
                'mouseover',
                lib.util.Curry(d.html.highlight, window, item, false),
              );
              link.event.addListener(
                'mouseout',
                lib.util.Curry(d.html.highlight, window, item, true),
              );

              returnParentVal =
                returnParentEl == item ? subContainer : returnParentVal;
              ...
              if (
                ...
                (view.visibility == 'hidden' || view.display == 'none')
              ) {
                ...
                if (childLen == 1 && item.childNodes[0].nodeType == 3) {
                  html.child.add(
                    document.createTextNode(
                      item.childNodes[0].nodeValue.substring(0, 50),
                    ),
                  );
                  html.child.add(document.createTextNode('</'));
                  html.child.add(
                    new lib.element('span').attribute
                      .addClass('Blue')
                      .update(item.nodeName.toLowerCase())
          ...
          switch (index) {
            case 'computedStyle':
              ...
              for (var i = 0, len = property.length; i < len; i++) {
                ...
                str +=
                  "<div class='CSSItem'><div class='CSSProperty'>" +
                  item +
                  "</div><div class='CSSValue'>" +
    ...
    inspector: {
      ...
      inspect: function(_element, _bgInspector) {
        with (firebug) {
          var pos = env.targetWindow.firebug.lib.util.Element.getPosition(
            _element,
          );

          env.targetWindow.firebug.el[
            (_bgInspector && 'bgInspector') || 'borderInspector'
          ].environment.addStyle({
            width: _element.offsetWidth + 'px',
            height: _element.offsetHeight + 'px',
            top: pos.offsetTop - (_bgInspector ? 0 : 2) + 'px',
            left: pos.offsetLeft - (_bgInspector ? 0 : 2) + 'px',
            display: 'block',
          });
          9;
          if (!_bgInspector) {
      ...
      toggle: function(_absoluteValue, _event) {
        with (firebug) {
          ...
          d.inspector.enabled =
            _absoluteValue != undefined && !_absoluteValue.clientX
              ? _absoluteValue
              : !d.inspector.enabled;
          el.button.inspect.attribute[
            (d.inspector.enabled ? 'add' : 'remove') + 'Class'
          ]('Enabled');
    ...
    scripts: {
      ...
      open: function(_index) {
        with (firebug) {
          ...
          var script = document.getElementsByTagName('script')[_index],
            uri = script.src || document.location.href,
            source;
          try {
            ...
            el.left.scripts.container.child.add(
              new lib.element('DIV').attribute
                .addClass('CodeContainer')
                .update('<em>Access to restricted URI denied</em>'),
            );
          }
        }
      },
      toggleLineNumbers: function() {
        with (firebug) {
          ...
          el.button.scripts.lineNumbers.attribute[
            (d.scripts.lineNumbers ? 'add' : 'remove') + 'Class'
          ]('Enabled');
      ...
      refresh: function() {
        with (firebug) {
          ...
          var collection = env.targetWindow.document.getElementsByTagName(
            'script',
          );
          for (var i = 0, len = collection.length; i < len; i++) {
            ...
            el.button.scripts.selectbox.child.add(
              new lib.element('OPTION').attribute
                .set('value', i)
                .update(fileName),
            );
          }
    ...
    xhr: {
      ...
      open: function() {
        with (firebug) {
          ...
          el.left.xhr.name = new lib.element('DIV').attribute
            .addClass('BlockContent')
            .insert(
              new lib.element('DIV').attribute
                .addClass('Block')
                .environment.addStyle({ width: '20%' })
                .insert(el.left.xhr.container),
            );
          el.left.xhr.nameTitle = new lib.element('STRONG')
            .update('Object Name:')
            .insert(el.left.xhr.name);
          el.left.xhr.nameContent = new lib.element('DIV').insert(
            el.left.xhr.name,
          );
          el.left.xhr.status = new lib.element('DIV').attribute
            .addClass('BlockContent')
            .insert(
              new lib.element('DIV').attribute
                .addClass('Block')
                .environment.addStyle({ width: '10%' })
                .insert(el.left.xhr.container),
            );
          el.left.xhr.statusTitle = new lib.element('STRONG')
            .update('Status:')
            .insert(el.left.xhr.status);
          el.left.xhr.statusContent = new lib.element('DIV').insert(
            el.left.xhr.status,
          );
          el.left.xhr.readystate = new lib.element('DIV').attribute
            .addClass('BlockContent')
            .insert(
              new lib.element('DIV').environment
                .addStyle({ width: '15%' })
                .attribute.addClass('Block')
                .insert(el.left.xhr.container),
            );
          el.left.xhr.readystateTitle = el.left.xhr.nameTitle = new lib.element(
            'STRONG',
          )
            .update('Ready State:')
            .insert(el.left.xhr.readystate);
          el.left.xhr.readystateContent = new lib.element('DIV').insert(
            el.left.xhr.readystate,
          );
          el.left.xhr.response = new lib.element('DIV').attribute
            .addClass('BlockContent')
            .insert(
              new lib.element('DIV').environment
                .addStyle({ width: (lib.env.ie ? '50' : '55') + '%' })
                .attribute.addClass('Block')
                .insert(el.left.xhr.container),
            );
          el.left.xhr.responseTitle = new lib.element('STRONG')
            .update('Response:')
            .insert(el.left.xhr.response);
          el.left.xhr.responseContent = new lib.element('DIV').insert(
            el.left.xhr.response,
          );
      ...
      refresh: function() {
        with (firebug) {
          ...
          for (var i = 0, len = d.xhr.objects.length; i < len; i++) {
            ...
            el.left.xhr.nameContent.child.add(
              new lib.element('span').update(item[0]),
            );
            try {
              el.left.xhr.statusContent.child.add(
                new lib.element('span').update(item[1].status),
              );
            } catch (e) {
              el.left.xhr.statusContent.child.add(
                new lib.element('span').update('&nbsp;'),
              );
            }
            el.left.xhr.readystateContent.child.add(
              new lib.element('span').update(item[1].readyState),
            );

            el.left.xhr.responseContent.child.add(
              new lib.element('span').child.add(
                new lib.element('A').event
                  .addListener(
                    'click',
                    lib.util.Curry(d.str.open, window, response),
                  )
                  .update('&nbsp;' + response.substring(0, 50)),
              ),
            );
          }
          if (env.dIndex == 'xhr') setTimeout(d.xhr.refresh, 500);
        }
      },
    },
    navigateRightColumn: function(_index, _open) {
      with (firebug) {
        el.left.container.environment.addStyle({
          width: _open ? '70%' : '100%',
        });
        el.right.container.environment.addStyle({
    ...
    navigate: function(_index) {
      with (firebug) {
        ...
        if (el.nav[close]) el.nav[close].attribute.removeClass('Selected');
        if (el.nav[open]) el.nav[open].attribute.addClass('Selected');
        ...
        switch (open) {
          ...
          case 'dom':
            d.navigateRightColumn(_index);
            if (
              el.left.dom.container.environment.getElement().innerHTML == ''
            ) {
              var t = Number(new Date());
              d.dom.open(
                eval(el.button.dom.textbox.environment.getElement().value),
    ...
    with (firebug) {
      if (_event.keyCode == 9) {
        if (_el.setSelectionRange) {
          ...
          _el.value =
            _el.value.substring(0, position.start) +
        ...
        if (lib.env.ie) setTimeout(_el.focus, 100);
      }
    }
  },
  listen: {
    addXhrObject: function() {
      with (firebug) {
        d.xhr.addObject.apply(
          env.targetWindow,
          el.button.xhr.textbox.environment.getElement().value.split(','),
        );
      }
    },
    consoleTextbox: function(_event) {
      with (firebug) {
        if (
          _event.keyCode == 13 &&
          (env.multilinemode == false || _event.shiftKey == false)
        ) {
    ...
    cssSelectbox: function() {
      with (firebug) {
        d.css.open(
          el.button.css.selectbox.environment.getElement().selectedIndex,
        );
      }
    },
    ...
    keyboard: function(_event) {
      with (firebug) {
        if (_event.keyCode == 27 && d.inspector.enabled) {
          ...
          (_event.keyCode === 123 && _event.ctrlKey) ||
          _event.metaKey
        ) {
          if (env.isPopup) {
            win.dock();
    ...
    runMultiline: function() {
      with (firebug) {
        d.console.eval.call(
          window,
          el.right.console.input.environment.getElement().value,
        );
      }
    },
    runCSS: function() {
      with (firebug) {
        var source = el.right.css.input.environment
          .getElement()
          .value.replace(/\n|\t/g, '')
          .split('}');
        for (var i = 0, len = source.length; i < len; i++) {
          var item = source[i] + '}',
            rule = !lib.env.ie ? item : item.split(/{|}/),
            styleSheet = document.styleSheets[0];
          console.log(rule);
          if (item.match(/.+\{.+\}/)) {
            if (lib.env.ie) styleSheet.addRule(rule[0], rule[1]);
            else styleSheet.insertRule(rule, styleSheet.cssRules.length);
          }
        }
      }
    },
    scriptsSelectbox: function() {
      with (firebug) {
        d.scripts.open(
          parseInt(el.button.scripts.selectbox.environment.getElement().value),
        );
      }
    },
...
(function(_scope) {
  ...
  pi.util = {
    Array: {
      ...
      forEach: function(_array, _function) {
        if (_array.forEach) return _array.forEach(_function);
    ...
    Extend: function(_superClass, _prototype, _skipClonning) {
      ...
      object.body =
        _superClass == pi.base
          ? _prototype
          : pi.util.Hash.merge(_prototype, _superClass.prototype);
      object.init =
        object.init ||
        function() {
          if (_superClass != pi.base) _superClass.apply(this, arguments);
        };
    ...
    IsArray: function(_object) {
      ...
      if (window.NodeList && window.NamedNodeMap && !pi.env.ie8) {
        if (
          ...
          _object instanceof NodeList ||
          _object instanceof NamedNodeMap ||
          (window.HTMLCollection && _object instanceof HTMLCollection)
        )
          return true;
      }
      if (
        !_object ||
        _object == window ||
        typeof _object == 'function' ||
        typeof _object == 'string' ||
        typeof _object.length != 'number'
      ) {
        return false;
      }
    ...
    IsHash: function(_object) {
      return (
        ...
        (_object == window || _object instanceof Object) &&
        !_object.nodeName &&
        !pi.util.IsArray(_object)
      );
    },
    Init: [],
    AddEvent: function(_element, _eventName, _fn, _useCapture) {
      _element[pi.env.ie ? 'attachEvent' : 'addEventListener'](
        (pi.env.ie ? 'on' : '') + _eventName,
        _fn,
        _useCapture || false,
      );
    ...
    RemoveEvent: function(_element, _eventName, _fn, _useCapture) {
      _element[pi.env.ie ? 'detachEvent' : 'removeEventListener'](
        (pi.env.ie ? 'on' : '') + _eventName,
        _fn,
        _useCapture || false,
      );
    ...
    Element: {
      addClass: function(_element, _class) {
        if (!pi.util.Element.hasClass(_element, _class))
          pi.util.Element.setClass(
            _element,
            pi.util.Element.getClass(_element) + ' ' + _class,
          );
      },
      getClass: function(_element) {
        return (
          _element.getAttribute(
            pi.env.ie && !pi.env.ie8 ? 'className' : 'class',
          ) || ''
        );
      },
      hasClass: function(_element, _class) {
        return (
          pi.util.Array.indexOf(
            pi.util.Element.getClass(_element).split(' '),
            _class,
          ) > -1
        );
      },
        ...
        if (pi.util.Element.hasClass(_element, _class)) {
          ...
          pi.util.Element.setClass(
            ...
            pi.util.Array.remove(
              names,
              pi.util.Array.indexOf(names, _class),
            ).join(' '),
          );
        }
      },
      ...
      toggleClass: function() {
        ...
        else pi.util.Element.addClass.apply(this, arguments);
      },
      getOpacity: function(_styleObject) {
        var styleObject = _styleObject;
        if (!pi.env.ie) return styleObject['opacity'];
      ...
      setOpacity: function(_element, _value) {
        ...
        _value *= 100;
        pi.util.Element.addStyle(_element, {
      ...
      getPosition: function(_element) {
        var parent = _element,
          ...
          view = pi.util.Element.getView(_element);
        while (
          parent &&
      ...
      getView: function(_element, _property) {
        var view = document.defaultView
          ? document.defaultView.getComputedStyle(_element, null)
          : _element.currentStyle;
        _property =
          _property == 'float'
            ? pi.env.ie
              ? 'styleFloat'
      ...
      merge: function(_hash, _source, _undeep) {
        for (var key in _source) {
          ...
          if (!_undeep && pi.util.IsArray(_source[key])) {
            ...
            } else value = pi.util.Array.clone(_source[key]);
          ...
          } else if (_hash[key]) value = _hash[key];
          _hash[key] = value;
        }
        return _hash;
      },
  ...
  pi.base = function() {
    ...
    this.build = function(_skipClonning) {
      var base = this,
        skipClonning = _skipClonning || false,
        _private = {},
        fn = function() {
          ...
          if (!skipClonning) {
            for (var key in this) {
              ...
              } else if (pi.util.IsHash(this[key])) {
                this[key] = pi.util.Hash.clone(this[key], function(
                  _key,
                  _object,
                ) {
          ...
          if (base.init) return base.init.apply(this, arguments);
          return this;
        };
    ...
    this.createAccessors = function(_p, _branch) {
      ...
      for (var name in _p) {
        var isPrivate = name.substring(0, 1) == '_',
          title = name.substring(1, 2).toUpperCase() + name.substring(2);

        if (isPrivate) {
          _branch[
            (_branch['get' + title] ? '_' : '') + 'get' + title
          ] = pi.util.Curry(getter, _p, name);
          _branch[
            (_branch['set' + title] ? '_' : '') + 'set' + title
          ] = pi.util.Curry(setter, _p, name);
        } else if (pi.util.IsHash(_p[name])) {
          _branch[name]._parent_ = _branch;
          if (!_branch[name]) _branch[name] = {};
    ...
    this.movePrivateMembers = function(_object, _branch) {
      for (var name in _object) {
        ...
        if (isPrivate) {
          ...
          delete _object[name];
        } else if (pi.util.IsHash(_object[name])) {
          _branch[name] = {};
  ...
  pi.element.body = {
    ...
    insert: function(_element) {
      _element = _element.environment
        ? _element.environment.getElement()
        : _element;
      _element.appendChild(this.environment.getElement());
      return this;
    },
    insertAfter: function(_referenceElement) {
      _referenceElement = _referenceElement.environment
        ? _referenceElement.environment.getElement()
        : _referenceElement;
      _referenceElement.nextSibling
        ? this.insertBefore(_referenceElement.nextSibling)
        : this.insert(_referenceElement.parentNode);
      return this;
    },
    insertBefore: function(_referenceElement) {
      _referenceElement = _referenceElement.environment
        ? _referenceElement.environment.getElement()
        : _referenceElement;
      _referenceElement.parentNode.insertBefore(
    ...
    query: function(_expression, _resultType, namespaceResolver, _result) {
      return pi.xpath(
        _expression,
        _resultType || 'ORDERED_NODE_SNAPSHOT_TYPE',
        this.environment.getElement(),
        _namespaceResolver,
        _result,
      );
    ...
    attribute: {
      ...
      has: function(_name) {
        return pi.env.ie
          ? this.get(_name) != null
          : this._parent_.environment.getElement().hasAttribute(_name);
      },
      ...
      addClass: function(_classes) {
        for (var i = 0, len = arguments.length; i < len; i++) {
          pi.util.Element.addClass(
            this._parent_.environment.getElement(),
            arguments[i],
          );
        }
        return this._parent_;
      ...
      hasClass: function(_class) {
        return pi.util.Element.hasClass(
          this._parent_.environment.getElement(),
          _class,
        );
      },
      setClass: function(_value) {
        return pi.util.Element.setClass(
          this._parent_.environment.getElement(),
          _value,
        );
      },
      removeClass: function(_class) {
        pi.util.Element.removeClass(
          this._parent_.environment.getElement(),
          _class,
        );
        return this._parent_;
    ...
    child: {
      ...
      add: function(_elements) {
        for (var i = 0; i < arguments.length; i++) {
          var el = arguments[i];
          this._parent_.environment
            .getElement()
            .appendChild(el.environment ? el.environment.getElement() : el);
        }
        return this._parent_;
      },
  ...
  pi.xhr.init = function(_url) {
    ...
    this.environment.getApi().onreadystatechange = pi.util.Curry(
      this.event.readystatechange,
      this,
    );
  ...
  pi.xhr.body = {
    ...
    send: function() {
      var url = this.environment.getUrl(),
        data = this.environment.getData(),
        dataUrl = '';

      if (!this.environment.getCache()) data['forceCache'] = Number(new Date());
      ...
      if (this.environment.getType() == 'GET')
        url +=
          (url.search('\\?') == -1 ? '?' : '&') +
          pi.util.String.format('{0}', dataUrl);

      this.api.open(
        this.environment.getType(),
        url,
        this.environment.getAsync(),
      );
  ...
  pi.xhr.body.event = {
    readystatechange: function() {
      var readyState = this.environment.getApi().readyState,
        callback = this.environment.getCallback();
