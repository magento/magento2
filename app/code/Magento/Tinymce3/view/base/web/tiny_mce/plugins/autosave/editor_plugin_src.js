  ...
  tinymce.create('tinymce.plugins.AutoSave', {
    ...
    init: function(ed, url) {
      ...
      ed.addButton(RESTORE_DRAFT, {
        ...
        onclick: function() {
          if (
            ed
              .getContent({ draft: true })
              .replace(/\s|&nbsp;|<\/?p[^>]*>|<br[^>]*>/gi, '').length > 0
          ) {
            // Show confirm dialog if the editor isn't empty
            ed.windowManager.confirm(PLUGIN_NAME + '.warning_message', function(
              ok,
            ) {
    ...
    storeDraft: function() {
      ...
      if (storage) {
        ...
        if (!storage.getItem(self.key) && !editor.isDirty()) return;
      ...
      if (storage) {
        ...
        if (exists) {
          ...
          if (!self.storage.autoExpires) {
            ...
            if (new Date().getTime() < expDate.getTime()) return TRUE;
    ...
    static: {
      ...
      _beforeUnloadHandler: function(e) {
        ...
        tinymce.each(tinyMCE.editors, function(ed) {
          ...
          if (ed.plugins.autosave) ed.plugins.autosave.storeDraft();
