    ...
    init: function(a, c) {
      ...
      function b() {
        var i = a.getDoc(),
          ...
          k = i.documentElement,
          h = tinymce.DOM,
          j = d.autoresize_min_height,
          g;
        g = tinymce.isIE ? f.scrollHeight : i.body.offsetHeight;
        if (g > d.autoresize_min_height) {
          j = g;
        }
        if (d.autoresize_max_height && g > d.autoresize_max_height) {
          j = d.autoresize_max_height;
          a.getBody().style.overflowY = 'auto';
        } else {
          a.getBody().style.overflowY = 'hidden';
        }
        if (j !== e) {
      ...
      d.editor = a;
      d.autoresize_min_height = parseInt(
        a.getParam('autoresize_min_height', a.getElement().offsetHeight),
      );
      d.autoresize_max_height = parseInt(
      ...
      a.onInit.add(function(f) {
        f.dom.setStyle(
          f.getBody(),
          'paddingBottom',
          f.getParam('autoresize_bottom_margin', 50) + 'px',
        );
      });
      a.onChange.add(b);
