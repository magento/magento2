...
tinyMCEPopup = {
  ...
  init: function() {
    ...
    // Setup local DOM
    t.dom = t.editor.windowManager.createInstance(
  ...
  requireLangPack: function() {
    var t = this,
      u = t.getWindowArg('plugin_url') || t.getWindowArg('theme_url');

    if (
      u &&
      t.editor.settings.language &&
      t.features.translate_i18n !== false &&
      t.editor.settings.language_load !== false
    ) {
  ...
  close: function() {
    ...
    if (tinymce.isOpera) t.getWin().setTimeout(close, 0);
    else close();
  },
  ...
  _onDOMLoaded: function() {
    ...
    if (t.features.translate_i18n !== false) {
      ...
      if ((nv = t.editor.translate(h)) && nv != h) document.body.innerHTML = nv;

      if ((nv = t.editor.translate(ti)) && nv != ti) document.title = ti = nv;
    }
    ...
    // Set inline title
    if (!t.isWindow) t.editor.windowManager.setTitle(window, ti);
    else window.focus();
  ...
  _closeWinKeyHandler: function(e) {
    ...
    if (e.keyCode == 27) tinyMCEPopup.close();
  },
