...
class LinkManagement implements \Magento\Bundle\Api\ProductLinkManagementInterface
{
    ...
    public function addChild(
        ...
        /**
         * selections
         * needs a store_id parameter as a filter.
         * the linked product is also present on catalog_product_website so there is no other way to have
         * even the same set or a different set of options when using multiple websites because they will be filtered
         * by the catalog_product_website query
         *
         * SELECT `e`.*, `selection`.* FROM `catalog_product_entity` AS `e`
        INNER JOIN `catalog_product_bundle_selection` AS `selection` ON selection.product_id = e.entity_id
        INNER JOIN `catalog_product_website` AS `product_website` ON product_website.product_id = e.entity_id AND product_website.website_id = '3' WHERE (((`e`.`required_options` != 1) OR (`e`.`required_options` IS NULL))) AND (selection.option_id IN ('24')) ORDER BY `selection`.`position` asc, `selection`.`selection_id` asc
         * generated by selection collection
         */
        if ($selections) {
