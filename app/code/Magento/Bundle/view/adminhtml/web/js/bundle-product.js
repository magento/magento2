  ...
  $.widget('mage.bundleProduct', {
    ...
    _initOptionBoxes: function() {
      ...
      syncOptionTitle = function(event) {
        ...
        optionBoxTitle.text(
          currentValue === '' && !originalValue.length
            ? newOptionTitle
            : currentValue,
        );
      };
    ...
    _bindAddSelectionDialog: function() {
      ...
      this._on({
        ...
        'click .add-selection': function(event) {
          ...
          $selectionGrid.modal({
            title:
              $optionBox.find('input[name$="[title]"]').val() === ''
                ? $.mage.__('Add Products to New Option')
                : $.mage.__('Add Products to Option "%1"').replace(
                    '%1',
                    $('<div>')
                      .text($optionBox.find('input[name$="[title]"]').val())
                      .html(),
                  ),
            modalClass: 'bundle',
            type: 'slide',
    ...
    _updateOptionBoxPositions: function() {
      $(this)
        .find('[name^=bundle_options][name$="[position]"]')
        .each(function(index) {
          $(this).val(index);
        });
    ...
    _updateSelectionsPositions: function() {
      $(this)
        .find('[name^=bundle_selections][name$="[position]"]')
        .each(function(index) {
          $(this).val(index);
        });
