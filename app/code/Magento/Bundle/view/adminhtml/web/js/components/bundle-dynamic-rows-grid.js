...
define(['underscore', 'Magento_Ui/js/dynamic-rows/dynamic-rows-grid'], function(
  _,
  dynamicRowsGrid,
) {
  'use strict';
  ...
  return dynamicRowsGrid.extend({
    ...
    onInputTypeChange: function(inputType) {
      if (
        this.defaultRecords.moreThanOne &&
      ...
      if (newData.length) {
        if (this.insertData().length) {
          ...
          _.each(
            newData,
            function(newRecord) {
              this.processingAddChild(
                newRecord,
                ++recordIndex,
                newRecord[this.identificationProperty],
              );
            },
            this,
          );
