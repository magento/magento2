 ...
 */
namespace Magento\PasswordManagement\Console;

use Symfony\Component\Console\Command\Command;
use Symfony\Component\Console\Input\InputArgument;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;
use Magento\PasswordManagement\Model\Resource\Admin\User as AdminUser;
...
class UnlockAdminAccountCommand extends Command
{
    ...
    const COMMAND_ADMIN_ACCOUT_UNLOCK = 'admin:user:unlock';
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $outputMessage = sprintf('Couldn\'t find the user "%s"', $adminUserName);
        if ($userData) {
            if (isset($userData[self::USER_ID]) && $this->adminUser->unlock($userData[self::USER_ID])) {
                $outputMessage = sprintf('The user "%s" has been unlocked', $adminUserName);
            } else {
                $outputMessage = sprintf('Couldn\'t unlock the user "%s"', $adminUserName);
            }
        }
    ...
    {
        $this->setName(self::COMMAND_ADMIN_ACCOUT_UNLOCK);
        ...
        $this->setHelp(
            <<<HELP
This command unlocks admin by its username.
To unlock:
      <comment>%command.full_name% user_name</comment>
HELP
        );
        parent::configure();
    }
