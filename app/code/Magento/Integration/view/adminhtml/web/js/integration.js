    ...
    var url = {
      ...
      IdentityLogin = {
        ...
        invokePopup: function(identityCallbackUrl, consumerKey, jqInfoDialog) {
          ...
          IdentityLogin.win = window.open(
            identityCallbackUrl + '?' + param,
            '',
            'top=' +
              IdentityLogin.Constants.TOP +
              ', left=' +
              IdentityLogin.Constants.LEFT +
              ', width=' +
              IdentityLogin.Constants.WIDTH +
              ', height=' +
              IdentityLogin.Constants.HEIGHT +
              ',scrollbars=no',
          );
          ...
          IdentityLogin.checker = setInterval(
            IdentityLogin.fnCheckLocation,
            IdentityLogin.Constants.CHECKER_INTERVAL,
          );
        },
        ...
        fnCheckLocation: function() {
          ...
          try {
            ...
            if (
              IdentityLogin.win.closed ||
              IdentityLogin.win.location.href ==
                IdentityLogin.successCallbackUrl //eslint-disable-line eqeqeq
            ) {
              //Stop the polling
              clearInterval(IdentityLogin.checker);
              $('body').trigger('processStart');
          ...
          success: function(result) {
            ...
            dialogProperties = {
              ...
              dialogClass:
                dialog == 'permissions'
                  ? 'integration-dialog'
                  : 'integration-dialog no-close', //eslint-disable-line
            };
    ...
    return {
      popup: {
        ...
        show: function(ctx) {
          ...
          ajaxUrl = url[dialog]
            .replace(':id', integrationId)
            .replace(':isReauthorize', isReauthorize);

          try {
            // Get integration name either from current element or from neighbor column
            integrationName =
              $(ctx).attr('data-row-name') ||
              $(ctx)
                .parents('tr')
                .find('.col-name')
                .html()
            ...
            if (integrationName.indexOf('<span') > -1) {
              ...
              integrationName = integrationName.substring(
                0,
                integrationName.indexOf('<span'),
              );
            }
          } catch (e) {
          ...
          okButton = {
            permissions: {
              text:
                isReauthorize == '1'
                  ? $.mage.__('Reauthorize')
                  : $.mage.__('Allow'), //eslint-disable-line
              class: 'action-primary',
              attr: {
                'data-row-id': integrationId,
                'data-row-name': integrationName,
                'data-row-dialog':
                  isTokenExchange == '1' ? 'tokensExchange' : 'tokens', //eslint-disable-line
              ...
              click: function() {
                ...
                var context = this.modal
                  .find('button.action-primary')
                  .clone(true);
