...
/*browser:true*/
/*global define*/
define(['jquery', 'uiComponent', 'Magento_Ui/js/modal/alert'], function(
  $,
  Class,
  alert,
) {
  ...
  return Class.extend({
    ...
    initObservable: function() {
      ...
      self.$selector.on('setVaultNotActive.' + self.getCode(), function() {
    ...
    initEventHandlers: function() {
      $(this.$container)
        .find('[name="payment[token_switcher]"]')
    ...
    createPublicHashSelector: function() {
      ...
      if (
        this.$container.find('#' + this.getNonceSelectorName()).size() === 0
      ) {
        $input = $('<input>').attr({
          type: 'hidden',
