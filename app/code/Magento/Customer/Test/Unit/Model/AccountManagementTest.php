...
class AccountManagementTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testSendNotificationEmailsIfRequired($testNumber, $oldEmail, $newEmail, $isPasswordChanged)
    {
        ...
        switch ($testNumber) {
            case 1:
                $xmlPathTemplate = AccountManagement::XML_PATH_RESET_PASSWORD_TEMPLATE;
                $expects = $this->once();
                break;
            case 2:
                $xmlPathTemplate = AccountManagement::XML_PATH_CHANGE_EMAIL_TEMPLATE;
                $expects = $this->exactly(2);
                break;
            case 3:
                $xmlPathTemplate = AccountManagement::XML_PATH_CHANGE_EMAIL_AND_PASSWORD_TEMPLATE;
                $expects = $this->exactly(2);
                break;
        }
        ...
        $this->storeManager->expects(clone $expects)
            ->method('getStore')
            ->willReturn($this->store);

        $this->customerRegistry->expects(clone $expects)
            ->method('retrieveSecureData')
            ->with($customerId)
            ->willReturn($this->customerSecure);

        $this->dataObjectProcessor->expects(clone $expects)
            ->method('buildOutputDataArray')
        ...
        $this->customerSecure->expects(clone $expects)
            ->method('addData')
            ->with($customerData)
            ->willReturnSelf();
        $this->customerSecure->expects(clone $expects)
        ...
        $this->scopeConfig->expects($this->any())
            ...
            ->withConsecutive(
                ...
                [AccountManagement::XML_PATH_FORGOT_EMAIL_IDENTITY, ScopeInterface::SCOPE_STORE, $customerStoreId],
                [$xmlPathTemplate, ScopeInterface::SCOPE_STORE, $customerStoreId],
                [AccountManagement::XML_PATH_FORGOT_EMAIL_IDENTITY, ScopeInterface::SCOPE_STORE, $customerStoreId]
            )
        ...
        $this->transportBuilder->expects(clone $expects)
            ->method('setTemplateIdentifier')
            ->with($templateIdentifier)
            ->willReturnSelf();
        $this->transportBuilder->expects(clone $expects)
     ...
     * @param bool $result
     *
     * @dataProvider validatePasswordByIdDataProvider
     */
    public function testValidatePasswordById($result)
    {
        $customerId = 1;
        $password = '1234567';
        $hash = '4nj54lkj5jfi03j49f8bgujfgsd';
        ...
        $this->assertEquals($result, $this->accountManagement->validatePasswordById($customerId, $password));
    }

    /**
     * @return array
     */
    public function validatePasswordByIdDataProvider()
    {
        return [[true], [false]];
    }
}
