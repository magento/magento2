...
define([
  ...
  $.widget('mage.addressTabs', $.mage.tabs, {
    ...
    _addNewAddress: function() {
      ...
      while (
        this.element.find('div[data-item=' + this.options.itemCount + ']')
          .length
      ) {
        this.options.itemCount++;
      }
      ...
      formTemplate = this.element
        .find(this.options.formTemplateSelector)
        .html();
      ...
      this.element
        .find(this.options.formsSelector)
        .append(this._prepareTemplate(formTemplate));
      ...
      newForm
        .find(this.options.formLastNameSelector)
        .val($(this.options.accountLastNameSelector).val());

      accountWebsiteId = $(this.options.accountWebsiteIdSelector).val();

      if (
        accountWebsiteId !== '' &&
        typeof this.options.defaultCountries[accountWebsiteId] !== 'undefined'
      ) {
        newForm
          .find(this.options.formCountrySelector)
          .val(this.options.defaultCountries[accountWebsiteId]);
      }
    ...
    _syncFormData: function(container) {
      ...
      if (container) {
        $(container)
          .find(':input')
          .each(function(index, inputField) {
                ...
                if (inputField.multiple) {
                  ...
                  text =
                    option.value == '0' || option.value === ''
                      ? ''
                      : option.text; //eslint-disable-line
  ...
  $.widget('mage.addressCountry', {
    ...
    _checkRegionRequired: function(elements, activeElement, regionField) {
      var regionRequired =
        this.options.requiredStateForCountries.indexOf(
          this.options.countryElement.value,
        ) >= 0;
      ...
      elements.each(function(currentElement) {
        ...
        if (!regionRequired) {
          ...
          if (
            currentElement.prop('tagName').toLowerCase() === 'select' &&
          ...
          if (activeElement == currentElement) {
            ...
            if (
              currentElement.prop('tagName').toLowerCase() === 'select' && //eslint-disable-line max-depth
              !currentElement.hasClass('validate-select')
            ) {
    ...
    _setPostcodeOptional: function(countryElement) {
      ...
      if (
        this.options.optionalZipCountries.indexOf(countryElement.value) !== -1
      ) {
