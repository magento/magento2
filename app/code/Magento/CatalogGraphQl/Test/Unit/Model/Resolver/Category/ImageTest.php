...
class ImageTest extends TestCase
{
    ...
    public function testResolve()
    {
        ...
        $context->expects($this->once())->method('getExtensionAttributes')->will($this->returnValue($extensionAttribute));
        $info = $this->createMock(\Magento\Framework\GraphQl\Schema\Type\ResolveInfo::class);
        $categoryMock  = $this->createPartialMock(
            \Magento\Catalog\Model\Category::class,
            ['getImage']
        );
        $categoryMock->expects($this->once())->method('getImage')->will($this->returnValue('media/catalog/tmp/category/image.jpeg'));
