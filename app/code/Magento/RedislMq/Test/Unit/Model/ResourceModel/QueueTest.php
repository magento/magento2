...
namespace Magento\RedisMq\Test\Unit\Model\ResourceModel;
...
class QueueTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * @var \Magento\RedisMq\Model\ResourceModel\Queue
     */
    private $queue;
    ...
    protected function setUp()
    {
        ...
        $this->queue = $objectManager->getObject(
            \Magento\RedisMq\Model\ResourceModel\Queue::class,
            [
        ...
        $connection->expects($this->once())->method('insertArray')->with(
            ...
            [
                [
                    $queueIds[0],
                    $messageId,
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_NEW
                ],
                [
                    $queueIds[1],
                    $messageId,
    ...
    public function testGetMessages()
    {
        ...
        $select->expects($this->once())->method('from')->with(
            ...
            [
                \Magento\RedisMq\Model\QueueManagement::MESSAGE_TOPIC => 'topic_name',
                \Magento\RedisMq\Model\QueueManagement::MESSAGE_BODY => 'body'
            ]
        )->willReturnSelf();
        $select->expects($this->exactly(2))->method('join')->withConsecutive(
            [
                ...
                [
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_QUEUE_RELATION_ID => 'id',
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_QUEUE_ID => 'queue_id',
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_ID => 'message_id',
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS => 'status',
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_UPDATED_AT => 'updated_at',
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_NUMBER_OF_TRIALS => 'number_of_trials'
                ]
            ],
            [
                ['queue' => $tableNames[2]],
                'queue.id = queue_message_status.queue_id',
                [\Magento\RedisMq\Model\QueueManagement::MESSAGE_QUEUE_NAME => 'name']
            ]
        )->willReturnSelf();
        $select->expects($this->exactly(2))->method('where')->withConsecutive(
            [
                ...
                [
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_NEW,
                    \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_RETRY_REQUIRED
                ]
            ],
            [
    ...
    public function testDeleteMarkedMessages()
    {
        ...
        $select->expects($this->once())->method('where')
            ->with('status <> ?', \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_TO_BE_DELETED)
            ->willReturnSelf();
    ...
    public function testTakeMessagesInProgress()
    {
        ...
        $connection->expects($this->exactly(2))->method('update')->withConsecutive(
            [
                $tableName,
                ['status' => \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_IN_PROGRESS],
                ['id = ?' => $relationIds[0]]
            ],
            [
                $tableName,
    ...
    public function testPushBackForRetry()
    {
        ...
        $connection->expects($this->once())->method('update')->with(
            $tableName,
            [
                'status' => \Magento\RedisMq\Model\QueueManagement::MESSAGE_STATUS_RETRY_REQUIRED,
