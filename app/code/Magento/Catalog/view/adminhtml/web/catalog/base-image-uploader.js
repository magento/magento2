  ...
  $.widget('mage.baseImage', {
    ...
    _create: function() {
      var $container = this.element,
        imageTmpl = mageTemplate(
          this.element.find('[data-template=image]').html(),
        ),
        $dropPlaceholder = this.element.find('.image-placeholder'),
        $galleryContainer = $('#media_gallery_content'),
        mainClass = 'base-image',
        maximumImageCount = 5,
        $fieldCheckBox = $container
          .closest('[data-attribute-code=image]')
          .find(':checkbox'),
      ...
      findElement = function(data) {
        return $container
          .find('.image:not(.image-placeholder)')
          .filter(function() {
            if (!$(this).data('image')) {
              return false;
            }
      ...
      updateVisibility = function() {
        ...
        $dropPlaceholder[
          elementsList.length > maximumImageCount ? 'hide' : 'show'
        ]();
      };
              ...
              .notification('add', {
                ...
                message: $.mage.__(
                  "You can't upload more than " +
                    this.options.maxImageUploadCount +
                    ' images in one time',
                ),
