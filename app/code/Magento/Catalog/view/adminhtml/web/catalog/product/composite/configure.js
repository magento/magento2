...
define([
  ...
  ProductConfigure.prototype = {
    ...
    _initWindowElements: function() {
      ...
      this.iFrameJSVarname = this.blockForm.select(
        'input[name="as_js_varname"]',
      )[0].value;
    },
    ...
    addItemsFilter: function(listType, itemsFilter) {
      ...
      this.itemsFilter[listType] = this.itemsFilter[listType].concat(
        itemsFilter,
      );
    ...
    onLoadIFrame: function() {
      this.blockFormConfirmed
        .select('[configure_disabled=1]')
        .each(function(element) {
          element.disabled =
            element.getAttribute('configure_prev_disabled') == '1';
        });
      ...
      if (response && 'object' == typeof response) {
        ...
        if (
          Object.isFunction(this.onLoadIFrameCallback[this.current.listType])
        ) {
    ...
    _getIFrameContent: function() {
      var content =
        this.blockIFrame.contentWindow || this.blockIFrame.contentDocument;
    ...
    clean: function(method) {
      ...
      var removeConfirmed = function(listTypes) {
        this.blockConfirmed.childElements().each(
          function(elm) {
    ...
    _processFieldsData: function(method) {
      ...
      switch (method) {
        case 'item_confirm':
          ...
          this.blockFormFields.childElements().each(
            function(elm) {
        ...
        case 'item_restore':
          ...
          getConfirmedValues(
            $(this.confirmedCurrentId).getElementsByTagName('input'),
          );
          getConfirmedValues(
            $(this.confirmedCurrentId).getElementsByTagName('select'),
          );
          getConfirmedValues(
            $(this.confirmedCurrentId).getElementsByTagName('textarea'),
          );
          ...
          restoreConfirmedValues(
            this.blockFormFields.getElementsByTagName('input'),
          );
          restoreConfirmedValues(
            this.blockFormFields.getElementsByTagName('select'),
          );
          restoreConfirmedValues(
            this.blockFormFields.getElementsByTagName('textarea'),
          );
            ...
            function(blockItem) {
              ...
              if (
                ...
                _renameFields(
                  method,
                  blockItem,
                  listInfo.complexTypes ? listType : null,
                );
        ...
        case 'form_confirmed_to_confirmed':
          ...
          this.blockFormConfirmed.childElements().each(
            function(blockItem) {
              var scopeArr = blockItem.id.match(/.*\[(\w+)\]\[([^\]]+)\]$/);
              var listType = scopeArr[1];
    ...
    changeOptionQty: function(element, event) {
      ...
      if (
        checkQty &&
        (Number(element.value) <= 0 || isNaN(Number(element.value)))
      ) {
        element.value = 1;
      }
    },
