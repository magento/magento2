  ...
  return function(config) {
    var optionPanel = jQuery('#manage-options-panel'),
      ...
      attributeOption = {
        ...
        renderWithDelay: function(data, from, step, delay) {
          ...
          setTimeout(
            this.renderWithDelay.bind(this, data, from, step, delay),
            delay,
          );
        },
    ...
    if (config.isSortable) {
      jQuery(function($) {
        $('[data-role=options-container]').sortable({
          ...
          axis: 'y',
          update: function() {
            $('[data-role=options-container] [data-role=order]').each(function(
              index,
              element,
            ) {
              $(element).val(index + 1);
    ...
    editForm.on('beforeSubmit', function() {
      ...
      if (optionPanel.hasClass(activePanelClass)) {
        optionsValues = jQuery.map(optionContainer.find('tr'), function(row) {
          return jQuery(row)
