  ...
  $.widget('mage.customOptions', {
      ...
      $.validator.addMethod(
        ...
        function(value, element) {
          var optionContainerElm = element.up('div[id*=_type_]'),
            ...
            selectTypeElements = $(
              '#' + optionContainerElm.id + ' .select-type-title',
            );
    ...
    _initOptionBoxes: function() {
      ...
      syncOptionTitle = function(event) {
        ...
        optionBoxTitle.text(
          currentValue === '' ? newOptionTitle : currentValue,
        );
      };
      this._on({
        /**
        ...
        'click button[id^=product_option_][id$=_delete]': function(event) {
          var element = $(event.target).closest(
            '#product_options_container_top > div.fieldset-wrapper,tr',
          );

          if (element.length) {
            $(
              '#product_' +
                element.attr('id').replace('product_', '') +
                '_is_delete',
            ).val(1);
         ...
         */
        'click button[id^=product_option_][id$=_add_select_row]': function(
          event,
        ) {
        ...
        'click #import_new_defined_option': function() {
          ...
          importContainer.modal({
            ...
            opened: function() {
              $(document)
                .off()
                .on('click', '#productGrid_massaction-form button', function() {
                  $('.import-custom-options-apply-button').trigger(
                    'click',
                    'massActionTrigger',
                  );
                });
            ...
            buttons: [
              {
                ...
                class:
                  'action-primary action-import import-custom-options-apply-button',
                ...
                click: function(event, massActionTrigger) {
                  ...
                  $.post(
                    widget.options.customOptionsUrl,
                    {
                      'products[]': request,
        ...
        'change select[id^=product_option_][id$=_type]': function(event, data) {
          ...
          if (previousGroup !== group) {
            ...
            if (disabledBlock.length) {
              ...
              tmpl = widget.element
                .find('#custom-option-' + group + '-type-template')
                .html();
              ...
              if (data['price_type']) {
                ...
                priceType
                  .val(data['price_type'])
                  .attr('data-store-label', data['price_type']);
              }
              this._bindUseDefault(
                widget.options.fieldId + '_' + data['option_id'],
                data,
              );
              //Add selections
    ...
    _initSortableSelections: function() {
      if (!this.options.isReadonly) {
        this.element
          .find('[id^=product_option_][id$=_type_select] tbody')
          .sortable({
            axis: 'y',
            handle: '[data-role=draggable-handle]',
    ...
    _bindCheckboxHandlers: function() {
      this._on({
        ...
        'change [id^=product_option_][id$=_required]': function(event) {
          ...
          $this
            .closest('#product_options_container_top > div')
            .find('[name$="[is_require]"]')
            .val($this.is(':checked') ? 1 : 0);
        },
      });
      this.element
        .find('[id^=product_option_][id$=_required]')
        .each(function() {
          $(this).prop(
            'checked',
            $(this)
    ...
    _updateSelectionsPositions: function() {
      $(this)
        .find('tr:not(.ignore-validate) [name$="[sort_order]"]')
        .each(function(index) {
          $(this).val(index);
        });
    },
    ...
    addSelection: function(event) {
      ...
      if (typeof element !== 'undefined') {
        data.id = $(element)
          .closest('#product_options_container_top > div')
          .find('[name^="product[options]"][name$="[id]"]')
          .val();
      ...
      this._bindUseDefault(
        this.options.fieldId + '_' + data.id + '_select_' + data['select_id'],
        data,
      );
      this.refreshSortableElements();
      this.options.selectionItemCount[data.id] =
        parseInt(this.options.selectionItemCount[data.id], 10) + 1;
    ...
    addOption: function(event) {
      ...
      if (data.type) {
        $('#' + this.options.fieldId + '_' + data.id + '_type')
          .val(data.type)
          .trigger('change', data);
      }
      ...
      if (data['is_require']) {
        $('#' + this.options.fieldId + '_' + data.id + '_is_require')
          .val(data['is_require'])
          .trigger('change');
      }
      ...
      $('#' + this.options.fieldId + '_' + data.id + '_title').trigger(
        'change',
      );
    },
    ...
    getFreeOptionId: function(id) {
      return $('#' + this.options.fieldId + '_' + id).length
        ? this.getFreeOptionId(parseInt(id, 10) + 1)
        : id;
    },
  });
});
