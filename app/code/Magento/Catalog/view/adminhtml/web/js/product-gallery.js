...
define([
  ...
  $.widget('mage.productGallery', {
    ...
    _create: function() {
      ...
      this.options.parentComponent =
        this.options.parentComponent || this.element.data('parent-component');

      this.imgTmpl = mageTemplate(
        this.element
          .find(this.options.template)
          .html()
          .trim(),
      );
      ...
      $.each(
        this.options.images,
        $.proxy(function(index, imageData) {
    ...
    _bind: function() {
      this._on({
        ...
        'mouseup [data-role=delete-button]': function(event) {
          ...
          $imageContainer = $(event.currentTarget).closest(
    ...
    _contentUpdated: function() {
      if (this.options.initialized && this.options.parentComponent) {
        registry.async(this.options.parentComponent)(function(parentComponent) {
    ...
    _addItem: function(event, imageData) {
      ...
      if (count === 0) {
        ...
        element.insertAfter(
          this.element.find(this.options.imageSelector + ':last'),
        );
      }

      if (
        (!this.options.initialized && this.options.images.length === 0) ||
        (this.options.initialized &&
          this.element.find(this.options.imageSelector + ':not(.removed)')
            .length === 1)
      ) {
        this.setBase(imageData);
      }
    ...
    _updateImagesRoles: function() {
      ...
      $images.forEach(function(img) {
        ...
        $img.find('[data-role=roles-labels] li').each(function(index, elem) {
          ...
          role.value === data.file ? $elem.show() : $elem.hide();
        });
      });
    },
    ...
    _updateImageTitle: function(event, data) {
      ...
      value =
        imageData['media_type'] === 'external-video'
  ...
  $.widget('mage.productGallery', $.mage.productGallery, {
    ...
    _initDialog: function() {
      ...
      $dialog.on(
        ...
        function(e) {
          ...
          this.element
            .find('input[type="hidden"][name="' + targetName + '"]')
            .val(desc);
    ...
    onDialogOpen: function(event) {
      var imageData = this.$dialog.data('imageData'),
        ...
        sizeSpan = this.$dialog
          .find(this.options.imageSizeLabel)
          .find('[data-message]'),
        resolutionSpan = this.$dialog
          .find(this.options.imageResolutionLabel)
          .find('[data-message]'),
        ...
        .each(
          $.proxy(function(index, checkbox) {
            var $checkbox = $(checkbox),
              ...
              selectedClass = 'selected',
              isChecked =
                this.options.types[$checkbox.val()].value == imageData.file; //eslint-disable-line
    ...
    _notifyType: function(event) {
      var $checkbox = $(event.currentTarget),
        $imageContainer = $checkbox
          .closest('[data-role=dialog]')
          .data('imageContainer');

      this.element.trigger('setImageType', {
        type: $checkbox.val(),
        imageData: $checkbox.is(':checked')
