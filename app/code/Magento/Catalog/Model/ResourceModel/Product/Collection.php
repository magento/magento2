...
class Collection extends \Magento\Catalog\Model\ResourceModel\Collection\AbstractCollection
{
    ...
    {
        $storeId = $this->_storeManager->getStore($this->getStoreId())->getId();
        $productIds = [];
        ...
        if ($this->_urlRewriteCategory) {
            $selectWithCategory = clone $select;
            $selectWithCategory->joinInner(
            ...
            foreach ($this->getConnection()->fetchAll($selectWithCategory) as $row) {
            ...
            $selectCat = $this->getConnection()
                ->select()
                ->from(['u' => $this->getTable('url_rewrite')], ['u.request_path'])
                ->where('u.store_id = ?', $storeId)
                ->where('u.is_autogenerated = 1')
                ->where(
                    'u.entity_type = ?',
                    \Magento\CatalogUrlRewrite\Model\CategoryUrlRewriteGenerator::ENTITY_TYPE)
                ->where('u.entity_id in (?) ', $this->_urlRewriteCategory);
            $categoryPath = $this->getConnection()->fetchOne($selectCat);
        }
        $select->joinLeft(
            ['cu' => $this->getTable('catalog_url_rewrite_product_category')],
            'u.url_rewrite_id=cu.url_rewrite_id'
        ...
        foreach ($this->getConnection()->fetchAll($select) as $row) {
            if (!isset($urlRewrites[$row['entity_id']])) {
                $requestPath = empty($categoryPath)
                    ? $row['request_path']
                    : str_replace('.html', '/' . $row['request_path'], $categoryPath);
                $urlRewrites[$row['entity_id']] = $requestPath;
            }
        }
