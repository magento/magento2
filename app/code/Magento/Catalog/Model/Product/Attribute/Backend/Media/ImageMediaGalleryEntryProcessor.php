...
class ImageMediaGalleryEntryProcessor extends AbstractMediaGalleryEntryProcessor
{
    ...
    {
        if ($product->getIsDuplicate() == true) {
            $attribute->getBackendModel()->duplicate($product);
            return;
        }
        ...
        $images = $this->productFactory->create()->getAssignedImages($product, $storeIds);
        ...
        foreach ($value['images'] as &$image) {
            ...
            $this->resourceEntryMediaGallery->insertGalleryValueInStore($data);
        }

        $this->resourceEntryMediaGallery->deleteGallery($recordsToDelete);
