...
class Full extends \Magento\Catalog\Model\Indexer\Category\Flat\AbstractAction
{
    ...
    protected function populateFlatTables(array $stores)
    {
        ...
        foreach ($stores as $store) {
            if (!isset($categories[$store->getRootCategoryId()])) {
                ...
                $categories[$store->getRootCategoryId()] = $this->getConnection()->fetchAll($select);
    ...
    protected function createTable($store)
    {
        ...
        $this->getConnection()->dropTable($temporaryTable);
    ...
    protected function switchTables(array $stores = [])
    {
        ...
        foreach ($stores as $store) {
            ...
            //switch tables
            $tablesToRename = [];
            if ($this->getConnection()->isTableExists($activeTableName)) {
            ...
            foreach ($tablesToRename as $tableToRename) {
                $this->getConnection()->renameTable($tableToRename['oldName'], $tableToRename['newName']);
            }

            //delete inactive table
            $tableToDelete = $oldTableName;

            if ($this->getConnection()->isTableExists($tableToDelete)) {
                $this->getConnection()->dropTable($tableToDelete);
            }
        }
