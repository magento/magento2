...
class StorageTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->urlRewrite->expects($this->any())->method('getMetadata')->willReturn(['category_id' => '5']);
        $this->urlRewrite->expects($this->once())->method('getEntityTYpe')->willReturn('product');
        $this->urlRewrite->expects($this->once())->method('getIsAutogenerated')->willReturn(1);
        $this->urlRewrite->expects($this->once())->method('getUrlRewriteId')->willReturn('4');
        $this->urlRewrite->expects($this->once())->method('getEntityId')->willReturn('2');
        $this->urlRewrite->setData('request_path', 'test');
        $this->urlRewrite->setData('store_id', '1');
        $productUrls = ['targetPath' => $this->urlRewrite];

        $this->urlFinder->expects($this->once())->method('findAllByData')->willReturn([$this->urlRewrite]);

        $this->productResourceModel->expects($this->once())->method('saveMultiple')->willReturnSelf();
