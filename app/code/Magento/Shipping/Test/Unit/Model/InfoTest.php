...
/**
 * Class for \Magento\Shipping\Model\Info tests
 */
class InfoTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var \Magento\Shipping\Model\ResourceModel\Order\Track\CollectionFactory|\PHPUnit_Framework_MockObject_MockObject
     */
    ...
    protected function setUp()
    {
        ...
        $this->trackFactory = $this->getMockBuilder(\Magento\Shipping\Model\Order\TrackFactory::class)
            ...
            ->setMethods(['create'])
            ->getMock();
        $this->trackCollectionFactory = $this->getMockBuilder(
            \Magento\Shipping\Model\ResourceModel\Order\Track\CollectionFactory::class
        )->disableOriginalConstructor()->setMethods(['create'])->getMock();
        $objectManagerHelper = new ObjectManagerHelper($this);
        $this->info = $objectManagerHelper->getObject(Info::class,
            [
    ...
    public function testLoadByHashWithOrderId()
    {
        ...
        $this->helper->expects($this->atLeastOnce())->method('decodeTrackingHash')->with($hash)
        ...
        $trackCollection = $this->getMockBuilder(\Magento\Shipping\Model\ResourceModel\Order\Track\Collection::class)
            ...
            ->getMock();
        $trackCollection->expects($this->atLeastOnce())->method('setShipmentFilter')->with($shipmentId)
            ->willReturnSelf();
        $trackCollection->expects($this->atLeastOnce())->method('getIterator')
    ...
    public function testLoadByHashWithShipmentId()
    {
        ...
        $this->shipmentRepository->expects($this->atLeastOnce())->method('get')->with($decodedHash['id'])
            ->willReturn($shipment);
        $track = $this->getMockBuilder(\Magento\Sales\Model\Order\Shipment\Track::class)
            ->disableOriginalConstructor()
            ->setMethods(['setShipment', 'getNumberDetail'])
        ...
        $trackCollection = $this->getMockBuilder(\Magento\Shipping\Model\ResourceModel\Order\Track\Collection::class)
            ...
            ->setMethods(['getIterator', 'setShipmentFilter'])
            ->getMock();
        $trackCollection->expects($this->atLeastOnce())->method('setShipmentFilter')->with($decodedHash['id'])
            ->willReturnSelf();
        $trackCollection->expects($this->atLeastOnce())->method('getIterator')
            ->willReturn(new \ArrayIterator([$track]));
        $this->trackCollectionFactory->expects($this->atLeastOnce())->method('create')->willReturn($trackCollection);
