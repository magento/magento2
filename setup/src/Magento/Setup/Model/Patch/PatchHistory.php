...
class PatchHistory
{
    ...
    public function fixPatch(PatchInterface $patch)
    {
        ...
        $adapter = $this->resourceConnection->getConnection();
        $adapter->insert($adapter->getTableName(self::TABLE_NAME), [self::CLASS_NAME => $patchName]);
    }
    ...
    public function revertPatchFromHistory(PatchInterface $patch)
    {
        ...
        $adapter = $this->resourceConnection->getConnection();
        $adapter->delete($adapter->getTableName(self::TABLE_NAME), [self::CLASS_NAME . "= ?" => $patchName]);
    }
