...
use Magento\Setup\Console\Command\Module\DisableCommand;
use Magento\Setup\Console\Command\Module\EnableCommand;
use Symfony\Component\Console\Tester\CommandTester;

class EnableDisableCommandTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    public function testExecute($isEnable, $clearStaticContent, $expectedMessage)
    {
        ...
        $commandTester = $isEnable
            ? new CommandTester(new EnableCommand($this->objectManagerProvider))
            : new CommandTester(new DisableCommand($this->objectManagerProvider));
    ...
    public function testExecuteEnableInvalidModule()
    {
        ...
        $commandTester = new CommandTester(new EnableCommand($this->objectManagerProvider));
    ...
    public function testExecuteDisableInvalidModule()
    {
        $this->status->expects($this->once())
            ...
            ->with(false, ['invalid'])
            ->willThrowException(new \LogicException('Unknown module(s): invalid'));
        $commandTester = new CommandTester(new DisableCommand($this->objectManagerProvider));
        $input = ['module' => ['invalid']];
        $commandTester->execute($input);
        $this->assertEquals('Unknown module(s): invalid' . PHP_EOL, $commandTester->getDisplay());
    }
