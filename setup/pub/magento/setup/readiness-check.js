...
'use strict';
angular
  .module('readiness-check', ['remove-dialog'])
  .constant('COUNTER', 1)
  .controller('readinessCheckController', [
    ...
    function(
      ...
      switch ($state.current.type) {
        case 'uninstall':
          $scope.dependencyUrl =
            'index.php/dependency-check/uninstall-dependency-check';
        ...
        case 'enable':
        case 'disable':
          $scope.componentDependency.enabled =
            $localStorage.packages[0].isComposerPackage;
          $scope.dependencyUrl =
            'index.php/dependency-check/enable-disable-dependency-check';
          if ($localStorage.packages) {
        ...
        default:
          $scope.dependencyUrl =
            'index.php/dependency-check/component-dependency';
          if ($localStorage.packages) {
            $scope.componentDependency.packages = $localStorage.packages;
          }
      }
        ...
        'php-extensions': {
          ...
          process: function(data) {
            ...
            if (data.responseType !== 'error') {
              $scope.extensions.length = Object.keys(
                $scope.extensions.data.required,
              ).length;
            }
        ...
        $scope.items['cron-script'] = {
          ...
          process: function(data) {
            ...
            if (data.updaterErrorMessage) {
              data.updaterErrorMessage = $sce.trustAsHtml(
                data.updaterErrorMessage,
              );
            }
            if (data.setupNoticeMessage) {
              $scope.cronScript.notice = true;
              data.setupNoticeMessage = $sce.trustAsHtml(
                data.setupNoticeMessage,
              );
            }
            if (data.updaterNoticeMessage) {
              $scope.cronScript.notice = true;
              data.updaterNoticeMessage = $sce.trustAsHtml(
                data.updaterNoticeMessage,
              );
            }
      ...
      $scope.isCompleted = function() {
        var cronProcessed =
          ...
          ($scope.componentDependency.processed ||
            !$scope.componentDependency.enabled) &&
      ...
      $scope.query = function(item) {
        if (item.params) {
          if (item.useGet === true) {
            ...
            return $http.post(item.url, item.params).then(
        ...
        return $http.get(item.url, { timeout: 60000 }).then(
          function successCallback(resp) {
            item.process(resp.data);
          },
      ...
      $scope.$on('$stateChangeSuccess', function(event, nextState) {
        if (
          nextState.id ==
          'root.readiness-check-' + nextState.type + '.progress'
        ) {
          $scope.progress();
        }
      });
      ...
      $scope.wordingOfReadinessCheckAction = function() {
        var $actionString =
          "We're making sure your server environment is ready for ";
        if ($localStorage.moduleName) {
          $actionString += $localStorage.packageTitle
            ? $localStorage.packageTitle
            : $localStorage.moduleName;
        } else {
          if (
            $state.current.type === 'install' ||
            $state.current.type === 'upgrade'
          ) {
            $actionString += 'Magento';
            if (
              $state.current.type === 'upgrade' &&
              $localStorage.packages.length > 1
            ) {
        ...
        if (
          ...
          $actionString +=
            ', which consists of the following packages:<br/>- ' +
            $localStorage.moduleName;
        }
      ...
      $scope.getExtensionsList = function() {
        return $scope.componentDependency.packages
          ? $scope.componentDependency.packages
          : {};
      };
      ...
      $scope.getExtensionInfo = function(name) {
        ...
        return extensionsVersions.hasOwnProperty(name)
          ? extensionsVersions[name]
          : {};
      };
