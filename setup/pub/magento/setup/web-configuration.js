...
'use strict';
angular
  .module('web-configuration', ['ngStorage'])
  .controller('webConfigurationController', [
    ...
    function($scope, $state, $localStorage, $http) {
      ...
      $scope.$watch('config.address.base_url', function() {
        if (angular.equals($scope.config.address.base_url, '')) {
          $scope.config.address.actual_base_url =
            $scope.config.address.auto_base_url;
        } else {
          $scope.config.address.actual_base_url =
            $scope.config.address.base_url;
        }
      });
      ...
      $scope.$watch('config.address.base_url', function() {
        if (
          angular.equals($scope.config.https.text, '') ||
          angular.isUndefined($scope.config.https.text)
        ) {
          $scope.config.https.text = $scope.config.address.base_url.replace(
            'http://',
            'https://',
          );
        }
      ...
      $scope.validateUrl = function() {
        if (!$scope.webconfig.submitted) {
          $http.post('index.php/url-check', $scope.config).then(
            function successCallback(resp) {
              $scope.validateUrl.result = resp.data;

              if (
                $scope.validateUrl.result.successUrl &&
                $scope.validateUrl.result.successSecureUrl
              ) {
                $scope.nextState();
              }
