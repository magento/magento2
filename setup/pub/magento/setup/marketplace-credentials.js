...
'use strict';
angular
  .module('marketplace-credentials', ['ngStorage'])
  .controller('MarketplaceCredentialsController', [
    ...
    function(
      ...
      $scope.actionMessage =
        $state.current.type == 'upgrade' ? 'upgrade' : 'upgrade or install';
      $scope.errors = false;
      ...
      if ($state.current.type == 'upgrade') {
        $scope.isHiddenSpinner = false;
        $http
          .get('index.php/select-version/installedSystemPackage', {
            responseType: 'json',
          })
          .then(
            function successCallback(resp) {
              ...
              if (data.responseType == 'error') {
                ...
                $scope.upgradeProcessErrorMessage = $sce.trustAsHtml(
                  data.error,
                );
              } else {
                $scope.checkAuth();
