...
'use strict';
angular
  .module('customize-your-store', ['ngStorage', 'ngSanitize'])
  .controller('customizeYourStoreController', [
    ...
    '$http',
    '$sce',
    function($scope, $localStorage, $state, $http, $sce) {
      $scope.store = {
        timezone: 'UTC',
        currency: 'USD',
        language: 'en_US',
      ...
      if (!$localStorage.store) {
        $http
          .get('index.php/customize-your-store/default-time-zone', {
            responseType: 'json',
          })
          .then(
      ...
      $scope.checkModuleConstraints = function() {
        ...
        $scope.loading = true;
        $http
          .post('index.php/modules/all-modules-valid', $scope.store)
          .then(function successCallback(resp) {
            ...
            if (
              ...
              $scope.store.errorMessage = $sce.trustAsHtml(
                $scope.checkModuleConstraints.result.error,
              );
              $scope.loading = false;
            }
          });
      ...
      $scope.validateModules = function(module) {
        ...
        var allParameters = {
          allModules: $scope.store.allModules,
          selectedModules: $scope.store.selectedModules,
          module: module,
          status: moduleStatus,
        };

        $http
          .post('index.php/modules/validate', allParameters)
          .then(function successCallback(resp) {
            $scope.checkModuleConstraints.result = resp.data;

            if (
              $scope.checkModuleConstraints.result.error !== undefined &&
              !$scope.checkModuleConstraints.result.success
            ) {
              $scope.store.errorMessage = $sce.trustAsHtml(
                $scope.checkModuleConstraints.result.error,
      ...
      $scope.toggleAllModules = function() {
        ...
        if ($scope.store.selectAll) {
          ...
          for (var i = 0; i < $scope.store.allModules.length; i++) {
            var idx = $scope.store.selectedModules.indexOf(
              $scope.store.allModules[i],
            );
            if (
              $scope.store.disabledModules.indexOf($scope.store.allModules[i]) <
              0
            ) {
              $scope.store.selectedModules.splice(idx, 1);
            }
      ...
      $scope.checkIfAllAreSelected = function() {
        if (
          $scope.store.selectedModules.length ===
            $scope.store.allModules.length &&
      ...
      $scope.$watch(
        function() {
          return $scope.customizeStore.$valid;
        },
        function(valid) {
          if (valid) {
