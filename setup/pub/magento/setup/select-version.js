...
'use strict';
angular
  .module('select-version', ['ngStorage'])
  .controller('selectVersionController', [
    ...
    function($scope, $http, $localStorage, $sce) {
      ...
      $scope.tryAgainEnabled = function() {
        return (
          ($scope.upgradeProcessed || $scope.upgradeProcessError) &&
          ($scope.updateComponents.no ||
            ($scope.updateComponents.yes &&
              ($scope.componentsProcessed || $scope.componentsProcessError)))
        );
      };

      $http
        .get('index.php/select-version/systemPackage', { responseType: 'json' })
        .then(
      ...
      $scope.$watch('updateComponents.yes', function() {
        if (angular.equals($scope.updateComponents.yes, true)) {
          ...
          if (!$scope.componentsProcessed && !$scope.componentsProcessError) {
            ...
            $http
              .get('index.php/other-components-grid/components', {
                responseType: 'json',
              })
              .then(
                function successCallback(resp) {
