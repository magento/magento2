...
'use strict';
angular
  .module('complete-backup', ['ngStorage'])
  ...
  .controller('completeBackupController', [
    ...
    function(
      ...
      $scope.query = function(item) {
        if (!$rootScope.hasErrors) {
          return $http
            .post(item.url, $scope.backupInfoPassed, { timeout: 3000000 })
            .then(
      ...
      $scope.$on('$stateChangeSuccess', function(event, nextState) {
        if (
          nextState.id ==
          'root.create-backup-' + nextState.type + '.progress'
        ) {
          $scope.progress();
        }
      });
