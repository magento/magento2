...
'use strict';
angular
  .module('create-admin-account', ['ngStorage'])
  .controller('createAdminAccountController', [
    ...
    function($scope, $state, $localStorage, $http) {
      ...
      $scope.passwordStatusChange = function() {
        ...
        if (
          p.length >= MIN_ADMIN_PASSWORD_LENGTH &&
          p.match(/[\d]+/) &&
          p.match(/[a-z]+/) &&
          p.match(/[A-Z]+/) &&
          p.match(/[!@#$%^*()_\/\\\-\+=]+/)
      ...
      $scope.validateCredentials = function() {
        ...
        $scope.validate();
        if ($scope.valid) {
          $http.post('index.php/validate-admin-credentials', data).then(
    ...
    return {
      ...
      link: function(scope, elm, attrs, ctrl) {
        var validator = function(value) {
          ...
          ctrl.$setValidity(
            'checkUserNamePasswordDifferent',
            password !== userName,
          );
          return value;
        };
