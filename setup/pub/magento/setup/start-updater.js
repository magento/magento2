...
'use strict';
angular
  .module('start-updater', ['ngStorage'])
  .controller('startUpdaterController', [
    ...
    '$http',
    '$window',
    function($scope, $state, $localStorage, $http, $window) {
      ...
      $scope.update = function() {
        ...
        var payLoad = {
          ...
          headerTitle:
            $scope.packages.size == 1 ? $scope.title : 'Process extensions',
          dataOption: $localStorage.dataOption,
        };
        $http.post('index.php/start-updater/update', payLoad).then(
