...
angular
  .module('extension-grid', ['ngStorage'])
  .controller('extensionGridController', [
    ...
    function(
      ...
      $http
        .get('index.php/extensionGrid/extensions')
        .then(function successCallback(resp) {
          ...
          if (typeof data.lastSyncData.lastSyncDate === 'undefined') {
            ...
            $scope.enabledInstall = data.lastSyncData.countOfInstall
              ? true
              : false;
      ...
      $scope.toggleActiveActionsCell = function(extension) {
        $scope.selectedExtension =
          $scope.selectedExtension == extension ? null : extension;
      };
      ...
      $scope.sync = function() {
        ...
        $http
          .get('index.php/extensionGrid/sync')
          .then(function successCallback(resp) {
            var data = resp.data;
      ...
      $scope.isAvailableUpdatePackage = function(packageName) {
        $localStorage.isMarketplaceAuthorized =
          typeof $localStorage.isMarketplaceAuthorized !== 'undefined'
            ? $localStorage.isMarketplaceAuthorized
            : false;
        var isAvailable =
          typeof $scope.availableUpdatePackages !== 'undefined' &&
          $localStorage.isMarketplaceAuthorized &&
        ...
        $localStorage.packages = [
          {
            name: extension.name,
            version:
              $scope.availableUpdatePackages[extension.name]['latestVersion'],
          },
        ];
