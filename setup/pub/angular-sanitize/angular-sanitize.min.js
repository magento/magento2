...
(function(s, d) {
      ...
      function I(a) {
        for (; a; ) {
          if (a.nodeType === s.Node.ELEMENT_NODE)
            for (var e = a.attributes, c = 0, b = e.length; c < b; c++) {
              var f = e[c],
                h = f.name.toLowerCase();
              if ('xmlns:ns1' === h || 0 === h.lastIndexOf('ns1:', 0))
                a.removeAttributeNode(f), c--, b--;
            }
          (e = a.firstChild) && I(e);
          a = t('nextSibling', a);
        }
      ...
      var L = /[\uD800-\uDBFF][\uDC00-\uDFFF]/g,
        ...
        u = (function(a, e) {
          ...
          if (e && e.implementation)
            h = e.implementation.createHTMLDocument('inert');
          else throw x('noinert');
          var d = (h.documentElement || h.getDocumentElement()).querySelector(
            'body',
          );
  ...
  d.module('ngSanitize').filter('linky', [
    '$sanitize',
    function(g) {
      ...
      return function(d, q, l) {
        ...
        for (
          ...
          (v = d[0]),
            d[2] || d[4] || (v = (d[3] ? 'http://' : 'mailto:') + v),
            (u = d.index),
            r(n.substr(0, u)),
            z(v, d[0].replace(p, '')),
            (n = n.substring(u + d[0].length));
        r(n);
        return g(m.join(''));
      };
    },
