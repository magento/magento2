...
define(['jquery', 'mage/backend/suggest'], function($) {
  'use strict';
  ...
  describe('mage/backend/suggest', function() {
    ...
    it('Check suggest create', function() {
      ...
      suggestInstance = $(suggestSelector)
        .suggest(options)
        .data('mage-suggest');
      ...
      expect(suggestInstance._control).toEqual(
        suggestInstance.options.controls,
      );
      expect(suggestInstance._recentItems).toEqual(
        window.localStorage ? recentItems : [],
      );
    ...
    it('Check suggest render', function() {
      ...
      expect(
        suggestInstance.element.closest('.test-input-wrapper').length,
      ).toBeGreaterThan(0);
      expect(
        suggestInstance.element.closest('.' + options.className).length,
      ).toBeGreaterThan(0);
      ...
      suggestInstance = $(suggestSelector)
        ...
        .data('mage-suggest');
      suggestInstance._render();
      expect(suggestInstance.element.prev().is(suggestInstance.dropdown)).toBe(
        true,
      );
      ...
      suggestInstance = $(suggestSelector)
        ...
        .data('mage-suggest');
      suggestInstance._render();
      expect(suggestInstance.element.next().is(suggestInstance.dropdown)).toBe(
        true,
      );
    });

    it('Check suggest createValueField', function() {
      var suggestInstance = $(suggestSelector)
          .suggest({
            template: '#test-template',
            choiceTemplate: '<li/>',
          })
    ...
    it('Check suggest prepareValueField', function() {
      ...
      expect(
        suggestInstance.element.prev().is(suggestInstance.valueField),
      ).toBe(true);
    ...
    it('Check suggest destroy', function() {
      ...
      expect($suggest.attr('name')).toBe(
        suggestInstance.valueField.attr('name'),
      );
      expect(
        suggestInstance.valueField.parents('html').length,
      ).not.toBeGreaterThan(0);
      expect(
        suggestInstance.dropdown.parents('html').length,
      ).not.toBeGreaterThan(0);
    });
    ...
    it('Check suggest value', function() {
      ...
      suggestInstance = $(suggestSelector)
        ...
        .data('mage-suggest');
      suggestDivInstance = $('#suggest-div')
        .suggest({
          template: '#test-template',
          choiceTemplate: '<li/>',
        })
    ...
    it('Check suggest select', function() {
      ...
      expect(suggestInstance.valueField.val()).toBe(
        suggestInstance._focused.id.toString(),
      );
      expect(suggestInstance.dropdown.is(':hidden')).toBe(true);
    });
    ...
    it('Check suggest multiselect', function() {
      ...
      expect(suggestInstance._term).toBe(null);
      expect(
        suggestInstance.valueField.find('option').length,
      ).not.toBeGreaterThan(0);
      expect(suggestInstance.dropdown.is(':hidden')).toBe(true);
      ...
      expect(
        suggestInstance._getOption(suggestInstance._focused).length,
      ).toBeGreaterThan(0);
      expect(
        selectedElement.hasClass(suggestInstance.options.selectedClass),
      ).toBe(true);
      expect(suggestInstance.dropdown.is(':hidden')).toBe(true);

      suggestInstance._selectItem(event);
      expect(suggestInstance._selectedItem).toEqual(
        suggestInstance._nonSelectedItem,
      );
      expect(suggestInstance._term).toBe('');
      expect(
        suggestInstance._getOption(suggestInstance._focused).length,
    ...
    it('Check suggest reset value', function() {
      ...
      expect(suggestInstance.valueField.val()).toBe(
        suggestInstance._nonSelectedItem.id,
      );
    });
    ...
    it('Check suggest reset multiselect value', function() {
      ...
      expect(suggestInstance.valueField.val()[0]).toBe(
        uiHash.item.id.toString(),
      );
    });
    ...
    it('Check suggest read item data', function() {
      ...
      expect(suggestInstance._readItemData(testElement)).toEqual(
        suggestInstance._nonSelectedItem,
      );
    ...
    it('Check suggest add option', function() {
      ...
      option = suggestInstance.valueField.find(
        'option[value=' + uiHash.item.id + ']',
      );
    ...
    it('Check suggest get option', function() {
      ...
      expect(
        suggestInstance._getOption(uiHash.item).length,
      ).not.toBeGreaterThan(0);
    ...
    it('Check suggest last added', function() {
      ...
      expect(suggestInstance.valueField.find('option').length).toBeGreaterThan(
        0,
      );
    ...
    it('Check suggest remove option', function() {
      ...
      expect(selectTarget.hasClass(suggestInstance.options.selectedClass)).toBe(
        false,
      );
    });
  });
