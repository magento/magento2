...
define(['jquery', 'mage/backend/tree-suggest'], function($) {
  'use strict';
  ...
  describe('mage/backend/tree-suggest', function() {
    ...
    beforeEach(function() {
      var $treeSuggest = $(
        '<input name="test-tree-suggest" id="tree-suggest" />',
      );
    ...
    it('Check treeSuggest filter', function() {
      var treeSuggestInstance = $(treeSuggestSelector)
          .treeSuggest()
          .data('mage-treeSuggest'),
        uiHash = {
          item: {
            id: 1,
            label: 'Test Label',
      ...
      expect(
        treeSuggestInstance._filterSelected([uiHash.item], {
          _allShown: true,
        }),
      ).toEqual([uiHash.item]);
    });
