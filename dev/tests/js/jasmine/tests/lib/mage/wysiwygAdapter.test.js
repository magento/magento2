...
define(['wysiwygAdapter'], function(wysiwygAdapter) {
  'use strict';
  ...
  describe('wysiwygAdapter - encoding and decoding directives', function() {
    ...
    function runTests(decodedHtml, encodedHtml) {
      var encodedHtmlWithForwardSlashInImgSrc = encodedHtml.replace(
        /src="((?:(?!"|\\\?).)*)/,
        'src="$1/',
      );
      ...
      describe('"decodeDirectives" method', function() {
        ...
        it(
          'converts directive URL img src with a trailing forward slash ' +
            'to media url without a trailing forward slash',
          function() {
            expect(encodedHtmlWithForwardSlashInImgSrc).not.toEqual(
              encodedHtml,
            );
            expect(
              obj.decodeDirectives(encodedHtmlWithForwardSlashInImgSrc),
            ).toEqual(decodedHtml);
          },
        );
      });
    ...
    describe('without SID in directive query string with secret key', function() {
      var decodedHtml =
        ...
        encodedHtml =
          ...
          '</p>',
        directiveUrl =
          'http://example.com/admin/cms/wysiwyg/directive/key/' +
          '5552655d13a141099d27f5d5b0c58869423fd265687167da12cad2bb39aa9a58/';
    ...
    describe('with SID in directive query string without secret key', function() {
      var decodedHtml =
        ...
        encodedHtml =
          ...
          '</p>',
        directiveUrl =
          'http://example.com/admin/cms/wysiwyg/directive?SID=something';
