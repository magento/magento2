...
define(['jquery', 'mage/backend/form'], function($) {
  'use strict';
    ...
    describe('Test for mage/form jQuery plugin', function() {
      ...
      beforeEach(function() {
        var element = $(
          '<form id="' +
            id +
            '" action="action/url" method="GET" target="_self" ></form>',
        );
      ...
      it('check store attribute', function() {
        ...
        expect(form.data('form').oldAttributes.action).toBe(
          initialFormAttrs.action,
        );
        expect(form.data('form').oldAttributes.target).toBe(
          initialFormAttrs.target,
        );
        expect(form.data('form').oldAttributes.method).toBe(
          initialFormAttrs.method,
        );
      });
      ...
      it('check get action URL', function() {
        ...
        form.data('form')._storeAttribute('action');
        expect(form.data('form')._getActionUrl(testArgs)).toBe(
          action + '/arg/value/',
        );
      ...
      it('check process data', function() {
        ...
        form.data('form')._storeAttribute('action');
        expect(processedData.action).toBe(
          form.data('form')._getActionUrl(testActionArgsData.action),
        );
      });
      ...
      it('check submit', function() {
        ...
        expect(form.attr('action')).toBe(
          form.data('form').oldAttributes.action,
        );
        expect(form.attr('target')).toBe(
          form.data('form').oldAttributes.target,
        );
        expect(form.attr('method')).toBe(
          form.data('form').oldAttributes.method,
        );
      ...
      it('check build URL', function() {
        var dataProvider = [
            {
              ...
              expected:
                'http://domain.com/key[one]/value%201/key2/%23%20value/',
            },
            {
              params: [
                'http://domain.com',
            ...
            },
            {
              ...
              expected:
                'http://domain.com?some=param&key[one]=value%201&key2=%23%20value',
            },
            {
              params: [
                'http://domain.com?some=param&',
        ...
        for (i; i < quantity; i++) {
          expect(dataProvider[i].expected).toBe(
            method.apply(null, dataProvider[i].params),
          );
        }
      });
    });
