...
define(['jquery', 'mage/multiselect'], function($) {
  'use strict';
  ...
  describe('Test for mage/multiselect jQuery plugin', function() {
    ...
    it('multiselect2 loadOptions success case', function() {
      spyOn(instance.data('mage-multiselect2'), 'appendOptions').and.callFake(
        function() {
          return true;
        },
      );
      ...
      expect(
        instance.data('mage-multiselect2').appendOptions,
      ).toHaveBeenCalled();
    });
    ...
    it('multiselect2 loadOptions negative case', function() {
      ...
      spyOn(instance.data('mage-multiselect2'), 'onError').and.callFake(
        function() {
          return true;
        },
      );
      ...
      expect($.get).toHaveBeenCalled();
      expect(instance.data('mage-multiselect2').onError).toHaveBeenCalledWith(
        errorMessage,
      );
    });
    ...
    it('multiselect2 onKeyUp check', function() {
      spyOn(
        instance.data('mage-multiselect2'),
        'getSearchCriteria',
      ).and.returnValue('some_string');
    ...
    it('multiselect2 paging test', function() {
      ...
      spyOn(instance.data('mage-multiselect2'), 'setCurrentPage').and.callFake(
        function() {
          return true;
        },
      );
      ...
      expect(
        instance.data('mage-multiselect2').setCurrentPage,
      ).toHaveBeenCalledWith(2);
    });

    it('multiselect2 item click', function() {
      var option =
          '<div><label><input type="checkbox" value="1"/><span>Label</span></label></div>',
        checkbox;
      ...
      spyOn(
        instance.data('mage-multiselect2'),
        '_createSelectedOption',
      ).and.returnValue(true);

      checkbox.click();

      expect(
        instance.data('mage-multiselect2')._createSelectedOption,
