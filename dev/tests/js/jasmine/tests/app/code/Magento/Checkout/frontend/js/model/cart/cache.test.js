...
define(['squire', 'ko'], function(Squire, ko) {
  ...
  beforeEach(function(done) {
    injector.mock(mocks);
    injector.require(['Magento_Checkout/js/model/cart/cache'], function(
      Constr,
    ) {
      cache = Constr;
      done();
  ...
  describe('Magento_Checkout/js/model/cart/cache', function() {
    describe('Check the "get" method', function() {
      it('Check default call with "cart-data" key', function() {
        ...
        spyOn(
          mocks['Magento_Customer/js/customer-data'],
          'get',
        ).and.returnValue(ko.observable(expectedResult));

        expect(cache.get('cart-data')).toBe(expectedResult);
        expect(
          mocks['Magento_Customer/js/customer-data'].get,
        ).toHaveBeenCalledWith('cart-data');
      });
    ...
    describe('Check the "clear" method', function() {
      ...
      it('Check if the "clear" method resets the object to default value with the "cart-data" key received', function() {
        var expectedResult = JSON.stringify(cache.cartData),
        ...
        spyOn(
          mocks['Magento_Customer/js/customer-data'],
          'get',
        ).and.returnValue(storage);
        spyOn(mocks['Magento_Customer/js/customer-data'], 'set').and.callFake(
          function(key, value) {
            storage(value);
          },
        );
