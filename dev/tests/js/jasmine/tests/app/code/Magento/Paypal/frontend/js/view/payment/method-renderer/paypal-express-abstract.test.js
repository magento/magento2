...
define(['jquery', 'squire', 'ko', 'mage/translate'], function(
  $,
  Squire,
  ko,
  $t,
  ...
  describe('paypal/js/view/payment/method-renderer/paypal-express-abstract', function() {
    var injector = new Squire(),
      ...
      setPaymentMock = jasmine
        .createSpy('set-payment-information', function() {
          return successPromise;
        })
        .and.callThrough(),
      validateMock = jasmine
      ...
      tplElement = $(
        '<div data-bind="with: child"><div data-bind="template: getTemplate()"></div></div>',
      )[0];
    ...
    function clickOnHelpLink(expectation) {
      $('div.payment-method-title.field.choice > label > a > span').trigger(
        'click',
      );
      expectation();
    }
    ...
    it('setPaymentMethodAction is called before redirect to paypal', function() {
      ...
      expect(validateMock.calls.mostRecent()).toEqual(
        jasmine.objectContaining({
          object:
            mocks['Magento_Checkout/js/model/payment/additional-validators'],
          args: [],
          returnValue: true,
        }),
      );
      expect(setPaymentMock).toHaveBeenCalled();
      expect(setPaymentMock.calls.mostRecent()).toEqual(
        jasmine.objectContaining({
