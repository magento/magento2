...
define(['squire'], function(Squire) {
  ...
  var injector = new Squire(),
    mocks = {
      'Magento_Checkout/js/action/set-payment-information': jasmine.createSpy(
        'placeOrderAction',
      ),
      'Magento_CheckoutAgreements/js/model/agreements-assigner': jasmine.createSpy(
        'agreementsAssigner',
      ),
    },
  ...
  describe('Magento_CheckoutAgreements/js/model/set-payment-information-mixin', function() {
    it('mixin is applied to Magento_Checkout/js/action/set-payment-information', function() {
      var placeOrderMixins =
        defaultContext.config.config.mixins[
          'Magento_Checkout/js/action/set-payment-information'
        ];
    ...
    it('Magento_CheckoutAgreements/js/model/agreements-assigner is called', function() {
      ...
      expect(
        mocks['Magento_CheckoutAgreements/js/model/agreements-assigner'],
      ).toHaveBeenCalledWith(paymentData);
      expect(
        mocks['Magento_Checkout/js/action/set-payment-information'],
