...
define(['Magento_Catalog/js/product/remaining-characters', 'jquery'], function(
  remainingCharacters,
  $,
) {
  'use strict';
  ...
  describe('Magento_Catalog/js/product/remaining-characters', function() {
    ...
    beforeEach(function() {
      ...
      note = $(
        '<p class="note note_1"><span class="character-counter"></span></p>',
      );
      $('body')
        .append(widget)
        .append(note);
    ...
    describe('Note text is updated on input change', function() {
      it('check empty input', function() {
        ...
        expect(note.find('.character-counter').text()).toBe(
