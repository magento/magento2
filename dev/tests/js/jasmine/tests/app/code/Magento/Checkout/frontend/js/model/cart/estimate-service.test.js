...
define(['squire', 'ko'], function(Squire, ko) {
  ...
  describe('Magento_Checkout/js/model/cart/estimate-service', function() {
    it('test subscribe when billingAddress was changed for  virtual quote', function() {
      ...
      expect(
        mocks['Magento_Checkout/js/model/cart/totals-processor/default']
          .estimateTotals,
    ...
    it("test subscribe when shipping address wasn't changed for not virtual quote", function() {
      ...
      spyOn(
        mocks['Magento_Checkout/js/model/shipping-service'],
        'setShippingRates',
      );
      ...
      expect(
        mocks['Magento_Checkout/js/model/shipping-service'].setShippingRates,
      ).toHaveBeenCalledWith(rates);
      expect(
        mocks['Magento_Checkout/js/model/cart/totals-processor/default']
          .estimateTotals,
      ).not.toHaveBeenCalled();
      expect(
        mocks['Magento_Checkout/js/model/shipping-rate-processor/new-address']
          .getRates,
      ).not.toHaveBeenCalled();
    });
