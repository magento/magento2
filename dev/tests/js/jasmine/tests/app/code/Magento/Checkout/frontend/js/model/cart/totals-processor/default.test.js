...
define(['squire', 'ko', 'jquery'], function(Squire, ko, $) {
  ...
  var injector = new Squire(),
    ...
    mocks = {
      'Magento_Checkout/js/model/resource-url-manager': {
        getUrlForTotalsEstimationForNewAddress: jasmine
          .createSpy()
          .and.returnValue('http://example.com'),
      },
  ...
  describe('Magento_Checkout/js/model/cart/totals-processor/default', function() {
    it('estimateTotals if data was cached', function() {
      spyOn(
        mocks['Magento_Checkout/js/model/cart/cache'],
        'isChanged',
      ...
      spyOn(
        mocks['Magento_Checkout/js/model/cart/cache'],
        'get',
      ).and.returnValue(totals);
      spyOn(mocks['mage/storage'], 'post');
      expect(defaultProcessor.estimateTotals(address)).toBeUndefined();
      expect(
        mocks['Magento_Checkout/js/model/quote'].setTotals,
      ).toHaveBeenCalledWith(totals);
    ...
    it("estimateTotals if data wasn't cached and request was successfully sent", function() {
      ...
      spyOn(mocks['mage/storage'], 'post').and.callFake(function() {
        data.shippingMethodCode = mocks[
          'Magento_Checkout/js/model/quote'
        ].shippingMethod()['method_code'];
        data.shippingCarrierCode = mocks[
          'Magento_Checkout/js/model/quote'
      ...
      expect(
        mocks['Magento_Checkout/js/model/totals'].isLoading.calls.argsFor(0)[0],
      ).toBe(true);
      expect(
        mocks['Magento_Checkout/js/model/totals'].isLoading.calls.argsFor(1)[0],
      ).toBe(false);
      expect(mocks['mage/storage'].post).toHaveBeenCalled();
      expect(
        mocks['Magento_Checkout/js/model/cart/cache'].get,
      ).not.toHaveBeenCalled();
      expect(
        mocks['Magento_Checkout/js/model/cart/cache'].set,
      ).toHaveBeenCalledWith('cart-data', data);
    });
    ...
    it("estimateTotals if data wasn't cached and request returns error", function() {
      ...
      expect(
        mocks['Magento_Checkout/js/model/error-processor'].process,
