...
define(['squire', 'ko', 'jquery', 'uiComponent'], function(
  Squire,
  ko,
  $,
  Component,
  ...
  describe('paypal/js/view/payment/method-renderer/paypal-express-abstract', function() {
    ...
    describe('"click" method checks', function() {
      it('check success request', function() {
        mocks['Magento_Paypal/js/action/set-payment-method'].and.callFake(
          function() {
        ...
        expect(
          mocks.paypalInContextExpressCheckout.checkout.initXO,
        ).toHaveBeenCalled();
        expect(
          mocks.paypalInContextExpressCheckout.checkout.startFlow,
        ).toHaveBeenCalledWith('url');
        expect(
          mocks.paypalInContextExpressCheckout.checkout.closeFlow,
        ).not.toHaveBeenCalled();
        expect(
          mocks['Magento_Customer/js/customer-data'].invalidate,
        ).toHaveBeenCalled();
      });
      ...
      it('check request with error message', function() {
        ...
        expect(
          mocks['Magento_Ui/js/model/messageList'].addErrorMessage,
