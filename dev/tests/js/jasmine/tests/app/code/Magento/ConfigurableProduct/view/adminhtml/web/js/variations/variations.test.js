...
define(['Magento_ConfigurableProduct/js/variations/variations'], function(
  Variations,
) {
  'use strict';
  ...
  describe('Magento_ConfigurableProduct/js/variations/variations', function() {
    ...
    it('checks that "serializeData" serializes data', function() {
      ...
      expect(variation.source.data['configurable-matrix-serialized']).toEqual(
        resultMatrix,
      );
      expect(
        variation.source.data['associated_product_ids_serialized'],
      ).toEqual(resultIds);
    });
    ...
    it('checks that "serializeData" uses old data if there is no data to serialize', function() {
      ...
      variation.source.data['configurable-matrix-serialized'] = JSON.stringify(
        matrix,
      );
      variation.source.data[
    ...
    it('checks that "unserializeData" unserializes data', function() {
      var matrixString =
          '[{"name":"Small Product","attributes":"Size: small","price":5.5},' +
      ...
      expect(
        variation.source.data['configurable-matrix-serialized'],
      ).toBeUndefined();
      expect(
        variation.source.data['associated_product_ids_serialized'],
      ).toBeUndefined();
      expect(variation.source.data['configurable-matrix']).toEqual(
        resultMatrix,
      );
      expect(variation.source.data['associated_product_ids']).toEqual(
        resultIds,
      );
    });
