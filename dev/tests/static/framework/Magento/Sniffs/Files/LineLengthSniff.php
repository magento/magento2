 ...
 */
class Magento_Sniffs_Files_LineLengthSniff extends Generic_Sniffs_Files_LineLengthSniff
{
    /**
    ...
    protected function checkLineLength(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $lineContent)
    {
        ...
        $errorMessage = 'Variable is not allowed as the first argument of translation function, use string literal instead';
        if ($currentLineMatch) {
            $variableRegexp = '~__\(\$.+\)|Phrase\(\$.+\)~';
            if (preg_match($variableRegexp, $lineContent) !== 0) {
                $phpcsFile->addError($errorMessage, $stackPtr, 'VariableTranslation');
            }
            return;
