 ...
 */
class ProductMediaGalleryTest extends GraphQlAbstract
{
    /**
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertNotEmpty($response['products']['items'][0]['media_gallery_entries']);
        $mediaGallery = $response['products']['items'][0]['media_gallery_entries'];
        self::assertCount(2, $mediaGallery);
        self::assertEquals('Image Alt Text', $mediaGallery[0]['label']);
        self::assertEquals('image', $mediaGallery[0]['media_type']);
        self::assertStringContainsString('magento_image', $mediaGallery[0]['file']);
        self::assertEquals(['image', 'small_image'], $mediaGallery[0]['types']);
        self::assertEquals('Thumbnail Image', $mediaGallery[1]['label']);
        self::assertEquals('image', $mediaGallery[1]['media_type']);
        self::assertStringContainsString('magento_thumbnail', $mediaGallery[1]['file']);
        self::assertEquals(['thumbnail', 'swatch_image'], $mediaGallery[1]['types']);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertNotEmpty($response['products']['items'][0]['media_gallery']);
        $mediaGallery = $response['products']['items'][0]['media_gallery'];
        self::assertCount(2, $mediaGallery);
        self::assertEquals('Image Alt Text', $mediaGallery[0]['label']);
        self::assertEquals(1, $mediaGallery[0]['position']);
        self::assertFalse($mediaGallery[0]['disabled']);
        self::assertTrue($this->checkImageExists($mediaGallery[0]['url']));
        self::assertEquals('Thumbnail Image', $mediaGallery[1]['label']);
        self::assertEquals(2, $mediaGallery[1]['position']);
        self::assertFalse($mediaGallery[1]['disabled']);
...
QUERY;
        ...
        self::assertNotEmpty($response['products']['items'][0]['media_gallery']);
        $mediaGallery = $response['products']['items'][0]['media_gallery'];
        self::assertCount(1, $mediaGallery);
        self::assertEquals('Video Label', $mediaGallery[0]['label']);
        self::assertTrue($this->checkImageExists($mediaGallery[0]['url']));
        self::assertFalse($mediaGallery[0]['disabled']);
        self::assertEquals(2, $mediaGallery[0]['position']);
        self::assertNotEmpty($mediaGallery[0]['video_content']);
        $video_content = $mediaGallery[0]['video_content'];
        self::assertEquals('external-video', $video_content['media_type']);
        self::assertEquals('youtube', $video_content['video_provider']);
        self::assertEquals('http://www.youtube.com/v/tH_2PFNmWoga', $video_content['video_url']);
        self::assertEquals('Video title', $video_content['video_title']);
        self::assertEquals('Video description', $video_content['video_description']);
        self::assertEquals('Video Metadata', $video_content['video_metadata']);
    }
