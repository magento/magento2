...
QUERY;
    ...
    public function testLayeredNavigationForConfigurableProducts()
    {
        ...
        self::assertEquals(2, $response['products']['total_count']);
        self::assertNotEmpty($response['products']['aggregations']);
        self::assertNotEmpty($response['products']['filters'], 'Filters is empty');
        self::assertCount(2, $response['products']['aggregations'], 'Aggregation count does not match');
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        self::assertEquals(3, $response['products']['total_count'], 'Number of products returned is incorrect');
        self::assertTrue(count($response['products']['filters']) > 0, 'Product filters is not empty');
        self::assertCount(3, $response['products']['aggregations'], 'Incorrect count of aggregations');

        $productItemsInResponse = array_map(null, $response['products']['items'], $filteredProducts);
        for ($itemIndex = 0; $itemIndex < $countOfFilteredProducts; $itemIndex++) {
            self::assertNotEmpty($productItemsInResponse[$itemIndex]);
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        self::assertArrayNotHasKey('errors', $response, 'Response has errors.');
        self::assertEquals(3, $response['products']['total_count']);
        self::assertNotEmpty($response['products']['filters']);
        self::assertNotEmpty($response['products']['aggregations']);
    }
...
QUERY;
        ...
        self::assertEquals(3, $response['products']['total_count']);
        self::assertArrayHasKey('filters', $response['products']);
        self::assertCount(3, $response['products']['aggregations']);
        ...
        foreach ($layers as $layerIndex => $layerFilterData) {
            self::assertNotEmpty($layerFilterData);
            self::assertEquals(
        ...
        self::assertCount(7, $response['products']['aggregations'][1]['options']);
...
QUERY;
        ...
        self::assertNotEmpty($response['products']['filters'], 'filters is empty');
        self::assertNotEmpty($response['products']['aggregations'], 'Aggregations should not be empty');
        self::assertCount(3, $response['products']['aggregations']);
        ...
        self::assertCount(6, $actualCategoriesFromResponse);
        ...
        foreach ($categoryInAggregations as $index => $categoryAggregationsData) {
            self::assertNotEmpty($categoryAggregationsData);
            self::assertEquals(
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertEquals(1, $response['products']['total_count'], 'More than 1 product found');
        self::assertCount(2, $response['products']['aggregations']);
        $this->assertResponseFields(
        ...
        self::assertEquals('Price', $response['products']['aggregations'][0]['label']);
        self::assertEquals('Category', $response['products']['aggregations'][1]['label']);
        //Disable the product
...
QUERY;
        ...
        self::assertEquals(0, $response['products']['total_count'], 'Total count should be zero');
        self::assertEmpty($response['products']['items']);
        self::assertEmpty($response['products']['aggregations']);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertEquals(3, $response['products']['total_count'], 'Total count is incorrect');
        self::assertCount(2, $response['products']['aggregations']);
    ...
    {
        self::assertArrayHasKey('filters', $response['products'], 'Product has filters');
        self::assertIsArray(($response['products']['filters']), 'Product filters is not array');
        self::assertTrue(count($response['products']['filters']) > 0, 'Product filters is empty');
...
QUERY;
        ...
        self::assertArrayNotHasKey('errors', $resultAsc);
        ...
        self::assertEquals($expectedProductsAsc, $productsAsc);
...
QUERY;
        ...
        self::assertArrayNotHasKey('errors', $resultDesc);
        $productsDesc = array_column($resultDesc['products']['items'], 'sku');
        // position equal and secondary sort by entity_id DESC
        self::assertEquals($expectedProductsAsc, $productsDesc);
...
QUERY;
        ...
        self::assertEquals(array_reverse($expectedProductsAsc), $productsDesc);
    }
...
QUERY;
        ...
        self::assertArrayHasKey('total_count', $response['products']);
        self::assertEquals(2, $response['products']['total_count']);
        $this->assertProductItems($filteredChildProducts, $response);
        self::assertEquals(4, $response['products']['page_info']['page_size']);
        self::assertEquals(1, $response['products']['page_info']['current_page']);
        self::assertArrayHasKey('sort_fields', $response['products']);
        self::assertArrayHasKey('options', $response['products']['sort_fields']);
        self::assertArrayHasKey('default', $response['products']['sort_fields']);
        self::assertEquals('position', $response['products']['sort_fields']['default']);
        self::assertArrayHasKey('value', $response['products']['sort_fields']['options'][0]);
        self::assertArrayHasKey('label', $response['products']['sort_fields']['options'][0]);
        self::assertEquals('position', $response['products']['sort_fields']['options'][0]['value']);
    }
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        self::assertEquals(2, $response['products']['total_count']);
        self::assertEquals(['page_size' => 2, 'current_page' => 1], $response['products']['page_info']);
        self::assertEquals(
            [
        ...
        self::assertArrayHasKey('aggregations', $response['products']);
        self::assertCount(2, $response['products']['aggregations']);
        ...
        self::assertEquals($expectedAggregations, $response['products']['aggregations']);
    }
...
QUERY;
        ...
        self::assertEquals($expectedProducts, $actualProducts);
    }
...
QUERY;
        ...
        $response = $this->graphQlQuery($query);
        self::assertEquals(2, $response['products']['total_count'], 'Incorrect count of products returned');
        /** @var CategoryLinkManagement $productLinks */
        $productLinks = ObjectManager::getInstance()->get(CategoryLinkManagement::class);
        ...
        foreach ($response['products']['items'] as $itemIndex => $itemData) {
            self::assertNotEmpty($itemData);
            self::assertEquals($response['products']['items'][$itemIndex]['sku'], $links[$itemIndex]->getSku());
            /** @var ProductRepositoryInterface $productRepository */
            $productRepository = ObjectManager::getInstance()->get(ProductRepositoryInterface::class);
            /** @var ProductInterface $product */
            $product = $productRepository->get($links[$itemIndex]->getSku());
            self::assertEquals($response['products']['items'][$itemIndex]['name'], $product->getName());
            self::assertEquals($response['products']['items'][$itemIndex]['type_id'], $product->getTypeId());
            ...
            foreach ($categoryInResponse as $key => $categoryData) {
                self::assertNotEmpty($categoryData);
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertEquals(3, $response['products']['total_count']);
        self::assertNotEmpty($response['products']['filters'], 'Filters should have the Category layer');
        self::assertEquals('Colorful Category', $response['products']['filters'][0]['filter_items'][0]['label']);
        self::assertCount(2, $response['products']['aggregations']);
        $productsInResponse = ['Blue briefs', 'Navy Blue Striped Shoes', 'Grey shorts'];
        $count = count($response['products']['items']);
        for ($i = 0; $i < $count; $i++) {
            self::assertEquals($productsInResponse[$i], $response['products']['items'][$i]['name']);
        }
    }
...
QUERY;
        ...
        $this->assertProductItems($filteredProducts, $response);
        self::assertEquals(20, $response['products']['page_info']['page_size']);
        self::assertEquals(1, $response['products']['page_info']['current_page']);
    }
...
QUERY;
        ...
        self::assertCount(2, $response['products']['filters'], 'Filter count does not match');
        $productCount = count($response['products']['filters']);
        for ($i = 0; $i < $productCount; $i++) {
            self::assertEquals($filterNames[$i], $response['products']['filters'][$i]['name']);
        }
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertEquals(0, $response['products']['total_count']);
        self::assertEmpty($response['products']['items'], "No items should be returned.");
    }
