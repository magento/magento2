...
QUERY;
        ...
        self::assertArrayHasKey('websites', $response['products']['items'][0]);
...
QUERY;
    ...
    {
        if (!$this->cleanCache()) {
            self::fail('Cache could not be cleaned properly.');
        }
        $prductSku = 'simple';
...
QUERY;
        ...
        $productRepository = $this->objectManager->get(ProductRepositoryInterface::class);
        $product = $productRepository->get($productSku, false, null, true);
        self::assertNotNull($response['products']['items'][0]['product_links'], "product_links must not be null");
...
QUERY;
    ...
    private function assertMediaGalleryEntries($product, $actualResponse)
    {
        ...
        self::assertCount(1, $mediaGalleryEntries, "Precondition failed, incorrect number of media gallery entries.");
        self::assertIsArray(
            [$actualResponse['media_gallery_entries']],
            "Media galleries field must be of an array type."
        );
        self::assertCount(1, $actualResponse['media_gallery_entries'], "There must be 1 record in media gallery.");
    ...
    private function assertCustomAttribute($actualResponse)
    {
        ...
        self::assertEquals($customAttribute, $actualResponse['attribute_code_custom']);
    }
    ...
    private function assertOptions($product, $actualResponse)
    {
        ...
        self::assertNotEmpty($actualResponse['options'], "Precondition failed: 'options' must not be empty");
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertNotEmpty($response['products']['items'][0]['categories'], "Categories must not be empty");
        ...
        self::assertCount(1, $productItemsInResponse);
        self::assertCount(3, $productItemsInResponse[0]['categories']);
...
QUERY;
    ...
    {
        self::markTestSkipped('MC-30965: Product contains invalid categories');
...
QUERY;
        ...
        $productItemsInResponse = $response['products']['items'];
        self::assertCount(1, $productItemsInResponse);
        self::assertCount(2, $productItemsInResponse[0]['categories']);
        $categoriesInResponse = array_map(null, $categoryIds, $productItemsInResponse[0]['categories']);
        foreach ($categoriesInResponse as $key => $categoryData) {
            self::assertNotEmpty($categoryData);
            /** @var Category | Category $category */
