...
QUERY;
        ...
        self::assertArrayHasKey('children', $response['category']);
        self::assertCount(6, $response['category']['children']);
    }
...
QUERY;
        $response = $this->graphQlQuery($query);
        self::assertArrayHasKey('products', $response['category']);
        self::assertArrayHasKey('total_count', $response['category']['products']);
        self::assertGreaterThanOrEqual(1, $response['category']['products']['total_count']);
        self::assertEquals(1, $response['category']['products']['page_info']['current_page']);
        self::assertEquals(20, $response['category']['products']['page_info']['page_size']);
        self::assertArrayHasKey('sku', $response['category']['products']['items'][0]);
    ...
    public function testAnchorCategory()
    {
        ...
        self::assertNotEmpty($categoryId, "Preconditions failed: category is not available.");
        $query = <<<QUERY
{
  category(id: {$categoryId}) {
    name
...
QUERY;
        ...
        self::assertEquals($categoryId, $categoryList[0]['id']);
        self::assertEquals('Parent Image Category', $categoryList[0]['name']);
        $categoryList[0]['image'] = str_replace('index.php/', '', $categoryList[0]['image']);
        self::assertEquals($expectedImageUrl, $categoryList[0]['image']);

        $childCategory = $categoryList[0]['children'][0];
        self::assertEquals('Child Image Category', $childCategory['name']);
        $childCategory['image'] = str_replace('index.php/', '', $childCategory['image']);
        self::assertEquals($expectedImageUrl, $childCategory['image']);
    }
...
QUERY;
    ...
    private function assertWebsites($product, $actualResponse)
    {
        ...
        self::assertEquals($actualResponse, $assertionMap);
    }
    ...
    private function assertAttributes($actualResponse)
    {
        ...
        foreach ($eavAttributes as $eavAttribute) {
            self::assertArrayHasKey($eavAttribute, $actualResponse);
        }
    }
}
