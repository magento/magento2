...
class ObserverTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->_persistentSession = $this->getMockBuilder('Magento\Persistent\Helper\Session')
            ->disableOriginalConstructor()
        ...
        $this->_url = $this->getMockBuilder('Magento\UrlInterface')
            ->disableOriginalConstructor()
            ->setMethods([])
            ->getMock();

        $this->_expressRedirectHelper = $this->getMockBuilder('Magento\Checkout\Helper\ExpressRedirect')
            ->disableOriginalConstructor()
            ->setMethods(['redirectLogin'])
            ->getMock();

        $this->_model = $helper->getObject('Magento\Persistent\Model\Observer', array(
    ...
    public function testPreventExpressCheckoutOnline()
    {
        ...
        $this->_persistentSession->expects($this->once())
            ->method('isPersistent')
            ->will($this->returnValue(true));
    ...
    public function testPreventExpressCheckoutEmpty()
    {
        ...
        $this->_persistentSession->expects($this->any())
            ->method('isPersistent')
            ->will($this->returnValue(true));
        ...
        $expressRedirectMock->expects($this->any())
            ->method('getRedirectActionName')
            ->will($this->returnValue($expectedActionName));
        ...
        $this->_url->expects($this->once())
            ->method('getUrl')
            ->will($this->returnValue($expectedAuthUrl));
        $this->_expressRedirectHelper->expects($this->once())
            ->method('redirectLogin')
            ->with($expressRedirectMock, $expectedAuthUrl);
        $this->_model->preventExpressCheckout($this->_observer);
    }
}
