...
class ReaderTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testRead()
    {
        ...
        $this->assertSame('test1', $result['NL']['pattern_1']['example']);
        $this->assertSame('^[0-9]{4}\s[a-zA-Z]{2}$', $result['NL']['pattern_1']['pattern']);

        $this->assertSame('test2', $result['NL']['pattern_2']['example']);
        $this->assertSame('^[0-5]{4}[a-z]{2}$', $result['NL']['pattern_2']['pattern']);

        $this->assertArrayHasKey('NL_NEW', $result);
        $this->assertArrayHasKey('pattern_1', $result['NL_NEW']);

        $this->assertSame('test1', $result['NL_NEW']['pattern_1']['example']);
        $this->assertSame('^[0-2]{4}[A-Z]{2}$', $result['NL_NEW']['pattern_1']['pattern']);
    }
}
