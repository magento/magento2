...
class ChangelogTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testGetVersion()
    {
        ...
        $model->setViewId('test_view_id_2');
        $model->create();
        $this->assertSame(0, $model->getVersion());
        $changelogName = $this->resource->getTableName($model->getName());
        $this->connection->insert($changelogName, [$model->getColumnName() => mt_rand(1, 200)]);
        $this->assertSame($this->connection->lastInsertId($changelogName, 'version_id'), $model->getVersion());
        $model->drop();
    }
    ...
    {
        $this->assertSame(0, $this->model->getVersion());
        ...
        $this->assertSame(1, $this->model->getVersion());
        $this->model->clear(1);
        $this->assertSame(1, $this->model->getVersion()); //the same that a table is empty
    }
    ...
    public function testGetList()
    {
        ...
        $this->assertSame(5, $this->model->getVersion());
        $this->assertSame(3, count($this->model->getList(0, 5)));//distinct entity_ids
        $this->assertSame(3, count($this->model->getList(2, 5)));//distinct entity_ids
        $this->assertSame(2, count($this->model->getList(0, 3)));//distinct entity_ids
        $this->assertSame(1, count($this->model->getList(0, 2)));//distinct entity_ids
        $this->assertSame(1, count($this->model->getList(2, 3)));//distinct entity_ids
        $this->assertSame(0, count($this->model->getList(4, 3)));//because fromVersionId > toVersionId
    }
}
