...
class GeneralTest extends AbstractPlugin
{
    ...
    public function testMethodCanBePluginized()
    {
        ...
        $this->assertSame('<P:D>1: <D>test</D></P:D>', $subject->D('test'));
    }
    ...
    {
        $subject = $this->_objectManager->create(\Magento\Framework\Interception\Fixture\Intercepted::class);
        $this->assertSame('<IP:C><P:C><C>test</C></P:C></IP:C>', $subject->C('test'));
    }

    public function testInternalMethodCallsAreIntercepted()
    {
        $subject = $this->_objectManager->create(\Magento\Framework\Interception\Fixture\Intercepted::class);
        $this->assertSame('<B>12<IP:C><P:C><C>1</C></P:C></IP:C></B>', $subject->B('1', '2'));
    }

    public function testChainedMethodsAreIntercepted()
    {
        $subject = $this->_objectManager->create(\Magento\Framework\Interception\Fixture\Intercepted::class);
        $this->assertSame('<P:D>1: <D>prefix_test</D></P:D>', $subject->A('prefix_')->D('test'));
    }
    ...
    public function testFinalMethodWorks()
    {
        $subject = $this->_objectManager->create(\Magento\Framework\Interception\Fixture\Intercepted::class);
        $this->assertSame('<P:D>1: <D>prefix_test</D></P:D>', $subject->A('prefix_')->D('test'));
        $this->assertSame('<E>prefix_final</E>', $subject->E('final'));
        $this->assertSame('<P:D>2: <D>prefix_test</D></P:D>', $subject->D('test'));
    }
    ...
    public function testObjectKeepsStateBetweenInvocations()
    {
        $subject = $this->_objectManager->create(\Magento\Framework\Interception\Fixture\Intercepted::class);
        $this->assertSame('<P:D>1: <D>test</D></P:D>', $subject->D('test'));
        $this->assertSame('<P:D>2: <D>test</D></P:D>', $subject->D('test'));
    }
}
