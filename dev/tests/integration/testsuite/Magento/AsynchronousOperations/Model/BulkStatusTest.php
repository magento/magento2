...
class BulkStatusTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $this->assertSame(BulkSummaryInterface::NOT_STARTED, $this->model->getBulkStatus('bulk-uuid-1'));
        $this->assertSame(BulkSummaryInterface::IN_PROGRESS, $this->model->getBulkStatus('bulk-uuid-2'));
        $this->assertSame(BulkSummaryInterface::FINISHED_SUCCESSFULLY, $this->model->getBulkStatus('bulk-uuid-4'));
        $this->assertSame(BulkSummaryInterface::FINISHED_WITH_FAILURE, $this->model->getBulkStatus('bulk-uuid-5'));
    }
    ...
    public function testGetFailedOperationsByBulkId()
    {
        ...
        foreach ($operations as $operation) {
            $this->assertSame(1111, $operation->getErrorCode());
        }
        $operations =  $this->model->getFailedOperationsByBulkId('bulk-uuid-5', 2);
        foreach ($operations as $operation) {
            $this->assertSame(2222, $operation->getErrorCode());
        }
    }
}
